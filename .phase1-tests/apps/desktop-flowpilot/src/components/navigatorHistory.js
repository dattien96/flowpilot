...
exports.isSyncableRun = isSyncableRun;
...
function isSyncableRun(item, remoteChatSessions) {
    const runKind = item.runKind ?? "";
    const isSyncableKind = runKind === "chat" || runKind === "" || runKind === "workflow";
    if (!isSyncableKind || isAgentHistoryItem(item) || item.unavailableReason)
        return false;
    ...
    if (item.syncStatus === "synced" || item.syncStatus === "unsyncable")
        return false;
    if (remoteChatSessions && item.sourceMachineId && item.sourceRunId) {
        const alreadySynced = remoteChatSessions.some((remote) => remote.sourceMachineId === item.sourceMachineId && remote.sourceRunId === item.sourceRunId);
        if (alreadySynced)
            return false;
    }
    return true;
