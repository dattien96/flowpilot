...
exports.SupabaseAdminRepository = void 0;
...
function mapProject(row) {
    return {
        ...
        artifactStoragePreference: (row.artifact_storage_preference ?? "supabase"),
...
function mapSupportedModel(row) {
    return {
        id: String(row.id),
...
class SupabaseAdminRepository {
    ...
    async updateProject(projectId, patch) {
        ...
        if (patch.platform !== undefined)
            payload.platform = patch.platform;
        if (patch.repositoryUrl !== undefined)
            payload.repository_url = patch.repositoryUrl;
        if (patch.directoryPath !== undefined)
            payload.directory_path = patch.directoryPath;
        if (patch.status !== undefined)
            payload.status = patch.status;
        if (patch.artifactStoragePreference !== undefined)
            payload.artifact_storage_preference = patch.artifactStoragePreference;
        if (patch.defaultProvider !== undefined)
            payload.default_provider = patch.defaultProvider;
    ...
    async saveBinding(projectId, binding) {
        const payload = {
            id: binding.id,
    ...
    async updateIntegration(id, patch) {
        ...
        if (patch.configEncrypted !== undefined)
            payload.config_encrypted = patch.configEncrypted;
        if (patch.status !== undefined)
            payload.status = patch.status;
        if (patch.mcpTypeEnabled !== undefined)
            payload.mcp_type_enabled = patch.mcpTypeEnabled;
    ...
    async createSupportedModel(model) {
...
exports.SupabaseAdminRepository = SupabaseAdminRepository;
