...
- Follow-up 2026-09-06 (live request run-577686): plain text typed while parked IS the feedback — no `/continue` prefix needed (a chat turn would just 409 server-side). Footer hint updated (`type note + Enter to revise`). Refined same night per operator: [Revise] no longer prefills `/continue ` — it clears the composer for a plain note + flash hint (typing the prefix is not required anywhere).
- Follow-up 2026-09-06 (live-found run-584646, Tab-trap): ring focus stayed on after Revise armed the composer, so a second Enter with typed text re-fired Revise (wiping the note + re-toasting) instead of submitting. Fixed: non-empty input always submits while parked (mirrors the approval pattern); empty input keeps Enter-as-default-action.
