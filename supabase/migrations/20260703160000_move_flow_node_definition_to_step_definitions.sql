-- Migration: Move CP-42 flow node definition fields onto step_definitions.
--
-- Domain contract: FLOW = workflow, NODE = step. workflow_steps is the
-- workflow-to-step relation/order table; the node/step definition itself
-- belongs to step_definitions.
...
-- Compatibility migration for databases that already applied the earlier CP-42
-- relation-table shape. Fresh databases no longer have these workflow_steps
-- columns, so this block must be conditional and dynamic.
--
-- Important: do NOT copy node fields into the existing shared step_type row
-- (for example `flow-agent-delegate-reviewer`). Multiple graph nodes can share
-- that row, so updating it would collapse reviewer_correctness and
-- reviewer_security into one definition. Instead, create one node-specific
-- step_definition per legacy workflow_steps row, copy the old row's model and
-- prompt defaults, then repoint workflow_steps.step_type to that new definition.
do $$
begin
  if exists (
    select 1
