...
| `/image` | List/attach images — **Alt+V** / `/image paste`; click **`[N img]`** to manage (click **`[x]`** detach); `/image rm <n>` · `/image clear` · `/image open <n>` (codex/claude native; grok path-fallback) |
