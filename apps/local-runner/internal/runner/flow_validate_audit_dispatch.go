...
// "Stop happened (or the flow already finished) — do not resurrect inline
...
// closed and Err() is context.Canceled — used by flowInlineContext so a late
...
// fresh non-cancelled context — that was the bypass a late/queued inline
...
//
// Mapping:
//   - oracle.SuitePassed → ExitCode 0 (passed)
//   - oracle.HasRegression → ExitCode 1 + REGRESSION-labeled stderr (retry path)
//   - suite fail without regression → ExitCode 1 + new-failure label
func runValidateWithOracleIfPossible(ctx context.Context, command, workspaceRoot, workDir, baseSHA string, changedFiles []string) ValidationResult {
	...
	// AdvanceRetryState takes skipped_env_error — never regression retry.
	if oracle.EnvError != "" {
		return ValidationResult{
			Command:  cmd,
			WorkDir:  workDir,
...
func validateResultIsCancelled(ctx context.Context, result ValidationResult) bool {
	// BUG-288 R11 #1: check the caller's ctx directly and unconditionally first —
	...
	// a "passed" result advance validate → audit/done after Stop.
	if ctx != nil && ctx.Err() == context.Canceled {
		return true
	}
	// Parent/flow ctx canceled by Stop — but only when the EnvError also looks
	...
	// *suite* env error while the caller's flow ctx may still be open — treat
	// as validation failure (caller maps EnvError≠cancel), not flow cancel.
...
func (s *InteractiveService) runValidateNode(ctx context.Context, parentRunID string, edges []agentpack.FlowEdge, nodes []agentpack.FlowNode, node agentpack.FlowNode, resultMessage string) bool {
	...
	if state.Status == "skipped_no_command" {
		...
		// V9-01: no verified suite → do not advance audit→done. Escalate so the
	...
	// distinguish regression (baseline green → red) from ordinary new failures.
	// No baseline → keep RunValidationCommand. Never double-run the suite.
	...
	// Stop/cancel must not advance validate → audit/done as skipped_env_error.
	...
	// authoritative record — a crash right after would then either replay the
	...
	case "retrying":
		...
		// target succeeds — not before (spawn fail left DONE with no successor).
...
func (s *InteractiveService) advanceToNextInlineOrDelegate(ctx context.Context, parentRunID string, edges []agentpack.FlowEdge, nodes []agentpack.FlowNode, fromNodeID, when, resultMessage string) bool {
	...
	// V9-11: stamp source DONE only after successful terminal action / spawn —
...
func (s *InteractiveService) runAuditNode(ctx context.Context, parentRunID string, edges []agentpack.FlowEdge, nodes []agentpack.FlowNode, node agentpack.FlowNode, resultMessage string) bool {
	...
	// V10R4 P1: observation failure must NOT look like a clean empty diff —
	...
	// artifact — fail-closed instead: escalate and stop.
	if store := s.persistenceStore(); store != nil {
		if err := PersistAuditDraft(ctx, store, parentRunID, node.ID, draft); err != nil {
			...
			if _, ferr := s.applyFlowControl(parentRunID, FlowControlInput{
				Status:  "escalate",
				Summary: "Audit draft was built but could not be durably recorded (" + err.Error() + "). Not advancing until this is resolved — retry Continue once storage is available.",
			}); ferr != nil {
	...
	// settles successfully — not before (failed successor left DONE false).
...
// auditCtxCancelled is true when Stop cancelled the inline flow context —
