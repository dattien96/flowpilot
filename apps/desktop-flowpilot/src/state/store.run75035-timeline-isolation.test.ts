...
for (const provider of PROVIDERS) {
  test(`run75035: orchestration freeform message_delta does not bleed into focused child (${provider})`, async () => {
    const gate = deferred<void>();
    seed(
      makeClient({
        streamRun: async function* (runId: string): AsyncIterable<ProviderEventDTO> {
          if (runId !== "main-run") return;
          await gate.promise;
          yield {
            ...BASE_EVENT,
            id: "hub-delta",
    ...
    const before = useStore.getState().timeline.map((i) => ("text" in i ? i.text : ""));
    gate.resolve();
    ...
    const texts = useStore
      ...
      .join("\n");
    assert.equal(
      texts.includes("Đúng, đã xong rồi"),
      false,
      `hub freeform must not appear on focused child timeline (${provider}); before=${JSON.stringify(before)} after=${texts}`,
    );
  });
}

for (const provider of PROVIDERS) {
  test(`run75035: focusAgentRun ignores main-tagged freeform replay (${provider})`, async () => {
    seed(
      makeClient({
        resumeRun: async (runId) => ({
          runId,
    ...
    const assistantTexts = useStore
      .getState()
      .timeline.filter((i) => i.kind === "assistant")
      .map((i) => (i.kind === "assistant" ? i.text : ""));
    assert.deepEqual(assistantTexts, ["CHILD_OWN_ONLY"]);
    assert.equal(
      assistantTexts.some((t) => t.includes("Đúng, đã xong rồi")),
      false,
    );
  });
}

test("run75035: backToMainRun restores hub snapshot without child-only assistant", async () => {
  seed(
    makeClient({
      resumeRun: async (runId) => ({
        runId,
  ...
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(
    useStore.getState().timeline.some((i) => i.kind === "assistant" && i.text.includes("CHILD_OWN_ONLY")),
    true,
  );
  ...
  const texts = useStore
    ...
    .timeline.filter((i) => i.kind === "assistant")
    .map((i) => (i.kind === "assistant" ? i.text : ""));
  assert.equal(texts.includes("HUB_ONLY"), true);
  assert.equal(texts.some((t) => t.includes("CHILD_OWN_ONLY")), false);
});
