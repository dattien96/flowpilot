...
function WorkflowDetailPage() {
  ...
  const editorProjectId = workflow?.projectId ?? searchProjectId ?? "";
  const canEdit = Boolean(workflow && editorProjectId);
  ...
  const saveWorkflow = async () => {
    if (!workflow || !editorProjectId) {
      window.alert("Choose a project context before editing this workflow.");
      return;
    }
    ...
    try {
      const saved = await saveWorkflowUseCase.current.execute({
        id: workflow.projectId ? workflow.id : undefined,
        projectId: editorProjectId,
        name,
        description,
        isTemplate: workflow.projectId ? workflow.isTemplate : false,
      ...
      if (saved.id !== workflow.id) {
        await navigate({
          ...
          search: { projectId: editorProjectId },
          replace: true,
        });
      }
    ...
    >
      <div className="grid gap-4 rounded-[1.5rem] border border-border bg-background/60 p-5 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border bg-card px-4 py-3">
          ...
          <p className="mt-2 text-sm font-medium">
            {workflow.projectId
              ? "Editable private workflow"
              : canEdit
                ? "Editable as private project copy"
                : "Read-only global definition"}
          </p>
        </div>
      </div>

      {!isPrivateWorkflow && (
        <div className="rounded-[1.5rem] border border-dashed border-border bg-background/40 p-5 text-sm text-muted-foreground">
          Global workflows stay reusable for the whole workspace. When you edit here, FlowPilot saves the result as a private workflow for the selected project.
        </div>
      )}
      ...
      <div className="grid gap-4 rounded-[1.5rem] border border-border bg-background/60 p-5 md:grid-cols-2">
        ...
        <label className="space-y-2 text-sm">
          <span className="font-medium">Run in project</span>
          <select
            className="w-full rounded-2xl border border-border bg-card px-4 py-3"
            disabled={isPrivateWorkflow}
            value={effectiveRunProjectId}
