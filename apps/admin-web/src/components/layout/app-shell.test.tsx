...
describe("AppShell", () => {
  ...
  it("includes the MCP Servers parent entry and child settings routes", () => {
    expect(
      settingsNavItems.some((item) => item.to === "/artifacts" && item.label === "Artifacts"),
    ).toBe(true);
    const mcpParent = settingsNavItems.find((item) => item.label === "MCP Servers");
    expect(mcpParent).toBeDefined();
    expect(mcpParent?.to).toBeUndefined();
    expect(mcpParent?.children?.map((item) => item.to)).toEqual([
