...
- `D-2` **`r-requirement` as a vibe-only, always-block gate.** Add one new rule to `flowgate.DefaultRules()`:
  ```go
...
- **unit:** `go test ./internal/agentpack` — `LoadBuiltinPack()` + `ValidateFlowDefinition`/`ValidateFlowSafetyTopology` + `validateManifestFlowMatchesDefinition` for all 6 flows (existing 3 + 3 vibe, now `selectableIn: []`). `go test ./internal/flowgate` — `r-requirement` (vibe-only, `always-block`) triggers on `Tests.Green && (RequirementDrift||Tampered)==true` as `block`, and the resolver walks `Violations` to `ask_user` only in `vibe`; `dev` hides the rule even if drift advisory were present.
