...
- `F-3` Control tool never called / cap hit → engine escalates (ask user) or terminates per policy; never loops unbounded (SS-16 `BR-6`).
  ...
  - **BUG-234 auto-advance gate**: once a loop has settled (`blocked` awaiting-user, `done`, `stopped`, `paused`), ALL auto-advance paths must stop — not just the hub reinvoke. This includes forward-edge node spawning (`tryAdvanceFlowFromNode`), the cohort-join hub-`RUNNING` transition, and dependent-agent release/resume. Gating only the hub reinvoke (as originally done) settled the loop STATE correctly but let the step/spawn machinery run away: a child completion arriving after the block kept re-spawning the next round's cohort and flipping the hub node back to `RUNNING`, so the synthesis step spun forever. The active-status check is applied at every advance/spawn site (including a per-spawn re-check to close the check-then-spawn race where a concurrent hub turn escalates mid-advance). A `continue`/resume flips the loop back to `running`, at which point advancing legitimately resumes.
  - **BUG-234 cohort node settlement**: a cohort member settles its OWN step-timeline node (`DONE` on completion, `FAILED` on failure) the moment it finishes — independent of the cohort barrier — so one-done-one-running reads honestly instead of showing every member `RUNNING` until the join. The barrier's bulk reviewer-`DONE` write remains as an idempotent backstop. Note: the reuse-vs-spawn split between a back-edge target (re-prompted in place) and a forward-edge target (spawned fresh) is driven purely by edge `Kind`; the `FlowNode.Lifecycle` attribute is not consulted for that decision.
