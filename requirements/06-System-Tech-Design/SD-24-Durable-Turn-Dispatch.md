...
> Operation names are verified in adapter code. The initial V2 rollout is limited to Codex + Grok + Claude, whose Task-257 probes concluded **unprovable (as a receipt) ⇒ `uncertain`** (no acceptance seam, no attach). Gemini is explicitly deferred and must not be V2-enabled until its own recorded outcome replaces its *verify live* cells. The design is safe under either answer: no proof ⇒ `uncertain`.
...
| Claude | spawn CLI process | `writeUserTurn` stdin (`claude_adapter.go:173`) — stdin write success is **not** a receipt | **none** — live kill-mid-turn probe found a transient stream `message_start` (not durable) and a durable but CLI-internal/undocumented `queue-operation dequeue` line; neither is a stable ReceiptID; `Accepted` is not wired ([evidence](./evidence/SD-24/claude.md)) | **evidenced, non-authoritative**: durable per-session JSONL (`~/.claude/projects/<hash>/<session-id>.jsonl`) distinguishes completed (trailing `assistant`+`last-prompt`) from in-flight/crashed (ends at `dequeue`/`ai-title`) — corroborating signal only, not `TerminalEvidence` | **proven none** — live probe: `--resume <session-id>` after a mid-turn kill starts a new turn with no memory of the interrupted one; respawn = new turn | V2 enabled only with three-outcome / `uncertain` handling |
