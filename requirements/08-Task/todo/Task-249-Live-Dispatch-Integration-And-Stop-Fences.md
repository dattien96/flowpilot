...
- **Consume** the scoped SD-24 §6.3 Task-257 matrix: Codex/Grok are eligible for V2 but have the explicit negative result `unprovable ⇒ uncertain`, therefore neither has an `Accepted` call site; Claude/Gemini are V2-disabled until their deferred evidence completes.
...
- `Q-2` **Resolved by SD-24 §6.3** (scoped capability matrix). Codex/Grok have the recorded negative outcome `unprovable ⇒ uncertain`; no receipt seam is wired. Claude/Gemini evidence is deferred and both providers are V2-disabled. A future enablement repeats Task-257; no proof remains `uncertain`.
...
- `T-7` Preflight: assert the Codex/Grok matrix rows have no *(verify live)* cells and enforce their recorded **no `Accepted` seam**; assert Claude/Gemini are rejected from V2 provider enablement until `CE-CL/GEM` closes. Never infer an acceptance signal.
...
**Step 4 — receipt → atomic commit.** Add `TurnBridge.Accepted(ReceiptEvidence)` beside `Emit`, and make it the **only** caller of receipt commit. The scoped Task-257 outcome supplies **no** Codex/Grok adapter `file:function`: session creation (`thread/start`/`session/new`) and their observed headless output have no call path. Claude/Gemini are V2-disabled, not fallback receipt candidates. Keep `Accepted` implemented only as the safe generic store seam for a future evidence-backed provider; no current adapter calls it. In `Accepted`:
...
```
`turnBridge` owns `svc *InteractiveService`, `ctx context.Context`, `rs *interactiveRun`, and `turnID string`; `ReceiptEvidence` is the SD-24 §6.3a canonical payload. Reject/log Accepted before `send_started` or after terminal; equal receipt identity **and canonical payload hash** replays are no-ops, but a different payload for the same identity is `ErrReceiptConflict`. Codex/Grok have no concrete adapter calls under the current evidence, so post-send recovery remains `uncertain`; Claude/Gemini remain V2-disabled. A future provider call is allowed only after its own Task-257 matrix row becomes evidence-backed.
...
- `V-4` Scoped capability seam test: Codex/Grok `thread/start`/`session/new`/headless output never calls `CommitReceiptAndClearIntent`; Claude/Gemini cannot be V2-enabled. A future evidence-backed provider test must prove the sole Accepted caller.
- `V-5` Scoped capability matrix: Codex/Grok have evidence links and zero *(verify live)* cells; Claude/Gemini are explicitly deferred + V2-disabled rather than silently assumed safe.
...
- result: **done — live path CreatePrepared/send_claimed/send_started linearization, TurnBridge.Accepted/Terminal seams, Claude/Gemini V2-disabled, Codex/Grok three-outcome only.**
