...
- `Q-1` **Where does the diff+upsert run?** (A, recommended) Desktop-only: the desktop already loads `provider.models[]` from `GET /providers`, so it can diff client-side and call `createSupportedModel` per missing model — no new runner endpoint. (B) Mirror admin-web's assumed `POST /providers/import` by adding that endpoint to the Go runner (`cli/root.go` currently has `/providers`, `/providers/install`, `/providers/auth` but NOT `/providers/import`) and have the desktop consume a normalized payload. (A) is less code and reuses data already on-screen; (B) centralizes the detection-method/version stamping server-side. Pick one during implementation.
...
- [ ] `DOD-1` Migration adds `grok` to the `ai_supported_models.provider_key` CHECK constraint, idempotently, with no row changes; a `grok` model inserts successfully.
- [ ] `DOD-2` Runner `detectGrokModels` returns the live model list from `~/.grok/models_cache.json` (hidden/unsupported filtered), wired into `resolveProviderModels`; `GET /providers` reports real Grok models with a `source`.
- [ ] `DOD-3` Desktop "Detect models" button diffs detected-vs-registered by `model_id` and inserts only missing models as `source:"detected"` with detection provenance; shows an import summary; refreshes the list.
- [ ] `DOD-4` Sync never overwrites an existing row's `is_enabled`/`display_name`/`sort_order`/`source` (skip-if-exists); re-running is idempotent.
- [ ] `DOD-5` Detected models render a "detected" badge distinct from manual rows.
- [ ] `DOD-6` Manual "Add Supported Model" + Codex/Claude/Gemini detection are unchanged (regression).
- [ ] `DOD-7` Tests: `detectGrokModels` unit (cache fixture), desktop diff/skip logic, migration check.
