...
- follow-ups: no live browser verification was possible (the desktop app requires real Supabase sign-in with no demo/bypass mode); the migration needs a real apply pass against a live Supabase project; wire per-step flow-engine fields through to the `workflow_steps` instance row (extend `WorkflowStep`/`saveWorkflow`/`cloneWorkflow`) so per-instance overrides actually persist, and fix the resulting `workflowFlowEngineAttrs.test.ts` compile failure; add client-side validation for unknown refs / invalid join-policy values.
