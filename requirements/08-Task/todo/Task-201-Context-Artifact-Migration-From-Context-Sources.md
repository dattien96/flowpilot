...
- `T-1` `ArtifactInstance.config_json.sources` is the long-term home for selected context source IDs.
...
- `Q-1` Should each existing workflow get one default context artifact instance during migration, or should only newly edited workflows use instances? Proposed: create defaults lazily when edited.
