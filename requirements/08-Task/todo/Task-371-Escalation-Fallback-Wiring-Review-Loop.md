...
- Parent Documents: [CP-65 P-4](../../07-Coding-Plan/todo/CP-65-Multi-Candidate-Tournament-Harness.md)
...
- [ ] AC-1: Flag bật, review loop đạt cap → run chuyển `tournament_escalation`, tournament-harness được dispatch với đúng intent/contract của run kẹt.
- [ ] AC-2: Flag bật, owner debate stall → cùng nhánh escalation (không parked-cap terminate).
- [ ] AC-3: Tournament merge thành công → run gốc resume validate/reviewer với code của winner; hòa + user từ chối → run gốc về escalate cũ (fail-closed).
- [ ] AC-4: Flag unset → hành vi byte-identical cũ (escalate card, parked cap) — không tournament, không state mới.
- [ ] AC-5: Không đệ quy: tournament flow chạm cap → escalate như cũ, không tự escalate thêm lần nữa.
- [ ] AC-6: 2 test signatures green:
  - `TestReviewLoopTriggersTournamentOnCapExceeded`
  - `TestVibeDebateTriggersTournamentOnStall`
- [ ] AC-7: Attempt được theo dõi từ escalation (`attempt: 1`) qua các vòng retry (tối đa `max_attempts`); hết lượt hoặc merge-conflict → `ask_user`, không retry thêm, không đệ quy tournament-con.
