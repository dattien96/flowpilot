...
- [x] Provider parity N/A — pure client view cache (noted in §11)
- [x] No `_runViews`/parallel cache registry added; existing `_runSnapshots` semantics extended
- [x] Pinned current/main/active ancestry cannot be evicted; cache miss has fetch fallback, never silent no-op
- [x] Every cache hit revalidates; rapid-switch stale-response and mutable-alias tests green
- [x] Stable scroll anchor, delete cleanup, memory cap, and Task-404 attention regression verified
- [x] `feature_key` = `project-nav` (view-cache là navigation UX); CA entry written
- [x] §8 acceptance checks verified by hand or test
- [x] GitNexus impact run for `cacheRunSnapshot`, `restoreRunSnapshot`, `openHistoryRun`, `backToMainRun` before code
- [x] GitNexus `detect_changes` shows only expected symbols before commit

## 11. Completion Notes

- result: DONE — _runSnapshots extended (no parallel _runViews): bounded LRU + pin set (focused/main/active ancestry), clone-on-capture/restore, stable scroll anchor, dirty-on-newer-mux-revision + always-revalidate, generation guards, cache-miss fetch fallback, selectProject/deleteHistoryRun pruning, Task-404 preserved. 10 tests green; clean-HEAD baseline reproduced identical failures -> zero regressions.
- follow-ups: none.
- upstream docs updated: CP-84, CA-934.
