...
- [ ] `DOD-01` Add `provider_account_id` to `ndjsonSessionRecord` with `omitempty`.
- [ ] `DOD-02` Map `ProviderSessionState.ProviderAccountID` through `sessionRecordFrom`.
- [ ] `DOD-03` Map `provider_account_id` through `sessionStateFromRecord`.
- [ ] `DOD-04` Existing `sessions.ndjson` lines without `provider_account_id` still load without error.
- [ ] `DOD-05` `UpsertProviderSession` remains last-wins by `run_id` for both in-memory state and NDJSON reload.
- [ ] `DOD-06` `sessionStateOf` persists the real provider resume handle when known, falling back to the synthetic FlowPilot session id only when no real handle is available.
- [ ] `DOD-07` `interactiveRun` stores the real provider session id separately from the synthetic FlowPilot id where needed.
- [ ] `DOD-08` Claude real session id captured from provider output is persisted back to the session store.
- [ ] `DOD-09` Codex rollout session id is persisted when exposed; if not exposed, a documented rollout discovery fallback is implemented.
- [ ] `DOD-10` Persisted provider session rows include `RunKind`, `LastPrompt`, `LastMessage`, `StartedAt`, and `UpdatedAt` exactly as BUG-080 requires.

### Post-Restart Run Reconstruction

- [ ] `DOD-11` `SessionHistoryReader` exposes `GetProviderSession(ctx, runID) (ProviderSessionState, bool, error)`.
- [ ] `DOD-12` `localFileSessionStore.GetProviderSession` reads from the loaded last-wins in-memory session map.
- [ ] `DOD-13` `SupabaseWorkflowStore` either implements the new lookup or the interface split keeps Supabase compilation and history behavior intact.
- [ ] `DOD-14` `resumeRun` keeps the existing in-memory fast path unchanged when `s.runs[runID]` exists.
- [ ] `DOD-15` `resumeRun` falls back to persisted session lookup when `s.runs[runID]` is missing.
- [ ] `DOD-16` Missing persisted session still returns `run_not_found`.
- [ ] `DOD-17` Non-chat restored runs are blocked with a typed unsupported/greyout-safe response for MVP.
- [ ] `DOD-18` Chat restored runs are reconstructed into `interactiveRun` with provider key, account id, cwd, status, prompt/message, timestamps, and real provider session id.
- [ ] `DOD-19` Reconstructed chat runs are registered back into `s.runs`.
- [ ] `DOD-20` Reconstructed chat runs reseed the deterministic `chat-<runId>` step.
- [ ] `DOD-21` Returned `RunHandle` includes the correct `StepID` for chat runs after reconstruction.

### Provider Account Home And Session Files

- [ ] `DOD-22` Account home resolution works for explicit provider accounts.
- [ ] `DOD-23` Account home resolution falls back correctly for empty/default account ids.
- [ ] `DOD-24` Missing account home produces a typed, user-safe error.
- [ ] `DOD-25` Provider auth is checked in the active account home before attempting cross-account resume.
- [ ] `DOD-26` Missing active-account auth produces a typed greyout-safe error.
- [ ] `DOD-27` Codex session locator finds rollout files under `<CODEX_HOME>/sessions/**/rollout-*-<sessionID>.jsonl`.
- [ ] `DOD-28` Claude session locator finds session files under the provider account's Claude projects directory.
- [ ] `DOD-29` Claude relocation preserves or derives the correct project hash directory from the source path.
- [ ] `DOD-30` Relocation copies provider session files additively and never overwrites unrelated files.
- [ ] `DOD-31` Session file contents are never logged.
- [ ] `DOD-32` Missing provider session file returns `session_unavailable` or equivalent typed error with message `session data not found on this machine`.

### Cross-Account Resume

- [ ] `DOD-33` If persisted `provider_account_id` matches `activeAccountID`, resume does not relocate files.
- [ ] `DOD-34` If persisted `provider_account_id` differs from `activeAccountID`, the runner attempts file relocation into the active account home.
- [ ] `DOD-35` Successful cross-account preparation re-points the same `run_id` to the active `provider_account_id`.
- [ ] `DOD-36` Re-pointing persists via `UpsertProviderSession`.
- [ ] `DOD-37` Re-pointing does not create a second logical history run.
- [ ] `DOD-38` If provider resume rejects the relocated file, the run remains visible in history and becomes disabled/greyed-out with a clear reason.
- [ ] `DOD-39` No transcript-history injection fallback is implemented.

### Claude Resume Path

- [ ] `DOD-40` Restored Claude runs seed the process pool with the real session id before the next turn.
- [ ] `DOD-41` Claude next turn uses `--resume <realSessionId>` and never resumes with the synthetic FlowPilot id.
- [ ] `DOD-42` Claude turn environment points to the active account home after cross-account relocation.
...
- [ ] `DOD-45` Fresh Codex runs continue to use the existing app-server path.
- [ ] `DOD-46` Reopened/restored Codex chat runs use a CLI-backed resume path.
- [ ] `DOD-47` Codex resume command uses `codex exec resume <rolloutSessionID>` with `CODEX_HOME` set to the active account home.
- [ ] `DOD-48` Codex resume command runs from the restored run working directory.
- [ ] `DOD-49` Codex sandbox and approval posture are derived from the existing yolo policy helper.
- [ ] `DOD-50` Codex resume output is mapped into provider events that the desktop can render.
- [ ] `DOD-51` If structured Codex JSON streaming is unavailable, final stdout is emitted as one completed assistant message plus turn-completed event.
...
- [ ] `DOD-54` History list remains unified across provider accounts.
- [ ] `DOD-55` No account badges, account grouping, or account filters are added.
- [ ] `DOD-56` Clicking a resumable post-restart history item opens it instead of returning `run_not_found`.
- [ ] `DOD-57` Clicking a resumable cross-account history item opens the same `run_id`.
- [ ] `DOD-58` On typed resume errors, the history item is shown greyed-out / disabled.
- [ ] `DOD-59` Greyed-out items show the server-provided reason in tooltip or equivalent UI affordance.
- [ ] `DOD-60` Greyed-out click handling does not navigate, clear current timeline, or crash.
- [ ] `DOD-61` Successful resume still clears/rebuilds the timeline through the existing stream replay path.
- [ ] `DOD-62` Existing in-memory resume behavior is unchanged.

### Compatibility Canaries

- [ ] `DOD-63` `scripts/quicktest.ps1` includes a session portability section.
- [ ] `DOD-64` Quicktest checks that the Codex `exec resume` CLI surface still exists.
- [ ] `DOD-65` Quicktest checks enough Codex rollout metadata shape to catch account/auth fields being introduced.
- [ ] `DOD-66` Quicktest includes a Claude session-store/path canary where feasible without spending tokens.
- [ ] `DOD-67` The Check Version flow and `quicktest.ps1` continue to use `.flowpilot/settings/compat-config.json` baseline values.

### Cross-PC Readiness Only

- [ ] `DOD-68` The implementation keeps `run_id` stable and provider/account pointers mutable so Task-069 can re-point restored runs later.
- [ ] `DOD-69` The implementation does not hard-code assumptions that prevent later workflow-run reconstruction by `workflow_id`.
- [ ] `DOD-70` The implementation does not add automatic Google Drive chat sync.
- [ ] `DOD-71` The implementation does not attempt to solve PC-to-PC `run_id` collisions in this slice.

### Tests And Regression Guards

- [ ] `DOD-72` Local file session store tests cover `provider_account_id` round-trip.
- [ ] `DOD-73` Local file session store tests cover last-wins re-pointing of `provider_session_id`.
- [ ] `DOD-74` Local file session store tests cover old NDJSON records without `provider_account_id`.
- [ ] `DOD-75` Unit tests cover `GetProviderSession` found and not-found behavior.
- [ ] `DOD-76` Unit tests cover `resumeRun` fallback reconstruction from persisted session.
- [ ] `DOD-77` Unit tests cover unsupported non-chat restore behavior.
- [ ] `DOD-78` Unit tests cover cross-account missing source file error mapping.
- [ ] `DOD-79` Unit tests cover cross-account missing active-account auth error mapping.
- [ ] `DOD-80` Unit tests cover successful re-point persistence after relocation.
- [ ] `DOD-81` Unit or integration tests cover Claude real-session persistence.
- [ ] `DOD-82` Unit or integration tests cover Codex resume command construction.
- [ ] `DOD-83` Desktop tests or focused manual QA cover greyed-out history item behavior.
- [ ] `DOD-84` Existing BUG-060 and BUG-080 history tests still pass.
- [ ] `DOD-85` `go test ./internal/runner/...` passes in `apps/local-runner`.
- [ ] `DOD-86` Desktop TypeScript typecheck/build passes for touched UI/client files.
- [ ] `DOD-87` No new test relies on real provider tokens unless marked manual.
...
- [ ] `DOD-94` All HIGH/CRITICAL GitNexus impact warnings, if any appeared during implementation, were reviewed before edits proceeded.
...
- [ ] `DOD-97` Any unimplemented checklist item is moved to a named follow-up task with an explicit reason.
- [ ] `DOD-98` Completion notes in Task-067, Task-068, Task-069, and this file are updated if implementation status changes their meaning.
