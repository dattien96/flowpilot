...
- Related Documents: [Task-303: Opencode Chat Flow YOLO Cards](../../08-Task/done/Task-303-Opencode-Chat-Flow-Model-Reasoning-YOLO-Cards-Tools.md), [Task-401: Devin Adapter MVP](../../08-Task/todo/Task-401-Devin-Controlled-Adapter-MVP.md), [CP-70-note.md](../../07-Coding-Plan/note/CP-70-note.md), [CP-70-Test-Steps.md](../../07-Coding-Plan/todo/CP-70-Test-Steps.md)
- Replaces: `None`
...
- Coding Plan: [CP-70: Devin Provider Integration](../../07-Coding-Plan/todo/CP-70-Devin-Provider-Integration.md) (Work Breakdown `P-5`, `P-6`, `P-8`, `P-9`, `P-10`, `P-11`)
...
- [ ] `DOD-1` When YOLO is OFF, file edit or bash command triggers `permission_required` modal; Deny cleanly halts turn (`TestDevinApprovalGateDenyCleanExit`).
- [ ] `DOD-2` When YOLO is ON, eligible actions are auto-approved; `ask_user` unconditionally blocks for user input (`TestDevinYoloOnQuestionGuard`).
- [ ] `DOD-3` `ask_user` renders dialog in Desktop and resumes turn with user response (`TestDevinAskUserRoundTrip`).
- [ ] `DOD-4` `spawn_agent` executes child on isolated process without resetting parent MCP connection (`TestDevinSpawnAgentChildIsolation`).
- [ ] `DOD-5` Child processes are reclaimed via `CloseProcessesForChildRun` after completion (`TestDevinChildProcessReclamation`).
- [ ] `DOD-6` Chat postures `scan` and `plan` block write operations; `code` permits them (`TestDevinChatPostureEnforcement`).
- [ ] `DOD-7` Tab key cycles between `plan` and `code` postures smoothly (`TestDevinPostureTabCycle`).
- [ ] `DOD-8` One-shot summary executes via `devin -p` (hoặc `acp --agent-type summarizer`) và prior summary được inject vào turn kế tiếp (`TestDevinOneShotSummarizer`).
- [ ] `DOD-9` Flow mode runs with Devin; gate violations trigger repair prompts on Devin (`TestDevinFlowModeGates`).
- [ ] `DOD-10` Vision Guard blocks image drops on UI when Devin is selected (`TestDevinVisionGuardBlocked`).
- [ ] `DOD-11` Google Drive `/sync` and `/restore` preserve Devin sessions (`TestDevinDriveSyncManifest`).
- [ ] `DOD-12` Complete manual test run recorded in `CP-70-Test-Steps.md` with all 17 sections marked PASS.
