...
- result: Not implemented in this turn (planning only, per request). Design captured from a multi-round discussion and reconciled against BUG-160/162/163/164/165 and SS-05/SD-06; code deferred.
- follow-ups:
  - Resolve `Q-1`..`Q-4` before implementation — especially Q-2 (built-in `model_override` write path, since the Go `WorkflowStore` only reads/patches today) and Q-1 (whether Project default is the intended last resort).
  - Update SS-05 §2.1/§3 and SD-06 §6/§7 to remove the `gpt-5.4` hard floor and document the mandatory-model + greyed/unrunnable rule and built-in `workflows.model_override`/YOLO mode editability (SS-13 §7.3 upstream-update trigger — required before or alongside implementation).
  - Confirm interaction with BUG-171 (provider reconciliation) to avoid duplicate logic.
- upstream docs updated: None yet — SS-05 / SD-06 updates flagged above as required.
