...
- Parent Documents: [CP-65 P-3](../../07-Coding-Plan/todo/CP-65-Multi-Candidate-Tournament-Harness.md)
...
- [ ] AC-7: Hòa/tất cả đỏ + còn lượt + `auto_pick=true` → back-edge retry về `parallel_rollout` với brief chưng cất, spawn run candidate mới (không tái dùng run cũ); hết 2 attempts hoặc merge-conflict → `ask_user`, không retry thêm; `max_attempts` default 1 giữ đúng hành vi plan cũ.
