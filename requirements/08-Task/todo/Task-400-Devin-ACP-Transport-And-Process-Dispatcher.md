...
- Related Documents: [Task-300: Opencode ACP Transport And Process/Dispatcher](../../08-Task/done/Task-300-Opencode-ACP-Transport-And-Process-Dispatcher.md), [CP-70-note.md](../../07-Coding-Plan/note/CP-70-note.md), [CP-70-Test-Steps.md](../../07-Coding-Plan/todo/CP-70-Test-Steps.md)
- Replaces: `None`
...
- Coding Plan: [CP-70: Devin Provider Integration](../../07-Coding-Plan/todo/CP-70-Devin-Provider-Integration.md) (Work Breakdown `P-1`, `P-2`)
...
- [ ] `DOD-1` All ACP message types serialize and deserialize through JSON without data loss (`TestDevinACPTypesRoundTrip`).
- [ ] `DOD-2` Standalone ACP primitives exist; `opencode_*.go`, `grok_*.go`, `gemini_*.go` have zero git diff.
- [ ] `DOD-3` `devinDispatcher` multiplexes concurrent sessions over a single stdio pipe correctly (`TestDevinDispatcherMultiplexesConcurrentSessions`).
- [ ] `DOD-4` `ensureDevinProcess` reuses process handle for matching scopeKey and creates isolated handle for `child:<runId>` (`TestEnsureDevinProcessReusesOrRespawns`).
- [ ] `DOD-5` `CloseProcessesForChildRun` terminates and deletes child processes cleanly without touching parent process (`TestCloseProcessesForChildRun`).
- [ ] `DOD-6` Process launch isolates environment: host secrets stripped, `HOME`/`XDG_CONFIG_HOME`/`XDG_DATA_HOME` trỏ managed home (`TestDevinProcessEnvIsolates`).
- [ ] `DOD-7` Credential redaction replaces authorization headers and token fields with `[redacted]` (`TestRedactDevinFrameForLogStripsCredentials`).
- [ ] `DOD-8` Base-regression guard: `go test ./internal/runner -run 'TestOpencode|TestGrok|TestGemini|TestCodex|TestClaude'` passes 100%.
- [ ] `DOD-9` Boot sequence gọi `initialize → authenticate` trước `session/new`; authenticate lỗi → typed `auth_required` (`TestDevinProcessAuthenticateBeforeSessionNew`, `TestDevinAuthenticateFailureTypedError`).
- [ ] `DOD-10` Dispatcher bỏ qua êm `_cognition.ai/*` và unknown methods (`TestDevinDispatcherToleratesExtensionNotifications`).
