...
- [ ] `DOD-2` `spawn_agent` works end to end for Grok with both wait modes and correct child persistence/panel visibility.
- [ ] `DOD-3` Google Drive (or another configured external MCP server) is visible to Grok in the same turn as FlowPilot tools, or is explicitly and honestly marked unsupported.
- [ ] `DOD-4` `OfferReviewOutcomeTool` gating parity is implemented and tested.
- [ ] `DOD-5` Spawn-prompt composition parity test passes against Codex/Claude baselines.
- [ ] `DOD-6` MCP-ready gate withholds the prompt until FlowPilot tools connect; slow init still yields `ask_user`/`spawn_agent` on turn 1 (`GR-30`, BUG-114).
- [ ] `DOD-7` Resumed Grok turns re-attach `mcpServers` + permission channel; gated tool + `ask_user` work after resume (`GR-31`, BUG-087).
- [ ] `DOD-8` FlowPilot tool names do not collide with native `spawn_subagent`/`ask_user_question`; normalized at the UI boundary (`GR-07`, BUG-124).
- [ ] `DOD-9` UI-initiated child spawn emits `EventAgentSpawnedByUser` and injects into the Grok parent's next turn (`GR-22`, BUG-121).
- [ ] `DOD-10` `defaultModelForProvider("grok")` returns a valid default; cross-provider spawn onto Grok with no model does not fail (additive edit).
- [ ] `DOD-11` **Base-regression (`P-0`):** shared `claudeMCP`/`agent_orchestrator`/`defaultModelForProvider` are reused/appended-only; Codex/Claude MCP + spawn_agent tests remain green and unchanged.
