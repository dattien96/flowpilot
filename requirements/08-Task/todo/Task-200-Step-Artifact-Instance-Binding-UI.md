...
- `T-1` Step form binds `artifact_instance_id`, not raw producer config.
- `T-2` Slot compatibility is checked by artifact type and direction.
...
- `Q-1` Should output slots require selecting an instance ahead of time, or can producer nodes create ephemeral run instances? Proposed v1: select configured instance for authoring clarity.
