...
- verified 2026-07-06: all of this task's own named tests pass (`BuiltinOrchestrationOptions*`, `HandleListBuiltinOrchestrationOptions*`, `StartResolvedFlow*`, `StartTurnWithFlowRef*`, `FlowNodeAgentName*`, plus the desktop `store.test.ts` builtin-orchestration cases) and `go build ./...` is clean — 6 of 7 §6 Acceptance Check items are fully confirmed. The 7th ("Existing CP-36 review-loop behavior is preserved") has one caveat: `TestE2EReviewLoopSynthesisFallbackEscalates` fails deterministically (expects synthesis step status `WAITING_USER_APPROVAL`, observes `RUNNING`) — a pre-existing, separately-tracked regression in this branch's in-flight BUG-234/235 step-status work, not caused by this task's own changes, but it does mean the CP-36 preservation guarantee isn't fully airtight yet. Left `Status: draft` (not marked `done`) pending that fix.
