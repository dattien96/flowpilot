...
- Hoàn tất binding nguồn per-flow ở runner/pack layer: flow YAML đã khai báo được source set, loader validate fail-fast, và executor thực sự truyền enabled set xuống builder.
...
- result: `Done` — `contexts.sources` is part of the flow pack contract, `rag-harness` declares the default trio explicitly, and runtime dispatch passes the resolved source set into `BuildFlowContextPackageWithSources`.
- follow-ups: Task-195 đăng ký nguồn `mcp.driver` để flow khai báo được; Task-196 covers step-level UI/persistence on top of this flow-level substrate.
- upstream docs updated: `CP-44` DOD/progress updated to reflect Task-194 shipped.
