...
- T-1: `flowStepsPanelLinesMax` renders a 3-line block per step when provider/model is known — line 1 `[glyph] name` (+ select fill only here), line 2 `    provider/model` (provider token in brand hue: claude orange / codex green / grok light-blue / opencode purple; `/model` dim; unknown provider → dim), line 3 `    agent: name` in agent hue (full width, never "…" cut). Without a provider line the agent stays inline on line 1 exactly as before; absent entirely → single line, byte-identical. Per-step wins, fallback to run posture `flowStepsProvider/Model` carried in `StepsRuntimeMsg` (TUI-internal threading in `cmdRefreshStepsRuntime` — needed because built-in nodes inherit posture server-side). Refined twice per operator screenshots (2026-09-05): inline chip → second line (width-clamp cut) → 3-line block with select-only-line-1 + provider hues.
...
- T-3: 10 tests in `task322_steps_provider_round_test.go` (provider color map, row layout both/halves/posture-wins/absent-identical, agent line-3 vs inline, selected-only-line-1, header chip/Cap-precedence/hidden, msg-carries-posture) — all PASS; full `go test ./internal/tui/...` PASS, zero old-test edits.
