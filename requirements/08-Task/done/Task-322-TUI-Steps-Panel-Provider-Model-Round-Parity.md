...
- T-1: `flowStepsPanelLinesMax` renders provider/model as an indented dim SECOND line under the row (`    provider/model`; per-step wins, fallback to run posture `flowStepsProvider/Model` carried in `StepsRuntimeMsg`; absent → single line, byte-identical). Never inline on row 1 — the width clamp would cut it to "…" (operator feedback 2026-09-05). Run-posture threading is TUI-internal (`cmdRefreshStepsRuntime` keeps `snap.Provider/Model`) — needed because built-in nodes inherit posture server-side (per-step often empty). The focused row keeps the selected fill on the sub-line so the selection block stays continuous.
