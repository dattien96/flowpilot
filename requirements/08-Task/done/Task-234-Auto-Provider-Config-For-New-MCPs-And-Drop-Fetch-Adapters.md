...
- follow-ups: (1) Atlassian OAuth browser handshake to auto-mint the Jira bearer token (G1 / Out of Scope, unchanged); (2) ~~Grok remote-HTTP Jira (`Q-2`)~~ **done G2**; (3) scope-threading `workflowRunId`/`workflowStepRunId`/`processKey` into the live per-turn merge for Telegram's approval queue (G3, shared with Drive proxy — see Task-233 Addendum §10); (4) optional: `PreflightJiraMcp` for `codex`/`gemini` (Ensure writers exist; preflight still Claude+Grok only); (5) credential re-resolve caching on live MCP helpers if merge frequency grows.
