...
- Phase: `task`
- Status: `done` (owner ran the manual click-through E2E against a real backend on 2026-07-07 — see §8.)
- Owner: `FlowPilot`
- Reviewers: `TBD`
- Created: `2026-07-06`
- Last Updated: `2026-07-07` (owner's live E2E run confirms the from-scratch custom flow authors, saves, and executes correctly end-to-end)
- Parent Documents: [CP-42: Flow Pack And Generic Node Behavior Refactor](../../07-Coding-Plan/done/CP-42-Flow-Pack-And-Generic-Node-Behavior-Refactor.md) (P-9), [Task-179: Settings Flow Pack Authoring UI](../done/Task-179-Settings-Flow-Pack-Authoring-UI.md)
...
- [x] **Owner's own manual click-through — done 2026-07-07**: the owner authored a from-scratch custom flow ("my-flow": nodes `my-coder` → `my-reviewer` → `synthesis`, 5 edges including a `done`/`continue`/`escalate` mix and a back-edge) entirely in the live Settings UI against a real backend, saved it (`validateFlowGraph` passed), then ran it in Flow Mode. Observed: `my-coder` auto-spawned as the entry node with no AI decision involved; on completion the `my-coder → my-reviewer` edge auto-advanced and spawned the reviewer; the cohort/join barrier completed and reinvoked the `synthesis` hub node, which the flow-control tool then marked `done`. The Flow Step Timeline, right-sidebar Agents panel, and main agent area all transitioned `PENDING → RUNNING → DONE` node-by-node, confirmed against the live run's own diagnostic log (`.flowpilot/logs/features/agent-flow-engine/run-4881.ndjson`) and the actual composed prompt sent to the provider (`.flowpilot/runs/<projectId>/run-4886/prompt-turn-4891.txt`), which contains the agent's full system prompt plus a `[FlowPilot sub-agent — agent: coder-agent | role: coder-agent | definition: ...]` identity line — not just the bare task text.
...
- result: `done` — all 5 implementation slices shipped 2026-07-06 on `task/flow-agents`, and the owner's own manual E2E click-through against a real backend on 2026-07-07 confirmed the from-scratch custom flow authors, saves, and runs correctly end-to-end (see the now-checked §6 item).
  ...
  - Owner's live E2E pass (2026-07-07) surfaced and drove several follow-on fixes on the same branch, tracked as their own changes rather than re-opening this task's scope: removed duplicate/legacy step-definition fields superseded by the node/edge model (Subagent, Agent type, editable Depends-on, Join mode, Cohort — Depends-on is now edge-derived), replaced the free-text-with-datalist Agent-ref and edge-outcome fields with real dropdowns, and fixed agent-ref resolution in `spawnChildRun` to accept a definition path (not just a bare name) with a base-name fallback for extensionless/moved refs — confirmed via `run-4881`'s diagnostic log and the actual composed prompt on disk.
  - follow-ups: a "fetch failed" error reported once during a workflow save was not reproduced or root-caused in this cycle (the assistant's dev sandbox has no reachable backend to reproduce against); revisit if the owner sees it recur. A semantically-mislabeled `continue`/back-edge `From` node in one of the owner's test flows is harmless at runtime (`resolveContinueBackEdgeTarget` never reads `.From`) but was flagged to the owner as worth tidying up.
