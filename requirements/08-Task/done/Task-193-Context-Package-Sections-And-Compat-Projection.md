...
- Mở rộng model package + renderer sao cho thêm một loại context mới không cần sửa struct/contract.
...
- result: `done` — 2026-07-08: `Sections []FlowContextSection` thêm vào `FlowContextPackage`; `projectContextSections` + `renderGenericSections` implement; `context_package_sections_test.go` (5 test item) pass; `PackageID` xác nhận bất biến; `BuildAuditDraft` (Task-171) đọc đúng qua package có Sections.
