...
- Make a freshly bound project engine-ready automatically from the admin-web flows that actually create or update directory bindings.
...
- Deliver CP-34 P-3 using the real binding save flows in admin-web instead of a nonexistent runner-side binding lookup.

### Key Decisions

- `T-1` Auto-init is launched from project create, project update, and directory-binding save flows, because those are the places where binding data exists.
- `T-2` Auto-init is async and best-effort; admin-web logs failures and does not block the primary save action.
...
- `T-4` The FlowPilot repo guard still applies, and the last init result is persisted for the Engine tab.

### Constraints

- Reuse Task-104's init path from admin-web instead of duplicating engine logic in the binding flows.
...
Engine setup needed to happen when a directory binding is created or updated, but the runner has no direct access to Supabase binding records, so orchestration had to be attached to the admin-web save flows.

## 4. Exact Change

- `T-1` Add a shared admin-web helper that calls `localRunnerGateway.initEngine(projectId, { workingDirectory, trigger: "bind" })` and swallows failures after logging.
- `T-2` Invoke that helper after binding-bearing save flows: project create, project update, and directory-binding create/update.
...
- files: `apps/admin-web/src/features/projects/project-engine-auto-init.ts`, `apps/admin-web/src/routes/_authenticated/projects/create.tsx`, `apps/admin-web/src/routes/_authenticated/projects/$projectId.tsx`, `apps/admin-web/src/routes/_authenticated/projects/$projectId/directory-bindings.tsx`, plus the reused Task-104 runner files
- modules: admin-web project save flows, `runner`, `changeledger`, `featurecatalog`, `skillpack`, `tooling`
- routes: none new
- tables: none
...
- [x] Project create flow triggers best-effort bind-time engine init for each normalized directory binding.
- [x] Project update flow triggers best-effort bind-time engine init for each normalized directory binding after bindings are persisted.
- [x] Directory-binding create/update flow triggers the same best-effort bind-time engine init helper.
...
- [x] Bind-trigger init persists last-init details that the Engine tab can display later.
...
- result: implemented through admin-web binding save flows plus the shared runner init endpoint.
- follow-ups: wire CP-31 normalization later if a stable callable entrypoint is introduced.
- upstream docs updated: `CP-34` updated to reflect that bind-time orchestration lives in admin-web and that CP-31 normalization is deferred in the current repo.
