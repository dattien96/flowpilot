...
- **UNLOCKED (2026-08-30, Task-319 / CA-693):** the recorded unlock condition has been met — live ACP probe proved the image round-trip (mimo-v2.5-free saw a base64 image block, turn completed). Opencode image support is now per-MODEL via `ProviderModel.InputImage` (models.dev input.image); the provider-level set in `visionProviders.ts` intentionally stays `codex/claude/grok` — see `supportsVisionFor`.
- follow-ups:
  - **Unlock condition (recorded):** `Vision` may flip `true` for Opencode only when the ACP image attachment round-trip is proven end-to-end — image → `.tmp/images` path fallback (Grok `CA-483` pattern) or native ACP image block → model sees it → turn completes. When proven, update: `opencode_adapter.go:184` (`Vision:true`), `ChatInput.tsx:92` (add `opencode` to `VISION_PROVIDERS`), `opencode_acp.go:88-90` (comment), CP-57 `P-12`/row 28/line 473, Task-303 `T-7`/`DOD-15`, CP-57-Test-Steps `M1`. — **PROVEN by Task-319** via native ACP image blocks (path fallback not needed); the per-model gate replaces the provider-level flip (Grok pattern, `Vision` stays `false` provider-wide by design).
