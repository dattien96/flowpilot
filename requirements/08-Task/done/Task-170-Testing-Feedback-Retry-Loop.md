...
- **CORRECTION 2026-07-06, prior to a manual CP-41 E2E test session: this task's own unit-level "done" verification above does NOT mean this code is reachable in a live Flow Mode run.** Tracing the full RAG Harness pipeline for the E2E prep found `RunValidationCommand`/`NewFlowValidationRetryState`/`AdvanceRetryState`/`ComposeRetryPrompt` have **zero production callers** — the registered `command.validate` behavior handler (`behaviorCommandValidate`, `behavior_registry_builtin.go:153-163`) never calls any of this code; it only inspects a pre-populated `exitCode` that nothing in the live dispatch path sets. See [BUG-243](../../09-BugFix/done/BUG-243-Flow-Mode-Validate-And-Audit-Behaviors-Disconnected-From-Task-170-171.md) for the full trace. This task's own DoD/tests remain accurate for what they claim (the retry-state library is correct and unit-tested); the disconnect is in the behavior-registry wiring (Task-176/CP-42 territory), not in this task's own code.

**RESOLVED 2026-07-09**: BUG-243 is fixed — `behaviorCommandValidate`'s live dispatch path (via the new `runValidateNode` in `flow_validate_audit_dispatch.go`) now calls `RunValidationCommand`/`AdvanceRetryState`/`ComposeRetryPrompt` for real, reachable via `tryAdvanceFlowFromNode`'s new mid-flow inline-dispatch path (`F-0`). This library's functions have production callers as of that fix.
