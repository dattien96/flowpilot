...
- Related Documents: [Task-301: Opencode Controlled Adapter MVP](../../08-Task/done/Task-301-Opencode-Controlled-Adapter-MVP.md), [Task-400: Devin ACP Transport](../../08-Task/done/Task-400-Devin-ACP-Transport-And-Process-Dispatcher.md), [CP-70-note.md](../../07-Coding-Plan/note/CP-70-note.md), [CP-70-Test-Steps.md](../../07-Coding-Plan/done/CP-70-Test-Steps.md)
- Replaces: `None`
...
- Coding Plan: [CP-70: Devin Provider Integration](../../07-Coding-Plan/done/CP-70-Devin-Provider-Integration.md) (Work Breakdown `P-3`, `P-4`, `P-5`, `P-9`, `P-11`)
...
- [x] `DOD-1` `devinAdapter` implements `ProviderRuntimeAdapter` and emits normalized events (`TestDevinAdapterSendTurnStreamsAndCompletes`).
- [x] `DOD-2` Adapter drives `devin acp` process boundary using ACP `initialize → authenticate`, `session/new`, `session/prompt` (`TestDevinGoldenFlow`, `TestEnsureDevinProcessBootOrderIncludesAuthenticate`).
- [x] `DOD-3` Capabilities are truthful: `Streaming/Resume/FileEvents/Interrupt/SkillSelection` true; `ApprovalEvents/Mcp/Vision` false in MVP (verified via `devinAdapter.Capabilities()` consumed by `devin_e2e_test.go` + capability-gated paths).
- [x] `DOD-4` `ProviderRegistryFor` returns Devin as `Available` when flag is on; `DefaultProviderRegistry` has no Devin entry (`TestDevinAgentEnabledFlag`).
- [x] `DOD-5` `ModelProviderKey` routes `devin/*` to `ProviderKeyDevin`; existing providers unchanged (`TestDevinPrefixedProviderModels`, `TestDevinModelIDForACP`).
- [x] `DOD-6` Real session ID captured and persisted via `ProviderSessionStore.UpsertSession`; resume test reuses it (`TestDevinAdapterResumesSlugSession`; live: `session/load{trusted-airmail}` after runner restart).
- [x] `DOD-7` Synthetic `thread-*` ID without existing session map is rejected with typed error (`TestDevinAdapterSyntheticAndForeignSessionsForceNew`).
- [x] `DOD-8` Post-prompt `_meta.sessionId` adoption updates session store (`TestDevinACPResponseSessionID`; live: slug adopted post-prompt).
- [x] `DOD-9` Polymorphic message chunks (string and array) parse without data loss or crash (`TestDevinTextContentShapes`, `TestMapDevinNotificationAgentMessageChunk`).
- [x] `DOD-10` Quota `stopReason` mapped to `EventTurnFailed(Recoverable: false)` without blind timeout (`TestDevinAdapterQuotaFailure`, `TestDevinStopReasonMapping`).
- [x] `DOD-11` Deny permission with zero reply text produces synthetic explanatory message (`TestDevinAdapterDeniedEmptyAnswerNotice`).
- [x] `DOD-12` Base regression: full runner test suite passes 100% (`go test ./internal/runner`).
