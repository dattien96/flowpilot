...
- Phase: `task`
- Status: `done` (2026-07-17: boot enumeration + redispatch fixed and hard-asserted; no-adapter fallback-to-uncertain path verified atomic and correct; provider reconcile/attach engine (T-4) formally waived — no adapter exposes the required capability, see §8)
- Owner: `FlowPilot`
...
- Related Documents: [BUG-288](../../09-BugFix/done/BUG-288-Flow-Mode-Three-Tier-Gate-And-Change-Contract-Reentry-Gaps.md), [Task-248](./Task-248-Durable-Dispatch-Record-And-State-Machine-Core.md), [Task-249](./Task-249-Live-Dispatch-Integration-And-Stop-Fences.md)
- Replaces: `None`
...
- result: **Closed 2026-07-17.** Boot enumeration + redispatch fixed and hard-asserted; the no-adapter fallback-to-`uncertain` path is verified atomic and correct at the store layer. Provider reconciliation (T-4) and real provider-cancel-on-required are formally **waived** — not a code gap, a capability gap in every adapter available today (see waiver below).
...
- **[FIXED 2026-07-17 — hardened] No-adapter fallback assertion was soft, tightened to hard.** `TestScanAllRecoverable_EnumeratesEveryNonTerminalState_NotJustUncertain` previously only `t.Logf`'d the post-scan state of a `send_started` record with no reconcile proof — it would have passed even if the record had been left dangling at `send_started` forever. Now hard-asserts `got.State == DispatchUncertain` and `Revision` bumped. Traced the actual store transaction (`memoryDispatchStore.CommitRecoveryUnknownOrRequireCancel`, `dispatch_store_memory.go:595-626`) to confirm: it is a single locked CAS — `canTransition(DispatchUncertain)` guard, `State=DispatchUncertain`, `revokeAttachLocked` (kills any live attach token), `Revision++`, audit line `"recovery_uncertain"` — never a bare unguarded write, and the record is provably never left at `send_started`/`provider_accepted` after a scan claims it.
- **Waived — Provider reconciliation (T-4) / real provider-cancel:** no `ReconcilableAdapter`/`Reconcile` hook exists, and `reconcileOne`'s `RecoveryCancelRequired` branch only logs instead of issuing a real provider cancel. Both are gated on the same missing adapter capability (query-by-operation-id / cancel-by-operation-id), which Task-257's live evidence confirms **no adapter (Codex, Grok, Claude) exposes today** — this is a provider/upstream limitation, not a FlowPilot code gap. The store-level fallback (`effectiveStopLocked` checks own-Stop **and** parent-fence atomically inside the same CAS that writes `uncertain`) already guarantees the record is never silently duplicated, never silently lost, and always surfaces to the operator (SS-17 AC-4) even without real provider confirmation — the correctness contract holds without T-4. Re-open T-4 only if/when an adapter ships query-by-id or cancel-by-id.
- Tests: `dispatch_recovery_test.go` (4 tests, one hardened this pass) plus the 2 pre-existing `TestRecoveryScanner_*` tests in `cp51_tasks_test.go`. Short of the 32 named §4.2 tests (attach-token, operator retry-as-new interplay, TP/SA proof pass-through) — those all presuppose a `ReconcilableAdapter`, so they stay unwritten alongside T-4 until an adapter exists to test against.
- follow-ups: re-open T-4 (provider reconcile + attach engine) and its §4.2 test list only when a provider adapter ships query-by-id/cancel-by-id capability; no action needed until then.
- upstream docs updated: task status closed; test hardened; CP-51 ledger + verification log updated in the same pass.
