...
- Live-found 2026-09-06 (BUG-360, run-584646): approve-after-restart is not merely untested — it is BROKEN. Park/topology durability held (card restored, advance dispatched past the plan gate with no re-park), but `preflight_contract_freeze` strict-parse fails: the planner draft lived only in the transient planner child turn, gone post-restart, and the prose fallback (`invalid character 'c'…`) escalates in a dead-end "no progress" loop no button escapes except Stop. Fix direction: freeze derives the draft from the on-disk plan doc (durable `plan_md`) when the child result is absent. Approve-path live verify moves to a fresh no-restart run.
...
- Follow-up 2026-09-06 (CA-753, live-found on run-594636): (1) feedback path PROVEN live — writer `turn_started` prompt carries the user's verbatim note + composed template (timeline evidence), same-child re-entry, loop running; suspicion of blind-retry disproven. (2) Operators couldn't discover `/continue <note>` — blocked bar gains a `[Revise]` chip (bar + ring + mouse) that prefills the composer with `/continue ` and sends nothing.
