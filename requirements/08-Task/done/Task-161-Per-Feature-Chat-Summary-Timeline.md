...
- [x] **DOD-10 (conversation-sticky resolution):** feature resolution for both injection and recording uses `resolveTurnsFeature`, which scans the conversation's user prompts newest→oldest and uses the first that clears the confidence threshold. When the latest prompt is an **explicit continuation** (`continue`/`try again`/`do it`/`go on`…, common after an AI error) it inherits the established feature — so it still gets history injected and contributes to the summary — while a greeting (`hi`), a bare acknowledgement (`ok`/`yes`/`no`), or an explicit topic change does **not** inherit (it resolves on its own feature or injects nothing). Continuations are matched by `isContinuationPrompt` against a **curated phrase set only** — no blind word-count fallback, so social/unrelated chatter never drags in stale context. Gate reprompts and cross-provider handoff envelopes are likewise treated as system prompts whose text never drives resolution. The same helper is used by the handoff summary lookup, so all paths agree. (CP-37 `V-161-10`/`V-161-11`; `isContinuationPrompt` battery test.)
