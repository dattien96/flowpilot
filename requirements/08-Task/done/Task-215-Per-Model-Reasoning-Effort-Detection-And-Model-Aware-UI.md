...
- verification:
  ...
  - Browser: initial attempt via `preview_start` (desktop-flowpilot, port 5173) was blocked by the bootstrap gate (no local runner/Supabase backend available in that sandbox). User subsequently tested the Reasoning dropdown and usage bar live in-browser directly and confirmed both work as expected — the model-aware Reasoning options and the context-window fallback render correctly.
