...
- Round-2 hardening (CA-840/CA-845, sau 2 vòng agent review độc lập): (1) fence-aware — DOD/checkbox trong fenced code block bị bỏ qua (chống game cổng); (2) strings.Split thay bufio.Scanner (hết giới hạn 64KB/line); (3) guard `..`/symlink trong MissingDodDocs; (4) **blocking round-2: heading DOD dạng CÓ SỐ** (`## 9. Definition of Done` — dạng 100% doc Task/BUG thật của repo dùng) giờ được nhận diện, kèm real-repo acceptance test khóa vĩnh viễn (16/16 doc contract-form nhận diện; 9 doc legacy kiểu prose/backtick cố ý ở trạng thái non-conforming để gate reprompt theo thiết kế CP-47 R-1); (5) section DOD chỉ đóng ở heading h1/h2 — `###` sub-headings bên trong DOD không còn cắt ngắn section.
