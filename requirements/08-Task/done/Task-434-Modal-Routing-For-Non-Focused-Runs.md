...
- [x] Provider parity: routing check `runId` thuần, không nhánh provider (R2)
...
- result: DONE — isFocusedRun(state, runId) single decision point (undefined->false), attentionQueue.ingestAttentionItem synthetic items deduped by runId+kind with auto-prune on resolve, consumeStream quota path routed (focused->pendingAccountSwitch unchanged; non-focused->quota inbox item carrying candidate id -> same confirm path), applyEvent gateBlock focus-guarded (non-focused still marks _gateBlockedRunIds). 8 tests green.
- follow-ups: global events without runId intentionally keep global surfaces (open question resolved).
- upstream docs updated: CP-84, CA-935.
