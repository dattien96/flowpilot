...
- Bọc 3 nguồn hiện có thành source, đăng ký vào `NewDefaultContextSourceRegistry()`, và refactor builder — không đổi output.
...
- result: `done` — 2026-07-08/09: `context_sources_builtin.go` (3 source) + refactor `flow_context_package.go` (`buildFlowContextPackage` chạy qua `Collect`) + golden test `context_source_migration_golden_test.go` (7 sub-test) byte-identical. Phát hiện phụ: golden test lộ ra bug non-determinism có sẵn trong `changeledger.GetFeatureHistory` (map iteration random khi `CommittedAt` bằng nhau) → fix + track riêng ở [BUG-266](../../09-BugFix/done/BUG-266-Changeledger-Feature-History-Order-Nondeterministic-On-Equal-CommittedAt.md).
- follow-ups: Task-193 phơi `Sections` ra ngoài + renderer generic.
- upstream docs updated: [BUG-266](../../09-BugFix/done/BUG-266-Changeledger-Feature-History-Order-Nondeterministic-On-Equal-CommittedAt.md) created.
