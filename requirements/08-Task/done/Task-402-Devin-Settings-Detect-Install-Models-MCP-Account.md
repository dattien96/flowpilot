...
- Related Documents: [Task-302: Opencode Settings](../../08-Task/done/Task-302-Opencode-Settings-Detect-Install-Models-MCP-Account.md), [Task-401: Devin Adapter MVP](../../08-Task/done/Task-401-Devin-Controlled-Adapter-MVP.md), [CP-70-note.md](../../07-Coding-Plan/note/CP-70-note.md), [CP-70-Test-Steps.md](../../07-Coding-Plan/done/CP-70-Test-Steps.md)
- Replaces: `None`
...
- Coding Plan: [CP-70: Devin Provider Integration](../../07-Coding-Plan/done/CP-70-Devin-Provider-Integration.md) (Work Breakdown `P-7`)
- Tech Design: `SD-06`
...
- [x] `DOD-1` `CheckTool("devin")` correctly detects binary presence and reports version (verified: `TestCheckToolDevinMissing`, `TestCheckToolDevinViaBinOverride`; live `/providers` INSTALLED `3000.10.31`).
- [x] `DOD-2` `runProviderInstallCommand("devin")` executes correct OS-specific command (verified: `check.go` devin install case; live card reported AUTH_REQUIRED→PKCE).
- [x] `DOD-3` `detectDevinModels()` returns catalog with `devin/*` prefixes (verified: `TestDevinModelsCacheRoundTripAndTTL`, `TestRecordDevinModelCatalog`; live catalog `devin/*`).
- [x] `DOD-4` `hasValidProviderAuthFile("devin")` returns true when `~/.local/share/devin/credentials.toml` exists và non-empty (verified: `TestDiscoverDevinAccountHomes_DefaultAndManaged`, `devin_integration_test.go` credentials.toml checks).
- [x] `DOD-5` `WriteDevinMCPConfig` merges Google Drive and Jira MCP servers into Devin config without corrupting existing keys (verified: `TestDevinMcpConfigWriteAndCheck`, `TestDevinJiraMcpConfigWriteAndCheck`, `TestEnsureDevinLocalMcpServersWritesLocalScope`).
- [x] `DOD-6` `CheckVersionSettings.tsx` renders Devin as 5th row (code shipped + `/compat` live returns `installedDevinVersion`; visual check deferred with UI phase) with version badge without visual regression on existing rows.
- [x] `DOD-7` `ProviderAccountsPanel.tsx` includes Devin (PROVIDERS row `devin` shipped; visual check deferred with UI phase) in `PROVIDERS` list and allows adding/switching accounts.
- [x] `DOD-8` `AiProvidersSettings.tsx` displays Devin card (devin branch + model select shipped; visual check deferred with UI phase) with accurate status, 1-click install, and detect models button.
- [x] `DOD-9` Base regression: existing settings for Claude, Codex, Gemini, Grok, Opencode function identically.
- [x] `DOD-10` `managedProviderHomePrefix("devin")` → `.devinHome` với layout `.config/devin` + `.local/share/devin` được tạo (verified: `TestIsValidDevinAccountPath`, `TestDiscoverDevinAccountHomes_DefaultAndManaged`).
