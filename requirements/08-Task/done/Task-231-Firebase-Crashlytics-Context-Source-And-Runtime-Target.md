...
## 10. Addendum (Task-234 revisit): adapter unwired again — this time for a verified reason

The "wire it" decision in §9 was itself revisited once the *actual* live-turn path was traced end-to-end: `firebase.crashlytics` (like `mcp.driver`, `jira.issue`, `jira.sprint`) is filtered out of `startInlineEntryChain`'s collect (`flow_executor.go`), so `firebaseToolsMcpAdapter.Fetch` was never invoked during a live AI turn regardless of whether it was wired — the AI instead reads Crashlytics via the prompt-note-injected MCP tool calls (`appendFirebaseCrashTargetPrompt`), which depend on the *provider-config* path, not this adapter. Task-234 unwired `SetFirebaseCrashlyticsAdapter` from `AttachRunner` accordingly and, in exchange, fixed the actual dependency: `flowpilotClaudeExtraMCPServers` now merges `firebase` into the live per-turn `--mcp-config` (previously `EnsureClaudeFirebaseMcpConfig` only wrote `.claude.json`, which `--strict-mcp-config` ignores at turn time). The adapter file/code is kept, unused, per explicit instruction — not a contradiction of §9's engineering, just a correction of *where* it plugs in.
