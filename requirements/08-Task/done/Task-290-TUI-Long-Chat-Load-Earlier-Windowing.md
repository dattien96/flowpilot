...
- Full transcript stays in memory; seq-0 replay contract unchanged.
...
- `Q-1` OPEN: if seq-0 SSE replay remains slow on very large runs, add tail-fetch in a follow-up task.
...
- `T-6` `chat_windowing_test.go` covers A8.16–A8.19.

## 5. Touched Areas

- `apps/local-runner/internal/tui/app/chat_windowing.go`
- `apps/local-runner/internal/tui/app/chat_windowing_test.go`
- `apps/local-runner/internal/tui/app/app.go` (`buildChatRows`, `addMessage`, `/new`, `ChatOpenedMsg`)
...
- result: Implemented render-side prompt-group windowing with Desktop copy and click target.
- follow-ups: Measure seq-0 replay on very large runs (`Q-1`).
