...
### Vòng 14 — Re-review code Grok (Vòng 13) + NEW-bug sweep (2026-07-16) — **Fixed**

Review lại diff Vòng 13 (Grok) + NEW-bug sweep (Claude). **Kết luận: không P1/P2 mới.** 4 residual **P3 → Fixed (2026-07-16)**.

| ID | Sev | Finding (review) | Fix |
| ---- | --- | ---------------- | --- |
| `R14-01` Fixed | P3 | `stalledSkipCause=true` vô điều kiện khi skip (gate in-flight) → flag leak sang Stop sau | Chỉ set khi `cancel != nil` |
| `R14-02` Fixed | P3 | Comment R13-02 finishTurn lộn xộn | Rút gọn 2 dòng |
| `R14-03` Fixed | P3 doc | Mất chi tiết findings Vòng 13 | Link ledger: `CA-329` + commit `4aed023` + bảng fix summary § Vòng 13 (full root-cause prose lúc OPEN nằm trong transcript re-review / git history pre-summary; CA-329 holds scope+ledger) |
| `R14-04` Fixed | P3 | Secret chỉ init NDJSON store | `initRunMarkerSecretForStore` trong `newInteractiveService` (NDJSON dataDir hoặc `UserConfigDir/flowpilot/marker`) |

**R14-03 detail pointer:** full per-finding root cause + solution for R13-01…R13-27 is in `change-audit/CA-329-bug288-vong13-residual-close.md` and the § Vòng 13 fix-summary table above; the original long OPEN inventory was condensed after fix to keep the BUG doc navigable.
...
### Vòng 20+ (post-Round-20 Codex re-review) — Structural close via SD-24 / CP-51

Codex re-review sau Vòng 20 (HEAD `6ea5417`) FAIL với **2 Critical + 4 Important + 2 verification gaps**, tất cả cùng một root cause: turn dispatch điều phối bằng RAM flag (`turnInFlight`) + marker `prep:`/bare 2-state thay vì một durable record chứng minh provider đã nhận gì. Đây đúng là durable lifecycle mà **Vòng 9 §11.9.4 Batch A** đã kê (`provider_done → gate_running → gate_blocked|gate_passed → finalized`) nhưng Vòng 10–20 chỉ vá điểm.
...
| Codex verdict (post-Vòng 20) | Ledger ID (CP-51 §10.1) | Task |
| --- | --- | --- |
| Critical 1 — durable turn lost/duplicated | `C1` | 248/249/250 |
| Critical 2 — Stop race after launch-ack | `C2` | 249 |
| Important 3 — gate checkpoint wedge/loss | `I3` | 251 |
| Important 4 — same-service FCP marker replay | `I4` | 252 |
| Important 5 — Supabase fail-open corrupt runtime | `I5` | 253 |
| Important 6 — active idempotency key pruned | `I6` | 254 |
| Gap 7 — crash/stop/outage/replay/corruption tests | matrix + `NR` | 255 |
| Gap 8 — `EventTurnStarted` contract contradiction | `G8` | 248 |

Out of scope của CP-51 (không chặn done, non-dispatch): Q-1 (Task-240 hub-tool schema + watchdog E2E), Q-2 (Gemini inbound permission bridge).

**Plan-review round (Codex, 2026-07-16):** Codex review chính bản plan → 7 Critical + 9 Important findings về protocol (receipt giả từ session id, thiếu revision/CAS/lease, Stop chưa linearize, state overlays mâu thuẫn, thiếu immutable envelope, receipt best-effort, settle worker sai hàm, rollback split-brain, hai source of truth, crash harness same-process, thiếu PG contract test, thứ tự task, FF không realistic). **Tất cả đã incorporate:** SS-17 mới (operator contract cho `uncertain`/`repair_required`), SD-24 **v2** (9-state forward-only + CAS/lease + DispatchEnvelope + capability matrix per-provider đã verify code + three-outcome contract + linearization + migration matrix), CP-51 re-sequenced (P-0 approval gate → 248+253 → 255-red → 249 → 250 → 251/252/254 → 255-final) + ledger thêm rows `MU/CC/SW/TA/SP/PG/OP1/OP2`, Task-248…255 code guides viết lại theo v2. Store defect RAM-before-disk verify tại `local_file_session_store.go:273`; `notifyTurnIdle` không chạy settle (chỉ flush resume/reprompt) verify tại `interactive_resume.go:1563-1582`; adapter op shapes verify tại `codex_adapter.go:167/226`, `grok_adapter.go:285/223`, `claude_adapter.go:173`.

**Plan-review round #5 (Codex 5.6 Sol, 2026-07-16):** FAIL — 5 Critical + 5 Important. Fixes: (1) **quarantine raw persist** — `OpenRepair(runID, reason, rawBlob, rawHash)` lưu chính blob atomically với RepairRecord (ledger `QB`), oversized ⇒ sidecar-then-commit; (2) **two-phase repair resolution** — `BeginRepairResolution` (CAS-claim + trả raw) → Go loader validate ngoài store → `CommitRepairResolution(resolved_retry_load|failed_still_open|resolved_abandon)`; Postgres RPC không phải chạy Go loader nữa; (3) **V2 authority hết mâu thuẫn** — bảng `run_protocol_activations`/activation line **non-prunable** (sống qua compaction) + API `GetRunProtocolVersion`; mọi consumer (missing-log, missing-blob Task-253, migration matrix) gọi API, không đọc mirror; (4) **release manifest** — anchors verify đúng (empty idempotency key `:2071/:2076`, prompt cleared `:2114`, random TurnID `:5863`): dependents release = effect records per-dependent `pending→created|suppressed` + downstream qua durable-intent deterministic keys (ledger `RM`); (5) **TOCTOU** — `session_upsert_guarded` RPC/trigger filter clear-set **trong** transaction; local serialize session upserts trên cùng writer goroutine; test concurrent interleaving (ledger `TO`); (6) **cohort durable** — consequence = payload-bearing effect records unique-keyed, RAM buffer thành projection (anchor `agent_orchestrator.go:92` RAM/label-dedupe) (ledger `CD`); (7) **Task-257 mới** — owner riêng cho capability spike (chạy *trong* P-0, throwaway harness, evidence format `evidence/SD-24/<provider>.md`), phá vòng circular P-0↔Task-249; (8) Task-251 hết contract cũ (header/T-1/T-2/driveSettle comment/tables theo doctrine convergent); (9) Touched Areas/schema đủ 4 bảng + RPCs ở Task-248/251/253; (10) **RetryAsNew supersede guard** — CAS intent-gen + envelope-hash, `ErrSuperseded` + test operator-vs-new-prompt (ledger `RS`). Ledger = **40 rows**.

**Plan-review round #4 (Codex 5.6 Sol, 2026-07-16):** FAIL — 5 Critical + 6 Important, tâm điểm là chỗ khó nhất: (1) effect ledger check-trước-effect vẫn **trùng effect** khi crash giữa effect và marker → thay bằng **idempotency doctrine**: mọi consequence phải replay-convergent (keyed-upsert / monotonic / create-if-absent hoặc unique-key trong chính transaction của consequence); marker `effectDone` chỉ là skip/observability, correctness KHÔNG phụ thuộc marker (SD-24 §5.3, Task-251 per-effect contracts); (2) thêm mutation API `RecordEffectDone` + bảng `dispatch_effects` UNIQUE(run,turn,effect_kind); (3) `OpenRepair`/`GetOpenRepair` + bảng `repair_records` — RepairRecord có đường tạo durable một-commit (record+quarantine+block+audit), `markRepair` nhận store; (4) **P-0 khai đúng là OPEN** — docs draft + capability spike chưa chạy, CE unchecked, không được tick; (5) **atomic V2 activation**: activation đi cùng commit `CreatePrepared` đầu tiên, authority derive từ dispatch store, marker session chỉ là mirror repairable (ledger `V2A`) — hết cửa sổ false-repair; (6) Task-251 Summary/T-2 hết mâu thuẫn effects-vs-CAS; (7) Stop-settle policy thống nhất: worker KHÔNG exit on Stop, bookkeeping hoàn tất, chỉ suppress dependents-release (V-4 viết lại); (8) Task-249 sketches đủ `IntentOwnerRunID`, hết `_, _ =`; (9) Task-250 signatures compile-consistent (runID xuyên suốt) + error handling fail-closed; (10) Task-255 matrix loop chạy settle sub-barriers thật (3 cửa sổ × mỗi phase: after-effect-pre-marker / after-marker-pre-CAS / after-CAS-pre-next); (11) Task-253 rewrite theo RepairRecord (hết "Independent", thêm 4 test race/atomicity/retry). Ledger: EL reword theo doctrine + row `V2A` mới = **35 rows**.

**Plan-review round #3 (Codex 5.6 Sol, 2026-07-16):** FAIL P-0 — 8 blocking + 6 important, chủ yếu là lỗi protocol thật: (1) settle CAS-trước-effect làm **mất** effect khi crash → đảo thành **effects-first, CAS-after** + **durable effect ledger** `effectDone{runID,turnID,effectKind}` (SD-24 §5.3); (2) INV-7 scoped lại vào durable consequences (event replace hiện chỉ check type `interactive_service.go:3239`, `_ = persistEvent` `:3245`, finalizer RAM map `finalizer.go:49`, waiter RAM `agent_orchestrator.go:195` — tất cả thành T-item retrofit trong Task-251); (3) Task-248 gỡ sạch dispatch khỏi `ProviderSessionState`/session blob (chỉ còn marker+repair+provenance scalars, ledger `SB`); (4) store API thêm read side (`Get/ListRecoverable/ListAttention/ListAudit/ListEffects/GetResolutionResult/IsIntentCleared`); (5) Task-253 guard missing-blob-V2 đặt trước early-return (reachable); (6) intent-clear filter cả **write lẫn read** trên cả hai backend + RAM callback (ledger `IR`); (7) Stop/recovery không bao giờ blind-terminalize turn đã gửi — CAS conflict phải reload+branch theo state (ledger `SC`); (8) settle dispositions `settle_superseded_reprompt`/`settle_suppressed_by_stop` (ledger `SD`); (9) `RepairRecord` + `ResolveRepair` CAS (ledger `RR`); (10) Task-252 hết mâu thuẫn (Ctx helper end-to-end, provenance persist ở cả 2 store); (11) Task-255 Exact Change theo v3 (B0..B8+B8a-e, StopOutcome, inventory đủ settle/model tests, 256 trong out-of-scope); (12) residuals quét bằng sweep rộng; (13) local log operational contract (process lock, compaction temp→fsync→rename→dirsync, missing-log-V2 ⇒ repair); (14) capability evidence = **P-0 precondition** (spike trước Task-249, ledger `CE`). Ledger giờ 34 rows — mỗi finding có binary row.

**Plan-review round #2 (Codex 5.6 Sol, 2026-07-16):** FAIL P-0 — 7 Critical + 7 Important về chính bản plan (parent↔child intent atomicity; settle phải là lifecycle chứ không phải boolean; Stop/send contract mâu thuẫn vật lý; Supabase storage shape chưa chốt; Task-250/251 guide vẫn v1; operator ops thiếu transaction; SS-17 không có owner; parity local↔Supabase; missing-blob-V2; Task-248 contradictions; provenance chưa durable; harness không khả thi cross-process; DoD overclaim). **Đã incorporate toàn bộ:** SD-24 **v3** (8 states + `IntentOwnerRunID` cross-record atomic commits + `SettlePhase` 6-phase lifecycle INV-7 + `StopOutcome` linearizable semantics + dedicated dispatch store: local `dispatch.ndjson` commit-log / Supabase `dispatch_records` table + transactional RPC + operator ops `ResolveUncertain`/`RetryAsNew` atomic-idempotent + §13 scope boundary); **Task-256 mới** (operator API/UI/audit owner); CP-51 ledger thêm `OP3/PAR/MB/ST`, §10.0 scoped honestly, coverage-boundary table §10.6; Task-248/250/251/252/253/255 rewrite theo v3 (không còn `intent_pending`/overlay/shadow/direct-clear; harness = subprocess kill cross-platform + parent-owned durable fake provider). Evidence mới verify: `ndjsonSessionRecord` thiếu ChangeType/SourceDocID/TurnCount (`local_file_session_store.go:65-109`); settle = 7 side effects (`interactive_service.go:3226-3266`); parent-owned restart intent (`deliverPendingRestart` `:2233`).
...
- **Closed 2026-07-24:** with that exception recorded explicitly, this doc and [CP-51](../../07-Coding-Plan/done/CP-51-Durable-Turn-Dispatch-State-Machine-And-Recovery-Reconciliation.md) both moved from `inprogress/` to `done/` on operator confirmation. The companion [CP-51-PhaseAB-Timeline-And-Verification-Log](../../07-Coding-Plan/inprogress/CP-51-PhaseAB-Timeline-And-Verification-Log.md) stays `inprogress` — it is a living checklist (its own remaining rows, e.g. A7/X3/X4/B1-B12, are optional or owned by CP-43, and were never this doc's own termination gate) rather than a phase doc that reaches its own terminal state the same way.
