...
- Phase: `bugfix`
- Status: `done` — fixed and verified live 2026-07-09.
- Owner: `FlowPilot`
- Reviewers: `TBD`
- Created: `2026-07-08`
...
- ~~Capture bug để fix sau~~ — done: desktop store now surfaces a `historyOpenError` modal state on the same `account_not_signed_in` / `account_unavailable` catch paths that were already stamping `unavailableReason`.
...
- `F-1` **Done.** Added `historyOpenError?: { code: "account_not_signed_in" | "account_unavailable"; message: string; providerKey?: ProviderKey }` to `AppState` (`apps/desktop-flowpilot/src/state/store.ts:216-223`) and a `HistoryOpenErrorModal` component (`apps/desktop-flowpilot/src/components/ChatWorkspace.tsx`, right after `GateBlockModal`) rendered unconditionally inside `ChatWorkspace`'s JSX alongside the other state-driven modals (`ProviderSwitchModal`, `AccountSwitchModal`, `GateBlockModal`) — it fires immediately from the same `catch` blocks that already stamp `unavailableReason`, reusing the existing `account-switch-overlay` / `account-switch-modal` / `gate-block-*` CSS classes (no new styles needed).
- `F-2` **Done.** The existing `unavailableReason` row/session stamping in both `openHistoryRun` and `restoreRemoteChatSession` is untouched — `historyOpenError` is set as an *additional* field on the same `set()` call, not a replacement.
- `F-3` **Done (no CTA).** Copy distinguishes the two cases by `err.code`:
  - `account_unavailable` → "`<Provider>` isn't set up on this machine. This chat was created with a `<Provider>` account that isn't available here."
  - `account_not_signed_in` → "`<Provider>` is available on this machine, but the active account isn't signed in. Sign in to the account this chat was created with, then try again."
  - `Q-1`'s optional CTA ("Open Provider Accounts") was skipped: `ProviderAccountsPanel` is an always-mounted right-sidebar panel, not a toggleable modal/route, so there was no existing single action to link to without adding new navigation plumbing beyond this bug's scope.
- `F-4` **Done.** Added `dismissHistoryOpenError()` action (mirrors `dismissGateBlock`) and 5 new tests in `store.test.ts` covering both call sites plus the negative case (`Q-2` resolved: only `account_not_signed_in` / `account_unavailable` trigger the modal — `session_unavailable`, `sync_integrity_failed`, etc. keep their existing `unavailableReason`-only UX, per `V-4`).

## 8. Validation

- `V-1` **Done.** Verified live in the browser preview (mock runner client, `ChatWorkspace` mounted standalone): setting `historyOpenError` via the store renders the modal immediately with the correct copy for both `account_not_signed_in` and `account_unavailable`, and the "Got it" button (`dismissHistoryOpenError`) clears it.
- `V-2` **Done.** Covered by existing tests `openHistoryRun treats active-account-not-signed-in as unavailable instead of replacing the current run` (unchanged) plus the new assertions appended to it — `runId` stays `current-run` and `unavailableReason` is still recorded.
- `V-3` **Done.** New test `restoreRemoteChatSession surfaces a historyOpenError modal state on account_not_signed_in (BUG-267)` asserts the same modal state fires for the remote-restore path.
- `V-4` **Done.** New test `openHistoryRun does not set historyOpenError for non-provider-account failures` and an added assertion on the existing `restoreRemoteChatSession marks remote entries unavailable on typed restore errors` test confirm non-provider-account error codes (`session_unavailable`, `sync_integrity_failed`, etc.) never set `historyOpenError`.

## 9. Regression Guard

- tests: `apps/desktop-flowpilot/src/state/store.test.ts` — 5 new/extended cases: `openHistoryRun` × (account_not_signed_in assertion added, account_unavailable, non-provider-account negative), `restoreRemoteChatSession` × (account_not_signed_in, non-provider-account negative on the existing sync_integrity_failed test), and `dismissHistoryOpenError clears the modal state`. `npx tsc --noEmit` clean across `apps/desktop-flowpilot/src`.
- alerts: none.
- audit checks: this note is the closing record.

## 10. Follow-Up Document Updates

- upstream docs: none required — this is a desktop-only UX surface change; runner error code/message contract (`SD-15`) is unchanged.
...
- deferred: `Q-1`'s CTA (jump straight to Provider Accounts / sign-in) is still open — worth a follow-up task once/if `ProviderAccountsPanel` gains a dedicated open/focus entry point.
