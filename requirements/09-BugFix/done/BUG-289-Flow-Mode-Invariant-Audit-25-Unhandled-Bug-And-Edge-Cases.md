...
- **Done (2026-07-17):** all 13 phases in §7.1 landed (`F-0`..`F-12`); each finding in §6 marked **Fix done**; package build + focused `TestBug289_*` green.
- Optional follow-up (not blocking close): remaining §8 acceptance cases on Unix CI (`go test ./internal/runner/...` — Windows lacks `sh` for a few pre-existing session e2e tests).
...
- `A3` `SettleOwed` records are created on every flow turn (`commitTerminal` → `SettlePending`) but `SettleDriver`/`CASAdvanceSettle` have **no production caller** (`dispatch_settle.go:21`), so `HasNonTerminal` stays true → 90-day prune blocked, `dispatch.ndjson` leaks. (overlaps CP-51 Task-251.) **Fix (`F-7`): done** — `dispatch_recovery.go` / `dispatch_settle.go` — wire `SettleDriver.DriveSettle`/`RetrySettleWithBackoff` into `ScanDispatchRecoveryOnBoot` (or the post-`commitTerminal` call site) so every `SettlePending` record gets a real settle attempt (owned by Task-251 per the 2026-07-17 handoff — land it once there).
...
- `F-7` **(A2, A3) done** Wire the CP-51 recovery/settle remediation: surface stopped+sent stranded records to an operator/attention path and terminalize; give `SettleDriver` a production caller (boot scanner / post-terminal). Reconcile with Task-250/251 (`Q-3`).
...
| 10 | `A2`, `A3` | `dispatch_recovery.go`, `dispatch_settle.go` / boot settle wiring in `dispatch_live.go` | **done** — coordinate with Task-251 (`F-7` handoff) |
...
- upstream docs that must change when fixes land:
  ...
  - SD-24 / CP-51: **resolved (`Q-3`)** — `A2`/`A3` stay in this bug's scope, not folded into Task-250/251; Task-251's `T-1`/`T-3` (settle-driver production caller) is delivered here as `F-7` instead, and Task-251 references this bug rather than duplicating the work. When `F-7` lands, update Task-251 §8 and CP-51's ledger together with this doc.
