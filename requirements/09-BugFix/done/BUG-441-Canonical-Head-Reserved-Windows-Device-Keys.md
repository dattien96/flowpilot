...
- Parent Documents: [BUG-427](../done/BUG-427-Preexisting-Suite-Failures-Frozen-Writer-Context-And-Platform-Deps.md), [CA-927b](../../../change-audit/CA-927-portable-canonical-head-feature-key-validation.md)

## 2. Symptom and Impact
`unsafeHeadFeatureKey` (`internal/changecontract/head.go:69-88`) rejects blank, `.`/`..`, `<>:"/\|?*` and controls <0x20 but accepts `CON`, `con`, `NUL`, `PRN`, `AUX`, `COM1`, `LPT1`. `<key>.json` is Windows-reserved even with extension. `StageHeadWrite` (`:140-155`) can stage on POSIX but fail or target a device on Windows, contradicting CA-927b's portability promise. Severity: **medium**.
...
## 5. Resolution (2026-09-23, CA-930b)
