...
- `V-4` **Live re-verification** — done 2026-07-09: maintainer re-ran the same reproduction (rag-harness clone + "History Only Context" instance still carrying `totally.unknown.source`) via "Run Flow" against the rebuilt runner. The launch now surfaces a visible error (BUG-270 closes the last mile — this fix alone made `ValidateFlowArtifactBindings` return the error, but `resolveWorkflowFlowRef` was separately swallowing it; see BUG-270) instead of either running silently (pre-fix) or falling back to plain chat (the BUG-270 gap found in between).
