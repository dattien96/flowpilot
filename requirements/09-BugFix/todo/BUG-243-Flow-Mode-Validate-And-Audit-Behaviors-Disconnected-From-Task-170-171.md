...
- Phase: `bugfix`
- Status: `deferred` — **diagnosed + fix-plan-ready, deliberately PARKED (2026-07-08, owner direction).** The full fix plan is detailed and ready below (`F-0` mid-flow inline-node execution + `F-1/F-2` validate/audit wiring + `F-3` audit UI; `K-1..K-6`; `V-1..V-6`), but **not scheduled now**: current CP-41 focus is narrowed to **the Plan-step context harness only** (the `context` node — delivered via [CP-44](../../07-Coding-Plan/todo/CP-44-Pluggable-Context-Source-Registry.md)). The `validate`/`audit` execution wiring on this bug is **flow-engine plumbing for the `validate`/`audit` nodes**, orthogonal to the context harness, so it is **un-folded from [CP-43](../../07-Coding-Plan/todo/CP-43-Change-Contract-And-Canonical-Intent-Signature.md)** and tracked standalone here, ready to pick up when the Testing/Audit half of Flow Mode is prioritized.
