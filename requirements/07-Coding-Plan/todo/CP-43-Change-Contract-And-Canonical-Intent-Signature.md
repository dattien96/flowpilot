...
- Reviewers: `TBD`
- Created: `2026-07-03`
- Last Updated: `2026-07-07` (folded in CP-42's two unresolved context-harness DOD items — see §11)
...
- Related Documents: [SD-17: Context And Regression Engine](../../06-System-Tech-Design/SD-17-Context-And-Regression-Engine.md) (D-11), [CP-35: Context And Regression Engine Rollout](../done/CP-35-Context-And-Regression-Engine-Rollout.md), [CP-23: Context Control & Wrong-Way Detection](./CP-23-Auto-Learn-To-Skill.md), [CP-32: UnitTest Rule](../done/CP-32-UnitTest-Rule.md), [CP-42: Flow Pack And Generic Node Behavior Refactor](../done/CP-42-Flow-Pack-And-Generic-Node-Behavior-Refactor.md) (folded-in DOD items, §11), [BUG-243: Flow Mode Validate And Audit Behaviors Disconnected From Task-170/171](../../09-BugFix/todo/BUG-243-Flow-Mode-Validate-And-Audit-Behaviors-Disconnected-From-Task-170-171.md) (folded-in scope, §11)
- Replaces: `None`
...
- `Q-1` Activating `SD-17 D-11` needs a design owner: does this CP warrant an `SD-17` addendum (a `D-13` "intent-signature & change-contract" decision) before build, or is this CP's model sufficient as the design of record?
- `Q-2` Contract capture UX — explicit AI declaration turn vs inferred-from-first-diff-then-confirm. Explicit is cleaner but adds a turn; inferred is cheaper but weaker.
- `Q-3` Should `r-scope` ever block at **file** level (structure absent), or always warn until symbol-level truth is available? (Ties to `SD-17 R-1` false-drift risk.)
- `Q-4` Canonical Head granularity: per `feature_key` only (v1), or also per code-unit (symbol) where GitNexus supplies stable identity?
...
- **Dependencies:** requires `CP-35` `changeledger` + `flowgate` + `structure` merged (they are). `P-2` symbol-level drift is gated on `structure.Available()`; degrades to file-level otherwise. `P-5` packing extends `CP-35 §4.2.1`.
...
>
> **Also folded into this CP's rework (2026-07-06):** the RAG-harness Flow-Mode **pipeline execution** gap documented in [BUG-243](../../09-BugFix/todo/BUG-243-Flow-Mode-Validate-And-Audit-Behaviors-Disconnected-From-Task-170-171.md) — the flow executor has no path to run an inline behavior node mid-flow (so `validate`/`audit` never execute after the coder), and the `command.validate`/`artifact.audit_draft` handlers are stubs not wired to their Task-170/171 implementations. Per owner direction this "RAG-harness mechanism for flow" is reworked here rather than patched standalone. When §1 is expanded, capture the mid-flow inline-node execution + validate/audit wiring + audit-draft UI as explicit CP-43 scope items.
>
> **Also folded in (2026-07-07), from [CP-42](../done/CP-42-Flow-Pack-And-Generic-Node-Behavior-Refactor.md) §10:** CP-42 shipped built-in-pack loading, the node behavior registry, and edge-driven flow execution, but closed with two DOD items unresolved because they are context-harness rework, not generic-flow-engine plumbing — they are moved here rather than left open against a done plan:
> - RAG Harness runs from a pack definition with **no `isPlanStepType`/`isCodingStepType` runtime branch** (CP-42 `P-8`; the alias-normalization shim landed, the literal step-name branches did not get removed).
> - **Context packages become typed artifacts** with producer/consumer bindings declared by flow data, not a CP-41-only special case (CP-42 `P-4`; `context.produce`/`context.render` currently wrap the existing `BuildFlowContextPackage`/`ComposeFlowCodingPrompt` helpers rather than replacing them with flow-data-declared bindings).
>
> When §1 is expanded, fold both into the same mid-flow inline-node execution / validate-audit-wiring scope above — they are the same underlying "context-harness is still Go-literal, not pack-data" gap.
