...
| --- | --- | --- |
| ACP transport contract (`initialize`/`session/new`/`session/prompt`/`session/update`/`session/request_permission`/`turn_completed`) | Verified live | CP-authoring probe against `grok 0.2.93` + `grok.com` login; captured logs `grok_acp_log*.txt`. |
| Approval round-trip (server→client `session/request_permission` with `options[]`; reply `{outcome:{selected, optionId}}`) | Verified live | `write` gated → approved via `allow-once` → file written; wrong `optionId` → `PermissionRejected`. |
| Tool + file events (`tool_call`/`tool_call_update` with diff) | Verified live | `list_dir` + `write` tool frames with `rawInput`/`rawOutput`/diff. |
| Real server session id + resume shape | Partially verified | `session/new` returns real `sessionId`; `session/load` resume request shape not yet exercised (`Q-2`). |
| MCP over ACP `mcpServers` + ambient-scan hazard | Verified hazard, integration pending | `session/new{mcpServers:[]}` accepted; ambient `.mcp.json`/compat auto-load leaked a credential — must disable + integrate FlowPilot MCP. |
| Sub-agents | Signal only | Native `spawn_subagent` present in `initialize` tool list; bridge integration not built. |
| Usage/quota | Signal only | Turn-time `402 personal-team-blocked:spending-limit` observed; no machine-readable quota source confirmed (`Q-5`). |
| Adapter / registry / desktop UI / accounts | Not started | Greenfield (repo-wide search for `grok`/`GROK` returned zero matches). |
| Full CP-46 DOD parity | Not complete | Requires `Task-206..212`; every capability flag stays false until its `GR-*`/live check passes. |
