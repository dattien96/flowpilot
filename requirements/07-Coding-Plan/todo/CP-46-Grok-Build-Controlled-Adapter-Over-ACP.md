...
| Approval round-trip (server→client `session/request_permission` with `options[]`; reply `{outcome:{selected, optionId}}`) | Verified live (original CP authoring only) + unit-tested | The decision-vocabulary encoder (`grok_permission.go`) and full YOLO on/off routing (`grok_adapter.go handleInbound`) are implemented and covered by fake-dispatcher tests (`grok_adapter_test.go`). Re-verifying the live write-approval round-trip end-to-end was not re-run in this pass (the connected test account's `~/.grok/config.toml` has `permission_mode = "always-approve"`, which itself is new evidence that a real machine can carry a stale Grok-side bypass — exactly the BUG-069-class risk the runner-side YOLO=false gate is now built to be immune to, since it never reads Grok's own config). |
...
| Real server session id + resume shape | Partially verified | `session/new`/`session/prompt` real `sessionId` confirmed live. `session/load` resume request shape (`Q-2`) was implemented per the ACP spec (fetched from agentclientprotocol.com: `session/new`/`session/prompt` have no vendor fields beyond `_meta`) but not exercised against a real resumed session in this pass. |
