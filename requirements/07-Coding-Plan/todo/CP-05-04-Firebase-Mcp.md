...
- [x] `DOD-4` `context_artifact.v1` bật `firebase.crashlytics` bind vào step → runtime hỏi crash target → prompt handoff có target note bounded. — Task-231: cơ chế + prompt-note + degrade path đã test; **production adapter giờ đã wire thật** (`firebaseToolsMcpAdapter`, `firebase_tools_mcp_client.go`) — một MCP **client** Go thật nói JSON-RPC chuẩn tới subprocess `firebase-tools mcp --only crashlytics` (đường chính thức CP-05-04 đã chốt), test end-to-end qua `TestFirebaseToolsMcpAdapterFetchEndToEnd` (giả lập subprocess bằng `sh`, không cần `npx`/GCP thật). Vẫn **chưa chạy qua một flow thật** (cần GCP project thật để verify credential live) — đó là phần còn lại của DOD-7.
