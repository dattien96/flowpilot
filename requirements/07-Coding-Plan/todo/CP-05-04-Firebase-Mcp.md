...
- [x] `DOD-3` Firebase MCP server chốt (`Q-1` → official `firebase-tools` MCP) + provider-config injection, stale detection có test. — Task-230 (Claude); **Task-234 mở rộng đủ 4 provider** (Claude/Codex/Gemini/Grok, `EnsureFirebaseMcpProviderConfig` dispatch) + **per-turn auto-merge thật** (`flowpilotClaudeExtraMCPServers` — trước đó `EnsureClaudeFirebaseMcpConfig` chỉ ghi `.claude.json` nhưng Claude chạy `--strict-mcp-config` nên live turn không thấy được entry đó; giờ merge vào `--mcp-config` mỗi turn, tự động theo integration đã connect, không cần bấm thủ công per-account).
- [x] `DOD-4` `context_artifact.v1` bật `firebase.crashlytics` bind vào step → runtime hỏi crash target → prompt handoff có target note bounded. — Task-231: cơ chế + prompt-note + degrade path đã test. **Task-234 revisit**: production adapter (`firebaseToolsMcpAdapter`, `firebase_tools_mcp_client.go`) built ở Task-231 hoá ra **thừa** — nguồn này bị filter khỏi collect (`flow_executor.go`), Fetch không bao giờ chạy trong live turn (CP-05-06 `R-1`: REST/MCP-client chỉ dành cho pick-list, không phải live content). Đã **unwire** khỏi `AttachRunner` (giữ nguyên file/code theo yêu cầu, chỉ bỏ wiring), live path chỉ còn prompt-note.
