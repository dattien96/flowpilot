...
| BUG-478 | merge card alternates + discard (CA-979) | tournament empty winner → card + discard | ☐ pending — needs tournament run reaching empty-winner escalation (heavy provider legs). |
...
| BUG-480 | vibeResumeConfirm via gate-decision (CA-981) | vibe park → generic Continue → gate consumed | ◐ IN PROGRESS 2026-09-25: vibe-cp-ingest run `run-130763` on CP-01 — flow churns owner-debate remediation loops (bed `go test` gate fails); has not reached `cp_writer→task_slicer` confirm boundary. Leg needs a vibe run reaching the park. |
