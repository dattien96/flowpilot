...
| BUG-481 | durable resolve phases + replay (CA-972) | kill mid-resolve → restart → converge | ◐ PARTIAL LIVE 2026-09-25: happy path verified — discard resolved via durable pipeline, session record carries `worktree_resolution_*` phase fields, converge to `discarded`. Kill-mid-resolve replay leg pending. |
