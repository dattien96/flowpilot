...
| A-55-2 | Amend→v2→resume | live on grok previously; re-run | live | ◑ guard leg verified live: `POST /agent-loop/amend` on non-parked run-79069 → `409 flow_not_blocked` typed error (not a silent no-op). Full amend→v2→resume needs a scope-drift park on a flow coder — deferred; unit `bug366_amend_allow_feature_keys`/`TestBug366*` + `changecontract` Amend tests green |
