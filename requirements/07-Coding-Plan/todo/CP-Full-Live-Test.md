...
| BUG-502 | second-project run durable rows absent after restart | chat run on project db51ec26 → turn → kill/restart | ⚠ LIVE-FOUND 2026-09-25 (b501) — **symptom confirmed, mechanism unproven**: `run-260777`/`run-260889` wrote zero `sessions.ndjson`/`approvals.ndjson` rows (turns file intact → never deleted); post-restart `run_not_found`/`chat_not_found`, `appr-260887` lost. Control `run-260775` (workspace) persisted+rehydrated fine; fresh second-project `run-266599` on same build writes rows and survives a graceful restart → write path exonerated; in-process repro test passes → no project gate in code. Suspects: cross-process rewrite by idle second runner sharing `sessions.ndjson` (port 19500, build e85776b3), or torn-tail ordering at kill-9. **Not fixed blind** — instrumented-repro plan + regression guard `bug502_cross_project_session_persist_test.go` in `requirements/09-BugFix/todo/BUG-502-*.md`. |
| A-1/A-58/A-67 + A-60 | task-harness chain + vibe-ingest on post-fix build | gate observe → flowRef-armed runs | ◐ PARTIAL LIVE 2026-09-25 (b501): `run-261204` armed `flowRef` on turn (not run-create — contract) → child `run-261219` `contract-planner` spawned on grok; `run-261500` vibe `flowRef:vibe-ingest` → child `run-261547` `ingest_reader`/`vibe-intake` running. Both parents were in-flight when the B-51 kill-9 drill cut the runner → terminalized `cancelled` (correct recovery contract, but chain completion not re-proven this build). Prior full-chain evidence stands (§A rows on earlier builds). |
