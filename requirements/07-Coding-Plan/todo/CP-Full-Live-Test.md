...
| A-61-3 | cp-harness reject path live | cp_reviewer `changes_requested` → writer re-entry (deferred from M-wave) | unit 2.4 | ◑ live attempt run-100134: engineered "thin CP" prompt — doc-writer template contract still produced conforming CP-03 (full DoD/risks/metadata) → reviewer legitimately approved. Reject leg can't be baited via prompt while writer output conforms by construction; needs a genuinely deficient doc or missing-verdict injection. audit escalate leg verified same run: tier-3 gate `code changed without declared Change Contract` on dirty bed → `waiting_question` + audit WAITING_USER_APPROVAL (fail-closed correct) |
