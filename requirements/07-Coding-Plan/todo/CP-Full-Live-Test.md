...
| B-51-7 | Quiet-flow self-recover | post-SIGKILL silent flow re-drives hub (BUG-404); stale terminal commits retry bounded (BUG-409) | unit | ◑ run-47170: re-drive works but only via operator resume + gate confirm — no autonomous re-drive observed on this run; BUG-404's silent-flow self-drive leg not separately drilled |

### B-2 CP-59 — chat SSOT cross-provider

| ID | Case | Pass criteria | Auto | Status |
|----|------|---------------|------|--------|
| B-59-1 | Switch legs | devin→opencode (and back) → new leg, `includedTurnCount`, E-9 divider, target identity | chat SSOT tests | ◑ cht_10a27db90766: devin leg (run-49030) → codex leg (run-49042) legSeq 1, handoff `raw`, `includedTurnCount:1`, E-9 divider correct, old leg closed `provider_switch`; switch-back → devin leg (run-49071) legSeq 2 created. **2 live bugs found+fixed**: BUG-465 — codex answered both turns with identical text → `collapseRepeatedFinals` dropped turn-2's answer from rendered timeline (raw records intact; CA-962, verified: seq 11 now renders); BUG-466 — switch on fresh process before lazy transcript-writer init → silent `fresh_start`, no seed turn, zero context on new leg + wrong E-9 stats (CA-963; pre-fix E-9 for run-49071 left in bed as evidence). Re-verify of fixed switch-back pending. |
