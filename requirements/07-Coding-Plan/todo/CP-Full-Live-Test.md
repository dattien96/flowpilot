...
| CP-84 | realtime mux legs L-1..L-7 (post-fix build) | full matrix on b501 | ☑ LIVE 2026-09-25 (b501): **L-1** runs `run-260775`/`run-260777` on projects `957928cc`/`db51ec26` both upsert on ONE stream with runId/projectId/kind; **L-2** fresh-attach snapshot carries `run-260777` `waiting_approval` + pending gate + `appr-260887` (catch-up); **L-3** grok→devin switch mints `run-260889` legSeq=1 — upsert arrives on same stream, no reattach; **L-4** disconnect → create `run-260901` → reconnect snapshot contains it exactly once, no dup, `run-260777` rev unchanged; **L-5** `appr-260887` deny → `{"status":"accepted"}` → card resolves, run back `running`; stale dispatch `expectedRev:999` → **409 `dispatch_conflict`**; **L-7** DELETE `run-260901` → reconnect snapshot drops it (no stale item). **L-6** `actionable` markers verified on wire; malformed-payload arm remains unit-covered (no server injection seam). |
| CP-82 | multi-project + worktree uniqueness | mux + 2 worktree runs same repo | ☑ LIVE 2026-09-25 (b501): two-project mux evidence via L-1 above; worktree isolation: `POST /client/workflow-runs {worktree:true}` requires `X-Client: desktop|tui` (bare call → `worktree_client_forbidden` correct fail-closed); with header → `run-260959`/`run-260961` → distinct dirs `.flowpilot/worktrees/cht_b26d7b9c8188`/`cht_9e6f4ba9dae4` + distinct branches `fp/run-7b9c8188`/`fp/run-4ba9dae4`, `state:active`, same git repo. |
| CP-71 | worktree resolve matrix L-1..L-8 (post-BUG-501) | full matrix on b501 | ☑ LIVE 2026-09-25 (b501, 8/8): **L-1** worktree run binds `state:active`/ownerId/path/`fp/run-*`/`baseCommit`; **L-2** uncommitted `wt71/keep.go` → keep_branch → 409 `requiresConfirm:true`+`uncommitted` bare body; **L-3** confirm → 200 `kept_branch`+`wres-260963`, branch survives, dir removed; **L-4** untracked `scratch71.txt` → discard → 409 `untracked` → confirm → `discarded`, dir+branch gone; **L-5** clean worktree → keep_branch → 200 immediate, no confirm; **L-6** main-side conflict → apply_patch → 409 `conflict:true`+`patchArtifactRef`+`wres-260973`+`merge_pending` (no `requiresConfirm`); **L-7** revert probe → retry SAME resolutionId → `applied:true` `merged` (durable phase resume — BUG-481 evidence); **L-8** uncommitted i-t-a file `wt71_merge.txt` landed in main patch. |
| B-51/B-59 | dispatch durability + multi-leg restart (post-fix) | kill -9 mid-turn → restart → reconcile; legs timeline | ☑ LIVE 2026-09-25 (b501): killed mid-`send_started` `turn-262206` (reviewer child `run-262020` of task-harness `run-261204`) → boot marks it `uncertain` seq 4055 + sibling `turn-262917`/`run-262912` (vibe-intake child) — **explicitly-uncertain, zero phantom retry**; operator resolve → `terminal_cancelled`, idempotent replay 200; `dispatch-attention` clears. Parents (`run-261204`/`run-261500`) terminalized `cancelled`. Workspace multi-leg chat `cht_d03564afc2e8` timeline intact post-restart. |
| BUG-502 | second-project run durable rows absent after restart | chat run on project db51ec26 → turn → kill/restart | ☑ LIVE-FOUND + ROOT-CAUSED + FIXED 2026-09-25 — **Root cause (proven)**: out-of-band `git reset --hard HEAD~1` at 19:30:50 (CP-71 L-7 probe cleanup) rewound git-tracked `.flowpilot/chats/sessions.ndjson`+`approvals.ndjson` on the bed — reflog + file birth times + byte-identical-prefix match vs committed blob all line up; 68 rows destroyed while runner kept appending on the reverted file, memory kept phantom state. Write path exonerated (in-process repro + fresh `run-266599` persisted fine). **Fix (b502)**: per-op blocking `flock` on `<file>.lock` (cross-process exclusion for append+rewrite), size+mtime guard + full resync-to-disk on any out-of-band change (dropped ids logged explicitly), `.flowpilot/` appended to `.git/info/exclude` + git-tracked WARNING. **Live re-verified on b502**: second-project `run-273982` persists; replayed `git checkout HEAD -- sessions.ndjson` (390→318) → next write logged `resynced to disk: 14 durable rows no longer on disk` with run_ids; sibling `:19500` append (`run-82865`) merged into `:19400` via resync (visible in project history); post-restart rehydrate consistent (`cancelled`/`idle`, no phantom). Guards: `bug502_external_revert_test.go` (6 tests) + `bug502_cross_project_session_persist_test.go`. CA-1003. |
| A-1/A-58/A-67 + A-60 | task-harness chain + vibe-ingest on post-fix build | gate observe → flowRef-armed runs | ☑ LIVE 2026-09-25 (b502): `run-278048` task-harness ran to `flow_run_complete_done` — full 13-node chain DONE: contract-planner (`run-278053`) → context → doc-writer (`run-278260`, Task-2 spec) → reviewer (`run-278587`) → plan_synthesis → freeze → scaffold-architect (`run-279085`) → coder ×3 → validate → reviewer → synthesis → audit. Human gates exercised live: plan_approval_park resolved via `agent-loop/continue` (empty feedback = approve → `plan_approval_approve`), coder blocked-verdict → `flow_control escalate` → operator `continue` re-drove loop. Coder pass-1/2 correctly `blocked` on out-of-scope stubs (TitleCase/TitleLabel); operator implemented them on the bed → pass-3 coder green, `synthesis_negotiation` SKIPPED (clean review). Earlier b501 partial: `run-261204`/`run-261500` armed `flowRef`, children spawned, cut mid-flight by B-51 kill-9 drill → `cancelled` (correct recovery contract). |
