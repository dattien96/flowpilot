...
- Child Documents: `Task-248, Task-249, Task-250, Task-251, Task-252, Task-253, Task-254, Task-255, Task-256, Task-257 (P-0 spike)`
...
- **P-0 status: PARTIALLY CLOSED.** The Codex/Grok evidence subgate is complete: both probes conclude **unprovable ⇒ `uncertain`**, so neither provider has an unsafe acceptance or attach seam and both may enter only the three-outcome V2 rollout. Claude/Gemini are explicitly V2-disabled pending their own Task-257 evidence. The document-approval prerequisite is now recorded; the remaining open piece is the deferred provider evidence scope.
...
- `Q-2` **Resolved (2026-07-16):** receipts are defined per provider by the SD-24 §6.3 **capability matrix**. Initial enabled scope: Codex and Grok have recorded `unprovable ⇒ uncertain` outcomes, so `thread/start`/`session/new` and observed headless output are **not** receipts and neither adapter calls `Accepted`. Claude spawn→`writeUserTurn` and Gemini one-shot remain V2-disabled until Task-257 records their outcome. The protocol is safe under either answer (no proof ⇒ `uncertain`).
...
- `P-0` **Approval gate — STATUS: PARTIALLY CLOSED**: (1) SS-17 approval, (2) SD-24 acceptance, and (3) §3.6 governance answers remain required. (4) Task-257 is complete for **Codex/Grok only**: `CE-CG` is ✅ with explicit `unprovable ⇒ uncertain` outcomes; Claude/Gemini are V2-disabled until `CE-CL/GEM` completes. The scoped evidence result permits no unsafe receipt/attach wiring; production coding still requires the recorded document approvals.
...
- database (**decided — SD-24 §6.5**): Supabase tables `dispatch_records`, `dispatch_effects` (UNIQUE run/turn/effect_kind, payload-bearing — cohort entries + release manifest live here), `repair_records` (incl. `quarantine_blob`+hash), **`run_protocol_activations` (non-prunable)** + transactional RPCs (`dispatch_cas_advance`, `dispatch_commit_receipt_clear_intent`, `dispatch_commit_terminal_settle`, `dispatch_record_effect`, `dispatch_open_repair`, `dispatch_begin/commit_repair_resolution`, `dispatch_retry_as_new`, **`session_upsert_guarded`**); `session_runtime` carries only the display mirror. Local: single-writer `dispatch.ndjson` commit log (activation lines non-prunable; session upserts serialized on the same writer), disk-before-RAM, torn-tail detection, lock + compaction protocol.
- external systems: initial V2 providers are Codex/Grok with no evidence-backed acceptance/reconcile/attach seam (three-outcome handling only); Claude/Gemini are provider-enable-blocked until Task-257. A session/thread id or stdin write is **not** a receipt.
...
| `FF` | Every **defect-regression** test fails on HEAD `6ea5417`, passes after fix (foundation tests exempt — see `MU`) | 255 | PR evidence | ☐ |
...
| `PG` | Store contract suite passes on a **real PostgreSQL/Supabase** (conditional transitions + concurrent claims) | 253/255 | env-gated integration run + PR evidence | ☐ |
...
| `PAR` | **Backend parity**: full-field `ProviderSessionState` round-trip contract passes on both stores (incl. `ChangeType`/`SourceDocID`/`TurnCount` — verified missing locally); local prune never removes a run with non-terminal dispatch/unfinalized settle | 248/255 | shared parity contract test | ☐ |
...
| `CE-CL/GEM` | **Capability evidence deferred — Claude/Gemini**: their Task-257 outcomes remain unrecorded; both are V2-disabled, not silently assumed safe | **257** | deferred matrix cells + provider-enable guard test | ☐ |
| `RC` | Receipt boundary is explicit: only a future Task-257-evidenced adapter predicate may call `TurnBridge.Accepted`; Codex/Grok session creation/headless output never does, and Claude/Gemini are V2-disabled | 249/255/257 | negative adapter seam + provider-enable guard tests + evidence links | ☐ |
