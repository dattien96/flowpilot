...
- [x] M-4: **DONE 2026-09-20** — architecture verified via direct HTTP inspection + automated: HTTP `POST .../flow-control` handler is schema-only by design (returns 200/`done` for incomplete verdicts); AC coverage enforcement lives at `turnBridge.SubmitFlowControl` (bridge layer) — `TestReviewACCoverage_*` 12/12 PASS confirm enforcement. Prior live: task-harness `run-708317` (grok-4.5) ran plan→implement→reviewer; agent did not voluntarily omit AC-3.
- [ ] ⏸ AWAITING USER (UI-only): Decision card hiển thị trên Desktop + TUI; chọn option gửi option_id; prose fallback hoạt động. Backend/logic đã pin bằng automated DecisionCard TUI tests (3/3 PASS) — chỉ còn visual/interaction cần user verify trên UI thật.
...
- [x] M-8: **DONE (backend/logic)** — dev backend + continuation + live vibe≥80 non-pause verified (2026-09-18); sub-items dưới đây đều PASS. Chỉ còn UI drift card pixels — xem mục ⏸ AWAITING USER ở CP-23 §6 (cùng một UI surface).
