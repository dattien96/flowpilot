...
- `Q-2` **(RESOLVED 2026-07-13 → FlowPilot-owned Bot-API proxy)** Khảo sát: không có official Telegram MCP; community hoặc là MTProto user-account (over-scoped, ban risk) hoặc <25★ single-maintainer. Chốt: self-host proxy tối thiểu expose đúng `send_message` (wrap `sendMessage` Bot API), không `npx` package chưa vet, tool surface tối thiểu, không lộ user account. Xem [Task-232](../../08-Task/done/Task-232-Telegram-Output-Artifact-Type-And-Bot-API-Proxy-MCP.md).
...
- `P-5` **Auto-approve gate.**
  - `send_message` chỉ thực thi khi integration `autoApprove=true` (keyring); UI toggle trong MCP Settings; default refuse.
- `P-6` **UI config.**
...
- `P-8` ~~**Use-case wiring:** built-in/pack flow~~ **out of scope** (2026-07-15): user tự bind `telegram.v1` OUTPUT trong workflow editor; không seed Investigate Bug / Analyze Sprint / Telegram notification pack.
...
- [x] `DOD-3` Telegram MCP server chốt (`Q-2` → tự viết FlowPilot-owned Bot-API proxy) + provider-config injection; bot token chỉ ở keyring. — Task-232 (Claude); **Task-234 mở rộng đủ 4 provider** (Codex/Gemini/Grok qua `EnsureTelegramMcpProviderConfig` dispatch) + **per-turn auto-merge thật** cho Claude/Grok qua `flowpilotClaudeExtraMCPServers` (trước đó `EnsureClaudeTelegramMcpConfig` chỉ ghi `.claude.json`, bị `--strict-mcp-config` bỏ qua ở live turn — đã fix, tự động theo integration đã connect, không cần bấm per-account). Test thật với `httptest` (không phải live Telegram).
...
- [x] `DOD-6` Auto-approve-gated send: mặc định refuse (`MCP_TOOL_APPROVAL_REQUIRED`); user bật **Allow Telegram sends** trong MCP Settings → `send_message` gửi thật. Per-send approval queue **removed** (CA-318); v1 = keyring `autoApprove` only. User live-verified 2026-07-15.
- [x] `DOD-7` End-to-end send path: user bind `telegram.v1` OUTPUT manually + bật auto-approve → live `send_message` delivers (user-verified). Built-in/pack flow seed **won't-do** — manual wiring only.
