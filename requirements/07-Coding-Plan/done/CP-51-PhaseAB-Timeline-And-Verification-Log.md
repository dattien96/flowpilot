...
- Status: `done` (2026-07-24 — see closure note below)
- Owner: `FlowPilot`
- Created: `2026-07-17`
- Last Updated: `2026-07-24`
- Parent Documents: [CP-51](./CP-51-Durable-Turn-Dispatch-State-Machine-And-Recovery-Reconciliation.md), [Task-238](../../08-Task/done/Task-238-Flow-Mode-Node-Edge-Behavior-State-Machine-Hardening-Audit.md), [BUG-288](../../09-BugFix/done/BUG-288-Flow-Mode-Three-Tier-Gate-And-Change-Contract-Reentry-Gaps.md)
...
> on operator confirmation. Kept as a historical record of the verification
> work, not deleted — a future CP/BUG touching this same dispatch/flow-graph
> seam should read it before assuming a scenario here was never checked.
...
| # | Scenario | Expect | ☐ |
|---|----------|--------|---|
| X1 | Full review-loop happy path + V2 on | Flow complete + dispatch terminal rows per AI turn | ☑ done (2026-07-24) — live `run-82214` (Grok hub, Gate-sandbox), API-driven (`POST /client/workflow-runs` + `/turns` with `subMode:"bug", flowRef:"flowpilot-core-flow-pack/review-loop"`, mirroring the desktop's own Chat→bug→Review Loop picker). Canary bug re-seeded (`Add` off by 156) for genuine work. Round 0: coder (`run-82219`, Codex) fixed `calc.go` for real (`return a+b` confirmed on disk) → 2 reviewers (`run-82647`/`run-82655`, Codex) → cohort join → hub synthesis `continue` (3 open issues). Round 1: 2 reviewers re-run (`run-84229` + one more) → synthesis `done`, `flow_control_done`, `loop_status:"done"`. Every turn's dispatch row (`turn-82216/82652/82660/83878/84234/84997`, both hub rounds + all 4 reviewer turns) shows `state:"terminal_completed"` — no `uncertain`, no dangling `send_started`. |
...
| | | Live A7–A9 | ☑ A7 automated-covered (2026-07-22); **A9 ☑** | A9 closed run-38462 (2026-07-22); A7 live desktop click-through still optional; A10 residual live re-open run-24377 ☑ (2026-07-24, operator confirmed); A11/A13 live closed |
...
| | | Live X1–X4 | X1 ☑ done (2026-07-24, `run-82214`); X2 ☑ done (2026-07-24, §2.2/§2.4/§2.5 bundles all green); X3/X4 not separately measured — not blocking (see §3.5 worked example for the same latency/log-noise reasoning) | |
...
- **C6 restore-side unverified:** only the sync-UP half was exercised live; "no empty overwrite of newer local on restore" (the doc's core anti-regression claim) has no live evidence yet — needs either a genuine second machine or a locally-simulated clean-state restore. ~~Separately, Grok restore specifically is a known, pre-existing, larger gap (`restoreTargetPath` only supports Codex/Claude; Task-210 itself already noted this as out of scope) — not addressed by BUG-310, which fixed only the sync-upload side.~~ **Cập nhật 2026-07-23 (đã sửa, BUG-312/CA-406):** operator hit đúng gap này khi bấm Restore 1 chat Grok thật trong REMOTE CHATS (`409 sync_integrity_failed`, live-reproduced trước khi sửa). Root cause: `restoreTargetPath` chỉ có `case` cho Codex/Claude, Grok rơi vào `default`. Fixed bằng cách thêm `case ProviderKeyGrok` — tính lại thư mục đích từ `cwd` của máy hiện tại (dùng lại `grokSessionDirPath`, cùng cách `relocationTargetPath` đã làm cho cross-account relocation nội bộ) thay vì tái sử dụng nguyên path đã percent-encode theo cwd của máy nguồn (sẽ ghi vào thư mục `LocateSessionFile` không bao giờ tìm lại được trên máy khác). 3 test mới (unit path-computation, round-trip cross-cwd, end-to-end sync→restore) đều fail trên baseline pre-fix với đúng lỗi live, pass sau fix. Live re-verify: restart runner, restore lại đúng `run-22023` từng 409 trước đó → `200 restored`, file nằm đúng thư mục mã hoá theo cwd hiện tại. "No empty overwrite on restore" (phần đầu bullet này) vẫn chưa có evidence riêng — BUG-312 chỉ đóng phần "Grok restore hoàn toàn không hoạt động", không phải property anti-regression đó. **Cập nhật cùng ngày (BUG-313/CA-407):** restore chạy được xong thì lộ tiếp lỗi *nội dung* — chat restore mở lên mất prompt, agent card dồn xuống đáy, prose hub sai. Root cause: manifest sync chưa từng mang theo turn-log sidecar (`<runID>-turns.ndjson`) — input duy nhất mà toàn bộ reconstruct timeline sau restart đọc (raw prompts, chuỗi session-id per-turn, transcript_turn frames, anchor card). Fixed: manifest thêm `turnLog`, restore ghi lại sidecar (skip khi local đã có). Red-first TDD 3 test (fail đúng lỗi trên HEAD chưa fix, cross-provider Codex+Claude+Grok), live round-trip thật trên `run-44695`: sync → xoá sidecar → restore → **byte-identical**; restore lần 2 không nhân đôi. Lưu ý: chat đã sync bằng manifest CŨ vẫn restore thiếu prompt cho tới khi re-sync từ máy còn sidecar gốc; restore-side vẫn tải lại full dispatch.ndjson (~3.4MB) mỗi lần (mirror CA-397, follow-up riêng). **Cập nhật 2026-07-23 (BUG-315/CA-409 — C6 restore-side coi như ĐÓNG):** lỗi cuối lộ ra khi restore đã chạy đúng — restore 1 flow chat rồi gửi follow-up bất kỳ thì flow **tự chạy lại toàn bộ** (spawn lại coder + reviewers + synthesis) thay vì follow-up vào hub. Root cause cùng lớp BUG-313: manifest sync mang đủ mọi field flow-runtime **trừ `TurnCount`** → hub restore về `turnCount=0` → cả `startTurn` lẫn `resolveWorkflowFlowRef` (đều gate trên `turnCount==0`) tưởng đây là turn đầu của chat mới. Restart cùng máy không dính vì đọc `TurnCount` từ local store (persist đúng). Fix: manifest mang `TurnCount` + guard `restoredFrom` ở cả 2 đường start-flow (heal luôn manifest cũ). Red-first cross-provider Codex/Claude/Grok, full-sweep 18=18 y hệt baseline (không regression), live: re-sync→delete→restore `run-46797` cho `turn_count=1` (không phải 0), follow-up KHÔNG spawn child mới (giữ 8), hub tự trả lời (`turn_count` 1→2). Với BUG-312/313/314/315 đóng, **C6 restore-down đã verified live end-to-end**; chỉ còn property "no empty-overwrite of newer local" (BUG-091 guard) là chưa có test live 2-writer riêng. **Cập nhật cùng ngày (BUG-314/CA-408, bug khác — restart cùng máy chứ không phải Drive restore):** operator test cùng 1 flow shape trên cả 3 provider (Codex `run-46797`, Claude `run-53157`, Grok `run-45881`) — riêng Claude sau restart mất hẳn card round 2 (coder + reviewer), dù live đã chạy round 2 thật (coder commit fix, reviewer request changes, coder commit lại). Root cause: flow Claude chỉ có 1 reviewer (không cohort) — số activation của 1 node reinvoke được suy ra từ đếm prompt turn-log, bị chặn trần bởi heuristic "peer start-time wave" (suy round 2 từ thời điểm start của các child KHÁC); không có child khác thì không có wave, nên trần luôn co về 1 bất kể chạy thật bao nhiêu lần. Codex/Grok thoát nạn chỉ vì flow của họ có ≥2 reviewer/round nên tình cờ có đủ tín hiệu wave. Fixed: `resumedParentAgentAnnotations` ưu tiên đọc sidecar step-transition (Task-239, ghi RUNNING/DONE thật của từng node) khi node đó chỉ có đúng 1 child claim label — chặn bằng `labelCounts` để không đụng tới case Codex (label dùng chung bởi nhiều child request-run mới mỗi round, vẫn đúng, để nguyên đường cũ). Red-first: repro fail đúng lỗi trên baseline (`git stash` cô lập), pass sau fix, cross-provider Codex+Claude+Grok cùng shape, guard test khoá case Codex không bị đếm nhân 3. Full sweep baseline 17 fail (16 pre-existing + 1 flake TempDir không liên quan) vs fix 16 fail — không fail mới. Live re-verify trên chính 3 run thật của operator: Claude hiện đủ 2 card mỗi bên (coder+reviewer), Codex/Grok không đổi. **Cập nhật cùng ngày (BUG-316/CA-410, bug khác — Grok restore chạy được nhưng follow-up sau đó fail):** operator report restore Grok xong, chat tiếp thì fail sau 3 lần "recovering" rồi "Path not found." (Claude ok). Nghi vấn ban đầu về cwd/process-pool của tôi SAI (operator chỉ ra đúng: cùng topology này đã chạy `session/load` thành công lúc 09:19 sáng cùng ngày) — điều tra lại từ đầu. Root cause thật: Grok là provider DUY NHẤT resume bằng state phía provider — `session/load` đọc nguyên thư mục session (chat_history.jsonl + events.jsonl + updates.jsonl + prompt_context.json + system_prompt.txt + summary.json + signals.json + resources_state.json + rewind_points.jsonl + announcement_state.json, tổng ~12-14 file), nhưng manifest sync (từ BUG-310) chỉ từng mang đúng 1 file (chat_history.jsonl). Xoá chat = xoá sạch thư mục provider (xác nhận thực nghiệm), nên restore dựng lại chỉ còn 1/12 file — đủ hiển thị lịch sử nhưng không đủ cho Grok tự resume. Fixed: manifest thêm `providerFiles` (Grok-only, loại `*.lock`), restore ghi lại đủ vào cùng thư mục (dùng lại path đã tính cho file chính từ BUG-312, cùng độ nghiêm ngặt conflict/integrity). Red-first 3 test + guard cross-provider (Codex/Claude không bao giờ có `providerFiles`), full sweep fix=17/baseline=16 (1 fail thêm là `TestFinalizerHookSurfacesArtifacts`, đã xác nhận pass 3/3 isolated — flake order-dependent đã biết, không phải regression). Live: tạo chat Grok thật → 1 turn → sync → **xoá hẳn** (xác nhận cả 12 file biến mất) → restore (9 file non-lock dựng lại, size khớp gốc) → resume → gửi turn tiếp → **Grok trả lời thật** ("bug316 followup ok"), không còn "Path not found.". Với BUG-312/313/314/315/316 đóng, **C6 restore-down Grok đã verified end-to-end kể cả continue sau restore**; property "no empty-overwrite of newer local" vẫn là gap duy nhất còn lại, chưa có test live 2-writer. **Cập nhật cùng ngày (BUG-317/CA-411, bug khác — KHÔNG phải restore/sync mà là hệ quả của thao tác xoá nhiều chat cũ khỏi HISTORY trong chính đợt test này):** tạo chat mới + run flow (Claude) fail ngay ở hub turn đầu với `dispatch record already exists (HTTP 502 / dispatch_prepare_failed)`, run Failed 0/N step, không child nào spawn. Root cause: `seedIDCounter` (BUG-117) chỉ seed id counter từ session index; dispatch store CP-51 là nguồn id persisted THỨ HAI (keyed `(run_id,turn_id)` trong per-project `dispatch.ndjson`) mà seed không hề biết. Xoá chat khỏi HISTORY bỏ entry session-index + `run-*-turns.ndjson` nhưng KHÔNG xoá dispatch records → session-index max tụt xuống dưới suffix mà record còn sót vẫn giữ → counter re-seed thấp → `nextID` tái sinh đúng id đó → `prepareDispatchV2`→`CreatePrepared` thấy record cũ khác envelope hash → `ErrAlreadyExists`. Bằng chứng đĩa thật: shard `db51ec26` giữ 182 run_id/312 turn_id nhưng session index chỉ còn 33 run; log `id counter seeded to 53698 from 33 persisted runs` < orphan `turn-53903`; `run-53890` có 28 dòng dispatch grok cũ (turn-53892/53903 terminal_completed) và KHÔNG còn `run-53890-turns.ndjson`. KHÔNG phải regression từ BUG-312→316 (không cái nào đụng seed/dispatch-prepare) — latent gap BUG-117×CP-51 bị phơi ra bởi bước delete. Fix: `SetDispatchStore` seed counter vượt `MaxPersistedIDSuffix` của dispatch store (thêm method trên memory + multi-project store, có đường reload-from-disk), optional interface `dispatchIDCeiling`, provider-agnostic (chỉ theo numeric suffix). Vì trong 1 process counter chỉ tăng nên seed đúng lúc boot đóng hẳn collision VÀ tự vô hại hoá orphan hiện có. Red-first cross-provider Codex/Claude/Grok (`suffix 11` vs `want >53903`), full sweep fix=18/baseline=16 (2 cái dôi đã chứng minh pre-existing flake: `TestGeminiAdapterPromptPrepAndEnv` pass isolation; `TestStartTurnGrok...` flake y hệt baseline 5-run pass/pass/FAIL/pass/FAIL, hardcode id nên seed không thể ảnh hưởng), `provider-accounts.json` xác nhận không bẩn sau sweep. Live real-data (không đụng runner đang chạy): `MaxPersistedIDSuffix` mở trên copy read-only shard thật = **54181** (turn-54181), vượt xa orphan gây lỗi; wiring vào service thì nextID > 54181. Nạp fix vào runner live bằng cách restart (`just dev`) — boot log sẽ hiện `id counter seeded to 54181 from durable dispatch records (BUG-317)`. **Cập nhật 2026-07-24 (BUG-322/CA-421 — property "no empty-overwrite of newer local" coi như ĐÓNG):** đào sâu property còn treo duy nhất của C6 thì phát hiện guard `localAhead` (BUG-091) tự nó **chưa đủ**, không chỉ thiếu test: chỉ bảo vệ 4 field cũ (`LastPrompt`/`LastMessage`/`Status`/`UpdatedAt`), không bảo vệ các field flow-runtime thêm sau (`TurnCount` từ BUG-315, `LoopState`, `ActiveFlowNodes`/`ActiveFlowEdges`, `AgentStatus`, `PendingAgentContext`, `FlowCohortID`) — nghĩa là restore lần 2 của 1 chat đã tiến xa hơn cục bộ có thể đè `TurnCount`/`LoopState` về giá trị cũ trong manifest, đúng nguy cơ tái sinh triệu chứng BUG-315. Fixed: mở rộng khối `localAhead` để bảo vệ thêm các field đó (`TurnCount`/`LoopState.Round` so sánh số `>`, còn lại check non-empty), giữ nguyên phạm vi Codex-only đã có. Red-first (`TestRestoreChatRunFromDrivePreservesFlowProgressFieldsWhenLocalAhead`, fail đúng lỗi trước fix), full sweep fix=2345p/17f — khớp y hệt 17 fail pre-existing đã cô lập trong sweep BUG-321 cùng phiên, không regression. Với fix này, **property "no empty-overwrite of newer local" đã đóng cho phần flow-runtime field** (phần rollout-file-bytes + 4 field cũ đã đóng từ BUG-091). Giới hạn còn lại (không phải gap mới, đã ghi rõ trong BUG-322): flow hub không có transcript (BUG-250 read-only restore) không có tín hiệu file-level để biết "local ahead", chưa được fix này phủ.
- ~~CP-51 **full barrier matrix B0…B8e + model suite** chưa đủ (Task-255 phase 2).~~ **Cập nhật 2026-07-22 (lỗi thời, đã sửa):** dòng này viết trước khi audit §10.1 ledger của [CP-51-Durable-Turn-Dispatch-State-Machine-And-Recovery-Reconciliation.md](../done/CP-51-Durable-Turn-Dispatch-State-Machine-And-Recovery-Reconciliation.md) — audit đó (cùng ngày) đối chiếu từng dòng ledger với test thật, chạy + mutation-verify, kết quả: **56/56 dòng ✅** (`GR` verify bằng `MU` thay thế do không replay lại được trên `6ea5417`; `CE-GEM` là non-goal xác nhận, không phải gap). Chỉ còn thật sự treo: `NR` có 18 fail khác phát hiện trong `internal/runner` nhưng nằm ngoài scope dispatch (đáng điều tra riêng, không chặn ledger này).
- Live provider e2e (Codex/Claude/Grok) phụ thuộc account/env.
- ~~BUG-288 formal status vẫn `inprogress` đến re-review sạch + CP-51 closure.~~ **Cập nhật 2026-07-24 (đã đóng):** CP-51 §10.1 ledger 54/56 ✅, 2 dòng còn lại (`GR`, `CE-GEM`) operator-waived. BUG-288 và CP-51 đã move sang `done/`.
...
- ~~C13–C17 chưa chạy live~~ **Cập nhật 2026-07-22 (lỗi thời, đã sửa):** dòng này viết trước khi §3.3 chạy live C13/C15/C16/C17 cùng ngày (C16/C17: kill+restart thật với automated watcher; C15: 3 lệnh API trực tiếp; C13: xác nhận unit-only là đúng thiết kế, không có UI surface). Chỉ C14 vẫn unit-only theo đúng lý do đã ghi (Task-257 chưa có adapter nào gọi `Accepted`, không phải do thiếu công sức verify).
...
| [BUG-288](../../09-BugFix/done/BUG-288-Flow-Mode-Three-Tier-Gate-And-Change-Contract-Reentry-Gaps.md) | Multi-round review epic |
| [CP-51](../done/CP-51-Durable-Turn-Dispatch-State-Machine-And-Recovery-Reconciliation.md) | Turn durable SM |
