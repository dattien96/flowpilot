...
- Reviewers: `TBD`
- Created: `2026-08-27`
- Last Updated: `2026-09-05` (closed per CA-744; live validation S/A/B/C/F3 done, B4 partial, D skipped, E1→BUG-357)
...
- Child Documents: [Task-304: Dual Back-Edge Validator And Edge-Driven Re-entry](../../08-Task/done/Task-304-Dual-Back-Edge-Validator-And-Edge-Driven-Re-entry.md), [Task-305: Task-Harness 11-Step With Plan Writer And Plan Review Loop](../../08-Task/done/Task-305-Task-Harness-11-Step-With-Plan-Writer-And-Plan-Review-Loop.md), [Task-306: CP-Harness 12-Step With CP Plan And Task Splitter](../../08-Task/done/Task-306-Cp-Harness-12-Step-With-Cp-Plan-And-Task-Splitter.md), [Task-307: Harness Plan Artifact Types And Panel Parity](../../08-Task/done/Task-307-Harness-Plan-Artifact-Types-And-Panel-Parity.md)
...
- [x] `DOD-3` (code half 2026-09-04) `task-harness` 12-node builtin loads (`LoadBuiltinPack`), shows in `/flow` picker (`selectableIn:flow`), `cloneable:true`, `cap:3`, `acceptance_nodes` includes `plan_synthesis`; `bug-harness` ships as byte-identical clone of `rag-harness` (Task-305 T-5 option (a): id/description/label + `chatBaseline` dropped, `TestBugHarnessPackClone` green, inventory 9→10). Remaining: picker visual confirm on live `/flow` + Supabase mirror row check (operator items).
