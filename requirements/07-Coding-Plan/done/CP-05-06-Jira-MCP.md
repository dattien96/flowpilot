...
- `P-7` ~~**Use-case wiring:** built-in/pack flows~~ **out of scope** (2026-07-15): user tự bind `context_artifact.v1` + `jira.issue`/`jira.sprint` trong workflow editor; không seed Investigate Bug / Analyze Sprint pack.
...
- [x] `DOD-2` **Partial (API-token Rovo path) — 2026-07-13.** Connect với email+API token (legacy form) → keyring; **Rovo MCP** auth primary = `Basic base64(email:apiToken)` + URL `https://mcp.atlassian.com/v1/mcp` (official headless). Configure Providers / live merge không bắt buộc bearer. OAuth browser 3LO **vẫn out of scope**. Org admin may need to enable Rovo MCP API token auth. REST legacy code kept; MCP Test Console UI retired.
...
- [x] `DOD-4` Provider-config injection trỏ Atlassian remote MCP; stale detection có test. — Task-228 (Claude) + Task-234 (Codex/Gemini static + Claude per-turn merge) + **G2/CA-300 (Grok `config.toml` url+headers + ACP HTTP forward)**. Bearer vẫn paste/persist tay (**DOD-2** OAuth browser còn mở). Tests: `TestEnsureJiraMcpProviderConfigDispatchesToClaude/CodexAndGemini/Grok`, `TestEnsureGrokJiraMcpConfig*`, `TestGrokACPExtraMCPServersForwardsStdioAndHTTPEntries`.
...
- [x] `DOD-6` Prompt-injection + preflight tổng quát hóa (không còn hardcode `google_drive`); Jira preflight chặn step khi chưa connect/OAuth stale với lỗi rõ. — Task-227/228; **G2:** `PreflightJiraMcp("grok")` + `checkGrokJiraMcpConfig` (tests ready/missing/stale). Preflight **codex/gemini** vẫn follow-up (Ensure đã có).
...
- [x] `DOD-8` **Live verified (Claude + Codex) — 2026-07-14.** Chạy thật với Jira demo (`flowpilot899.atlassian.net`, project SCRUM) dùng API token classic/scoped (Basic path, DOD-2): connect → Test pass → Configure Providers → cả **Claude CLI** và **Codex CLI** tự gọi Jira MCP tools và đọc đúng 1 ticket thật (ví dụ SCRUM-6) không bịa dữ liệu; hai chat run qua FlowPilot desktop (run-4731, run-4757, Claude + Codex) cũng đọc thành công qua cùng đường live-merge. Test Console UI đã retired (CA-301) nên tiêu chí gốc "Test Console `jira_list_bugs`" không còn áp dụng — bằng chứng live giờ là provider CLI + FlowPilot chat thật. **Grok**: live-tested cùng phiên phát hiện 2 bug chặn (ACP `session/new` trả "Invalid params"; Grok CLI standalone hiện `jira [unavailable]`) — root-cause + fix đã vào code (CA-306: headers ACP phải là mảng `{name,value}` không phải map; `config.toml` không được rò rỉ `command=""`/`args=[]` cho entry HTTP) và có unit test, nhưng **chưa re-verify live sau rebuild** (cần user rebuild runner + chạy lại Configure Providers + thử lại chat/CLI Grok). Xem thêm CA-302 (fix route connect 404 sinh ra trong cùng phiên), CA-304 (verify probe đổi sang Rovo MCP endpoint để token scoped không còn 401), CA-305 (Claude keychain detection — thiếu cái này thì Claude còn không xuất hiện trong danh sách account để Configure Providers ghi config).
