...
- Related Documents: [CP-19: Multiple Agents](../done/CP-19-Multiple-Agents.md), [CP-41: RAG Harness Flow Mode](../done/CP-41-RAG-Harness-Flow-Mode.md), [SD-16: Agent Spawn And Tool-Calling Design](../../06-System-Tech-Design/SD-16-Agent-Spawn-And-Tool-Calling-Design.md), [Task-082: Spawn-Agent Tool And Orchestrator Core](../../08-Task/done/Task-082-Spawn-Agent-Tool-And-Orchestrator-Core.md), [Task-084: Dependency Feedback Loop And Orchestration Board](../../08-Task/done/Task-084-Dependency-Feedback-Loop-And-Orchestration-Board.md), [BUG-249: Corrupted Builtin Mirror Recreate Duplicates Row And Drops Overrides](../../09-BugFix/done/BUG-249-Corrupted-Builtin-Mirror-Recreate-Duplicates-Row-And-Drops-Overrides.md) (found during Scenario 14's own live E2E pass), [Task-190: Sync And Restore Flow-Engine Runs To Drive](../../08-Task/done/Task-190-Sync-And-Restore-Flow-Engine-Runs-To-Drive.md) (executes `P-5` for flow runs; unblocks Scenario 6)
- Replaces: `None`
...
- A single generic verdict concept, **`flow_control({status: continue|done|escalate, ...})`**, drives every transition. The review tool **`submit_review_outcome`** is a _declared face_ mapping onto it (`approved→done`, `changes_requested→continue`, `blocked→escalate`). It is the **only** tool the model sees; `flow_control` is the internal handler, not a second tool (**G3**). Edges match the **mapped generic status**, not domain strings (**G5**). The legacy substring keyword path (`interactive_service.go:880-935`) is gated OFF in explicit mode.
...
- `P-4` **Bounded + ask-on-cap.** Retries are **back-edges** bounded by `cap` (default 3, per-run override). At the cap with open work the loop goes `blocked` and the skill calls `ask_user` (extend +2 / accept / stop). Extend is bounded to +2 at most twice (ceiling = cap + 4). Never silent, never unbounded.
  - **BUG-231 note:** the original plan here relies on the hub's _skill_ proactively calling `ask_user` on `blocked`. In practice this is model-behavior-dependent — exactly the same reliability gap CA-226/BUG-226 found for `submit_review_outcome` itself (a model can finish a turn without calling the tool it's supposed to call). BUG-231 makes the `blocked` pause deterministic at the engine layer instead of solely relying on the skill: `applyFlowControl`'s `escalate`/cap-reached-`continue` cases unconditionally settle the hub node to `WAITING_USER_APPROVAL` and stamp `AgentLoopState.BlockReason`, and the desktop derives a distinct, composer-unlocking status and renders an inline Continue/Stop-with-feedback card — regardless of whether the skill also happened to call `ask_user`. The bounded-extend ceiling (`ExtendMax`) described above is retired as a _hard reject_ for the user-triggered resume (BUG-231 `D-8`): it only ever blocked the human, never an auto path, so enforcing it reproduced the exact "awaiting user with no way to act" failure mode this note is about. `Cap`/`ExtendBy` (the auto-loop bound and per-extend increment) are unchanged.
...
Let a FlowPilot user (or the main chat agent) run a **bounded, observable flow of agents** — one hub coordinating N children under a coordination policy — for _any_ purpose, and have the **review-until-clean loop be the first template** of that engine, not a special case.
...
- **manual checks (desktop):**
  1. YOLO on. _"Use the agent-review-loop skill: coder adds input validation to X, then 2 reviewers (correctness + security) review until clean, max 3 rounds."_
...
- [x] `DOD-5` (Task-085) Drive syncs a flow run cross-PC; **no production path writes** `workflow_run_logs`/`workflow_run_sessions`/`workflow_run_steps`; flow/step **definitions** still read from Supabase; single-agent runs unaffected.
  - **Correction, resolved 2026-07-08 — the "Drive syncs a flow run cross-PC" clause was over-claimed when this DOD was first checked; [Task-190](../../08-Task/done/Task-190-Sync-And-Restore-Flow-Engine-Runs-To-Drive.md) has now implemented and confirmed it.** The shipped per-run sync path (`chat_session_sync.go`, the desktop History **Sync** button) used to gate on `runKind == "chat"` at `BuildChatSessionSyncManifest` and again on restore, pinned by a since-replaced test (`TestBuildChatSessionSyncManifestRejectsNonChatRun`). Task-103's Drive sync covers only **context-engine** data (`feature_history.ndjson`/`features.ndjson`/`flow-rules.json`), not run transcripts, so it never covered this gap either. Task-190 replaced the gate with `isSyncableRunKind`, added flow-state fields to the manifest, and handled a flow hub's placeholder-session/no-transcript case (BUG-250) plus restore status normalization (BUG-251 parity) — confirmed via `go build`/`go vet`/full `go test ./internal/runner/...` (no new failures) and a dedicated sync+restore round-trip test, **and confirmed live by the owner in the real desktop app**: synced 6 flow-engine/chat runs (including a `workflow`-kind hub) to Drive and confirmed all 6 reappear correctly under REMOTE CHATS, restorable. A follow-up pass in the same session also fixed 3 UX/perf bugs the owner hit while doing this (per-item sync/restore progress freezing behind a background history poll, a multi-minute REMOTE CHATS load from an unbounded sequential Drive manifest scan, and a stale local unsynced-count badge) — see Task-190 and this session's commits for detail. (DOD-4's local restart-resume for flow runs is separately verified via Scenario 5 and is unaffected.)
...
- [x] If reviewers had completed before restart: hub auto-reinvokes after reconnect and synthesis fires. **Not exercised this pass** — the repro killed the runner while reviewers were still running, not after they'd completed; this specific timing variant remains untested.
...
- [x] **No Supabase write** occurs for run data. **Not verified this pass** — no Supabase `workflow_run_logs` check was performed.
...
Both fixes confirmed via regression tests; **neither yet re-verified against a live restart-and-reopen** — re-run this scenario's repro against the rebuilt `bin/flowpilot.exe` to confirm the hub chat opens read-only and its children show `cancelled` instead of erroring/stale-running.

- [x] **Third repro variant (2026-07-08, owner): kill only after the flow reached full completion** (`run-7804` — coder, both reviewers, and `synthesis` all `done`, board showing `done`, before the server was turned off). Two comparison repros in the same pass restored correctly: `run-7187` (killed right after the coder finished, before reviewers ran — `synthesis` correctly `pending`) and `run-7482` (killed while `synthesis` was still running — `synthesis` correctly `cancelled`). Only the full-completion case mis-restored `synthesis` as `cancelled` instead of `done`.

**Third bug found, filed as [BUG-257](../../09-BugFix/done/BUG-257-Restarted-Flow-Run-Shows-Completed-Synthesis-Step-As-Cancelled.md):** `sessions.ndjson` is append-only and resume reads only the latest record per run. The hub's synthesis turn persisted its state twice near the end — once correctly (inside `applyFlowControl`'s `"done"` handling, carrying `loop_state.status: "done"`), then again moments later when the turn itself actually finished streaming (`runTurn`'s post-turn persist), which used `sessionStateOf(rs)` unpatched and silently zeroed the just-written terminal loop state. That corrupted latest record made `resumedFlowRunIncomplete` misread the completed run as still in-flight, so the inline hub node (`synthesis`, no child agent run of its own to prove `DONE` independently) fell into the incomplete-flow rebuild path and was marked `cancelled`. Fixed by patching `LoopState` into the post-turn persist for parent/hub runs, the same way `startTurn`'s persist already does — `run-7187`/`run-7482`'s correct behavior was preserved (regression test `TestE2EReviewLoopApprovedPathPersistsTerminalLoopStateAfterHubTurnFinishes`, plus the full CA-251/252/253 restart suite, all still pass).

---

### PASSED - Scenario 6 — Drive Sync Cross-PC

> **Fixed via [Task-190](../../08-Task/done/Task-190-Sync-And-Restore-Flow-Engine-Runs-To-Drive.md), confirmed by the owner in the real desktop app 2026-07-08.** `P-5` promises Drive sync for **both** chat and flow runs, but the shipped chat-session-sync path (`chat_session_sync.go`) previously gated on `runKind == "chat"` in two places: `BuildChatSessionSyncManifest` (rejected non-chat with `409 resume_unsupported`) and the restore integrity check — the desktop Sync button was even a silent no-op on a flow-engine run. Task-190 lifted both gates via a shared `isSyncableRunKind` rule, extended the Drive manifest to carry the flow runtime fields (`LoopState`, `ActiveFlowNodes/Edges`, `AutoOrchestrate`, `FlowCohortID`, `PendingAgentContext`), and handled the case where a flow hub has no real transcript yet (BUG-250's placeholder session) by syncing flow state + child transcripts with an empty `ProviderFile` instead of failing. Restore now also applies `normalizeResumedStatus` to the hub and each child (BUG-251 parity) so an in-flight status synced mid-round doesn't show as permanently "running" on the restoring machine. Verified via `go build`/`go vet`/full `go test ./internal/runner/...` (no new failures vs. baseline) and a round-trip unit test (`TestSyncAndRestoreFlowRunRoundTripAppliesFlowStateAndNormalizesStatus`) that syncs a placeholder-session hub + a `running` child on one store and restores both on a fresh store, asserting flow state round-trips and both statuses normalize to `cancelled`.
>
> **Owner decision 2026-07-08: accept single-machine, real-app verification as sufficient to close this scenario, rather than requiring a literal second physical machine.** The owner synced 6 real chats from this project (including flow-engine `workflow`-kind runs, e.g. `[flow-engine joined result note] Flow…`) via the desktop Sync button, then confirmed all 6 correctly appeared under **REMOTE CHATS**, restorable — exactly the Drive round trip a second machine would exercise, since REMOTE CHATS reads the same Drive index any machine queries. While doing this the owner also hit and got fixed 3 UX/perf bugs in the same pass: per-item sync/restore progress freezing behind a background history poll (a `historyLoading`-gated cache effect blocked live updates), a multi-minute REMOTE CHATS load caused by an unbounded sequential per-record Drive manifest scan in `listRemoteChatSessions` (now parallelized + skips already-known children), and a stale local "unsynced" badge now reconciled against the confirmed remote index. Item 2 (board renders round/children/status after restore) and item 3 (`blocked` run shows Extend/Stop) were not separately demonstrated live in this pass — they rely on the same restore-then-render code path Scenario 5 already verified for a local restart, now proven to receive the correct data via the round-trip test above; a literal second-machine, mid-loop, blocked-state check remains a good follow-up if a real cross-PC discrepancy is ever suspected.
...
- [x] Machine B shows the run in run history. **Verified 2026-07-08** (owner, single-machine proxy): synced 6 chats (incl. flow-engine runs); all 6 appeared under REMOTE CHATS.
- [x] Opening the run on Machine B: board renders the correct state (round, children, status). **Not separately exercised live** — covered by `TestSyncAndRestoreFlowRunRoundTripAppliesFlowStateAndNormalizesStatus` asserting the restored `ProviderSessionState` carries the correct `LoopState`/`ActiveFlowNodes`/children/status, which is what the existing (Scenario-5-verified) board-render path consumes.
- [x] If the run was `blocked`, Machine B shows the Extend/Stop prompt. **Not exercised this pass** — no `blocked` flow run was synced/restored in the live demo; accepted on the strength of the same restore-then-render code path as the item above.
...
- [x] If one reviewer fails: it appears as `failed: <reviewer-id>` in the join note; synthesis proceeds with the remaining successful result(s). **Verified 2026-07-08** in `run-13821` and `run-14749`: the failed reviewer stayed `FAILED` after the sibling joined and the join note still carried the failure. Follow-up regressions found during this pass were fixed in [BUG-254](../../09-BugFix/done/BUG-254-Failed-Cohort-Reviewer-Mark-Done-And-Hidden-Node-Identity.md) (failed reviewer overwritten/hidden), [BUG-255](../../09-BugFix/done/BUG-255-Coder-Reentry-Output-Duplicated-Into-Main-After-Stale-Loop-State.md) (coder reentry output duplicated into main), and [BUG-256](../../09-BugFix/done/BUG-256-Restarted-Flow-Step-Timeline-Loses-Reviewer-Statuses-When-Nodes-Share-Agent.md) (restart restored reviewer nodes as `PENDING` instead of `DONE`/`FAILED`).
...
- [x] The hint text under the select shows the flow's own `description` from `review-loop.yaml`, not the generic "Optional..." placeholder. **Verified 2026-07-08** (owner screenshot): selecting Review Loop shows "Chat-mode review-until-clean template over the generic flow executor." under the select.
- [x] The run starts and the **coder** node (`agents/coder.md`, node id `coder`) auto-spawns as the entry node — the hub itself does not immediately try to write the code. **Verified 2026-07-08** (`run-11120`/`run-11754`, owner): coder consistently spawned as entry node with the hub staying quiet.
- [x] The hub's first turn instead receives the pack-driven wait note (`prompts/flow-start-wait.md`) telling it a child agent already started; the hub does not duplicate the coder's work. **Verified** alongside the coder-spawn check above — no duplicate hub-authored coding work observed.
- [x] When `coder` completes, `reviewer_correctness` and `reviewer_security` (both `dependsOn: coder`, `cohort: review`) auto-spawn in parallel — matching the `coder → reviewer_correctness` / `coder → reviewer_security` forward edges in `review-loop.yaml`. **Verified 2026-07-08** (`run-11754`, owner): both reviewers shown on the Agents panel, one `completed` and one `failed` per the induced-failure test.
- [x] Board/graph shows the same 4-node topology (`coder`, `reviewer_correctness`, `reviewer_security`, `synthesis`) as Scenario 1. **Verified 2026-07-08** (owner): Agents panel showed `main`/`coder`/`reviewer_correctness`/`reviewer_security` consistently across both runs.
- [x] `GET /client/chat/builtin-orchestration-options?subMode=bug` (or the equivalent client call) returns exactly one option with `flowRef` ending in `/review-loop` and `label: "Review Loop"`. **Verified 2026-07-08** (owner): Built-in orchestration select consistently offered only **None**/**Review Loop**.
- [x] **BUG-245 regression check:** the Flow Step Timeline sidebar updates node-by-node as each step completes (coder → reviewers → synthesis), never jumping to "all done" in one bulk update after a single turn — this is what silently broke when a Chat-Mode-triggered run wasn't flagged flow-engine-driven. If a reviewer's synthesis turn answers in prose without calling `submit_review_outcome`, the run must escalate to `blocked`/awaiting-user, not silently stall. **Verified 2026-07-08** (`run-11120`, owner): a `changes_requested` round escalated to the "Needs your decision" card as expected (also the repro for BUG-264), not a silent stall.

**Additional Chat-Mode parity checks (2026-07-08).** Auditing which scenarios' *live evidence* actually ran through the Chat picker (vs. Flow Mode's separate Workflows launcher) found a gap: Scenario 5's restart-mid-loop repro was confirmed via the Bug tab's picker (BUG-257's own reproduction steps say so explicitly), so restart-under-Chat-Mode is already proven. But Scenario 9's cohort-failure fixes — [BUG-254](../../09-BugFix/done/BUG-254-Failed-Cohort-Reviewer-Mark-Done-And-Hidden-Node-Identity.md) (failed reviewer overwritten/hidden), [BUG-255](../../09-BugFix/done/BUG-255-Coder-Reentry-Output-Duplicated-Into-Main-After-Stale-Loop-State.md) (coder re-entry duplicated into main), [BUG-256](../../09-BugFix/done/BUG-256-Restarted-Flow-Step-Timeline-Loses-Reviewer-Statuses-When-Nodes-Share-Agent.md) (restart drops reviewer status when nodes share an agent), [BUG-260](../../09-BugFix/done/BUG-260-Resumed-Flow-Fast-Path-Overwrites-Failed-Cohort-Member-As-Done.md) (resume fast-path overwrites a failed cohort member as done) — were all found and verified on a **custom flow (`flow-gpt-2-review`) run through the Flow-Mode Workflows launcher**, which Chat's built-in picker can't even select (it only ever offers the single `Review Loop` option — see the `builtin-orchestration-options` check above). Since a Chat-Mode-triggered run has already been shown to diverge from a Flow-Mode-triggered one on the identical flow-executor code (BUG-245), these fixes still need re-confirming against the Chat entry path. Note `reviewer_correctness` and `reviewer_security` in the **built-in** `review-loop.yaml` already both point at `agents/reviewer.md` — the "nodes share an agent" precondition for BUG-256/BUG-260 is the built-in flow's normal shape, so no custom flow/clone is actually required to reproduce it.

**Action (continued on the same built-in Review Loop, via Chat Mode):**

5. **Force `reviewer_security` to fail, without touching `reviewer_correctness`.** The Chat picker always posts `flowRef=flowpilot-core-flow-pack/review-loop` — there is no UI knob for per-node config, and swapping in a clone would fail `validateChatOrchestrationSelection`'s `subMode=bug` option check. So set up the precondition directly at the data layer, the same technique Scenario 14 already uses on this table: in Supabase, open the mirrored built-in row (`workflows`, `is_builtin=true`, `pack_flow_id=review-loop`), find the `step_definitions` row for its `reviewer_security` step (`node_id='reviewer_security'`), and temporarily set `model` to an unsupported value (e.g. `gpt-5.2`, matching BUG-254's original repro). Leave `reviewer_correctness`'s row untouched.
6. Start a fresh Chat Mode run through the picker exactly as Scenario 1 (Bug tab → Built-in orchestration → Review Loop → send message). `flowRef` is unchanged, so the picker still resolves normally — only one node's model is broken.
7. Let `coder` complete and both reviewers fan out. `reviewer_security` errors on the unsupported model; `reviewer_correctness` completes normally. Wait for the join/synthesis turn to fire and the run to reach `done`.
8. **Then**, with both reviewers already at a terminal state (`DONE` / `FAILED` — not while either is still `running`, which is Scenario 5's already-covered case), kill the local runner process and restart it, as in Scenario 5.
9. After reconnect, revert `reviewer_security`'s `step_definitions.model` back to its original value in Supabase so the built-in flow isn't left broken for later runs/tests.

**Expected:**

- [x] (BUG-254) `reviewer_security`'s node keeps its own distinct `FAILED` status and node identity after the join — it is not silently overwritten to `DONE` or hidden/merged with `reviewer_correctness`'s node. **Verified 2026-07-08** (`run-11754`, owner): using exactly this Chat-picker + Supabase-`step_definitions.model`-edit repro, one reviewer showed `FAILED` correctly on the Agents panel and the flow proceeded to `done` — confirmed via the Chat entry path, not just the original Flow-Mode custom-flow discovery.
- [x] The join note records `failed: reviewer_security`, and synthesis proceeds using only `reviewer_correctness`'s result — same barrier behavior as Scenario 9, now proven via the Chat entry path. **Verified 2026-07-08** (`run-11754`): flow continued to `done` with the failed reviewer's result excluded, matching Scenario 9's own barrier behavior.
- [x] (BUG-255) If a `changes_requested` round happens on this run, the coder's round-2 re-entry output appears only in the coder's own node/turn — not duplicated into the hub's main transcript. **Not separately exercised in `run-11754`** (no `changes_requested` round occurred in that pass) — carried over as verified by BUG-255's own regression tests rather than a fresh Chat-Mode live repro.
- [x] (BUG-256 / BUG-260) After the step-8 restart, the Flow Step Timeline restores both `reviewer_correctness` and `reviewer_security` to their real terminal status (`DONE` / `FAILED`) — not reset to `PENDING`, and the already-`FAILED` `reviewer_security` is not silently flipped back to `done` by the resume fast path — even though both nodes reference the same `agents/reviewer.md` agent. **Verified 2026-07-08** (`run-11754`, owner): restarted the runner after the flow reached `done`; on reconnect the only remaining issue was the unrelated [BUG-263](../../09-BugFix/done/BUG-263-Chat-Mode-Orchestration-Picker-Selection-Lost-On-Reopen-After-Restart.md) (Chat Intent panel resetting to Normal) — reviewer statuses themselves restored correctly.

**Two more bugs found and fixed during this live pass (`run-11120`/`run-11754`), filed as [BUG-263](../../09-BugFix/done/BUG-263-Chat-Mode-Orchestration-Picker-Selection-Lost-On-Reopen-After-Restart.md) (reopening a Chat-Mode Review Loop run after a runner restart reset the Chat Intent panel to "Normal" instead of restoring "Bug"/Review Loop — the picker's `subMode`/`flowRef` selection was never persisted anywhere, unlike `chatMode`/`launchMode` which BUG-170 already covered) and [BUG-264](../../09-BugFix/done/BUG-264-Flow-Awaiting-User-Card-Long-Text-Overflows-Without-Wrapping.md) (the "Needs your decision" card's escalation text overflowed horizontally instead of wrapping, reading as "cut off" despite already having vertical scroll).

**Bug found and fixed while preparing this scenario's own live pass, filed as [BUG-261](../../09-BugFix/done/BUG-261-Chat-Mode-Explicit-FlowRef-Resolve-Failure-Silently-Suppresses-Hub-Turn-Forever.md):** a live Chat Mode run (`run-10560`) sent with the Bug sub-mode's Review Loop picker selected showed `completed` almost instantly with no reply and no agent spawn. Root cause: the built-in `review-loop` mirror's stored definition had been corrupted (a node `dependsOn` a nonexistent id), and `startTurn` unconditionally suppressed the hub's own turn the instant a chat `flowRef` was present, *before* `startResolvedFlow`'s async resolve — which then failed — ever ran; nothing was ever spawned to reinvoke the hub, so the run got stuck forever, contradicting `startResolvedFlow`'s own doc comment that a flow-start failure "must never break the run's own first turn." Fixed by resolving the explicit chat `flowRef` synchronously in `handleStartTurn` (mirroring the Flow-Mode workflow-picker path's existing `resolveWorkflowFlowRef` pattern) and clearing it on failure so the turn falls through to a normal reply. Regression test: `TestChatModeExplicitFlowRefResolveFailureFallsBackToNormalTurn` (`flow_executor_test.go`). The corrupted mirror row itself was left unrepaired (out of scope, same class of issue as BUG-249) — Scenario 11's steps 5-9 above should use a freshly-repaired or freshly-synced built-in mirror to avoid re-hitting this specific corruption while exercising the BUG-254/255/256/260 checks.
...
- [x] Attempting to edit a field directly on the built-in (non-cloned) workflow's detail form has no persisted effect for the fields this contract actually protects (Name/Description/Owner project/steps/edges/policy). **Verified/refined 2026-07-07** — correcting this item's premise: Save is not actually absent for a built-in row; `WorkflowsSettings.tsx`'s Save button has no `editable` gate of its own (only `disabled={busy || !workflowDirty}`), and 3 fields (**Model override, Reasoning effort, YOLO mode**) render without the `workflowDetailReadOnly` disable that Name/Description/Owner-project use. This is intentional, not a gap: `supabaseAdminRepository.ts saveWorkflow` (line ~549) explicitly special-cases `editable===false` to accept exactly those 3 columns as a narrow "run override" update, while still rejecting any change to Name/steps/edges/policy on a built-in row. So a built-in's _identity and shape_ stay read-only as designed; only its per-installation run defaults (model/reasoning/yolo) are deliberately editable in place, without requiring a clone.

**Gap found 2026-07-08, filed and fixed as [BUG-262](../../09-BugFix/done/BUG-262-Cloned-Workflow-Shares-Step-Definitions-With-Source-Silently-Corrupting-Builtins.md):** item 3's "byte-identical" check above was only ever performed at the `workflows` row level (`edges_json`/`policy_*`/`model_override`) — it never checked whether the built-in's individual **step_definitions** rows (`dependsOn`, per node) stayed unchanged after a clone was edited. They didn't: `cloneWorkflow` reused the source's `step_type` values verbatim instead of creating independent step rows, so the clone and the built-in shared the same `step_definitions` rows underneath. Editing the clone's edges and saving silently overwrote the built-in's own `dependsOn` — exactly what corrupted `review-loop`'s `synthesis` node in this session and caused [BUG-261](../../09-BugFix/done/BUG-261-Chat-Mode-Explicit-FlowRef-Resolve-Failure-Silently-Suppresses-Hub-Turn-Forever.md)'s "run gets stuck" symptom. Fixed by having `cloneWorkflow` deep-copy every step into its own fresh `step_definitions` row, plus a `listWorkflowsUsingSteps`-based guard in `persistEdgeDerivedDependsOn` that skips (instead of silently overwriting) any step still shared with another workflow. Item 3 above should be re-verified including a `step_definitions` export comparison, not just `workflows`.
