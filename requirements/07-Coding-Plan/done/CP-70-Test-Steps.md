...
| **R6** | ✅ PASS (gate machinery) | `run-876` `flowRef:"bug-harness"` resolve qua embedded pack (workspace `supabase-config.json={}` shadow global demo-ref → store=nil). `preflight_contract_plan` DONE qua Devin child `run-881`/`rogue-cafe`; `contract.freeze` strict-parse REJECT draft → `WAITING_USER_APPROVAL` + loopState `blocked/escalate` + `agent-loop/continue` accept 200. **Finding**: Devin Sonnet từ chối prompt `[SYSTEM_PROMPT]` như prompt-injection → trả prose thay draft — gate escalate đúng thiết kế; tuning prompt template là follow-up |
| **R7** | ✅ PASS | `spawn_agent` trên resumed session → child `run-314` session riêng `blushing-raver` (không reuse slug), trả `CHILD_OK` về parent |
...
- **Mã Run ID kiểm thử E2E:** `run-1` (R1/R2/R3/R5/R8), `run-83`/`run-270` (R4 deny/approve), `run-120` (R9-Grok), `run-314` (R7 child), `run-876` (R6 flow)
- **Kết quả nghiệm thu:**
  - [⬜] Smoke Test (Mục S): **DEFERRED** — TUI steps (`/provider`, `/model`, Ctrl+C restore) chưa chạy; tương đương đã verify qua API (provider select, model switch, resume)
  - [⬜] Settings Desktop (Mục A): **DEFERRED (UI)** — API-level đã verify: `/providers` INSTALLED `3000.10.31`, `/compat` trả `installedDevinVersion`, model catalog `devin/*`; card/buttons chưa verify trên Desktop app
  - [⬜] TUI & State Restore (Mục B): **DEFERRED (UI)** — state restore đã verify qua resume API (R5); TUI picker commands chưa chạy
  - [x] Chat & Streaming (Mục C): **PASS** (R1 chat stream → `end_turn`; R8 cancel → `stopReason:"cancelled"`)
  - [x] Mid-chat Model Switch (Mục D): **PASS** (R3 `set_config_option{model:grok-4-5-low}` cùng session slug — BUG-329 không lặp lại)
  - [x] Approval Gate & YOLO (Mục E): **PASS** (R4: `request_permission` → `waiting_approval`; deny `appr-97`→`reject_once`, approve `appr-284`→executed; YOLO→`bypass`)
  - [x] Sub-Agent Isolation (Mục F): **PASS** (R7 `spawn_agent` → child `run-314` session riêng `blushing-raver`, `CHILD_OK` về parent — BUG-334 không lặp lại)
  - [x] Kỹ Năng & Ngữ Cảnh Tự Động (Skills & Context) (Mục G): **PASS** (`.devin/skills` discovered `source:workspace`; injected → reply `SKILL_INJECTED_OK`)
  - [x] Tư Thế Hội Thoại (Posture) (Mục H): **PASS** (`chatPosture:scan`→`ask` không write; `plan`→plan doc `~/.devin/plans/` không write workspace)
  - [⬜] Session Persistence & Cross-Account (Mục I): **PARTIAL** — I1..I4 resume/restart PASS (`session/load{trusted-airmail}`); I5 Drive sync + I6 cross-account DEFERRED (cần external creds)
  - [⬜] Tóm Tắt Một Lần (Summarizer) (Mục J): **BLOCKED** — `devin -p` dùng REPL credential store tách ACP PKCE; cần operator chạy `devin auth login`
  - [x] Flow Mode (Mục K): **PASS** (R6 `bug-harness` qua embedded pack: `preflight_contract_plan` DONE qua Devin child; `contract.freeze` strict-parse reject → `WAITING_USER_APPROVAL` + `blocked/escalate`; `agent-loop/continue` 200)
  - [x] MCP trong Chat (Mục L): **PASS** (`mcp__flowpilot__ask_user` E2E trên resumed session: `q-260` → answer B → reply B; cần fix MCP-resume `3144188`)
  - [x] Vision Guard (Mục M): **PASS** (image block qua ACP; `claude-sonnet-5-low` đọc ảnh trả "Pink"; grok upstream error surfaced `turn-failed` đúng)
  - [x] Base Regression Guard (Mục N): **PASS** (`go test ./internal/runner` Devin xanh; fails còn lại pre-existing không liên quan CP-70: `TestOpencodeProcessEnvIsolatesWindows`, 5 TempDir flakes, 1 tsc BUG-340)
  - [x] Quota & Timeout Guard (Mục O): **PASS** (`token_usage_updated` + `modelContextWindow:500000`; quota stopReason → `turn-failed` unit-tested `TestDevinAdapterQuotaFailure`)
  - [⬜] Account Usage (Mục P): **PARTIAL** — usage từ `turn_completed._meta` đã normalize (R10); Account Panel UI deferred
  - [x] Chuyển Giao Ngữ Cảnh (Handoff) (Mục Q): **N/A by design** — `supportsHandoffSource(devin)=false` parity với opencode (CA-899 deviation đã ghi)
  - [x] Live Verification gate-sandbox HTTP (Mục R): **PASS** (R1..R10 có runId + sessionId slug + log evidence)
