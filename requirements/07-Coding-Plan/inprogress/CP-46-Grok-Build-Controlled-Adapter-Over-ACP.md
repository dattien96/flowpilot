...
- Child Documents: [Task-206: Grok ACP Transport And Process/Dispatcher](../../08-Task/done/Task-206-Grok-ACP-Transport-And-Process-Dispatcher.md), [Task-207: Grok Controlled Adapter MVP (Chat/Stream/Resume)](../../08-Task/inprogress/Task-207-Grok-Controlled-Adapter-MVP.md), [Task-208: Grok Permission Channel And YOLO Posture](../../08-Task/inprogress/Task-208-Grok-Permission-Channel-And-Yolo-Posture.md), [Task-209: Grok MCP, Ask-User, And Spawn-Agent Parity](../../08-Task/inprogress/Task-209-Grok-MCP-Ask-User-Spawn-Agent-Parity.md), [Task-210: Grok Account Model — Detect, Connect, Switch, Quota](../../08-Task/inprogress/Task-210-Grok-Account-Model-Detect-Connect-Switch-Quota.md), [Task-211: Grok Desktop UI Surface](../../08-Task/inprogress/Task-211-Grok-Desktop-UI-Surface.md), [Task-212: Grok Parity Hardening And Live DOD](../../08-Task/inprogress/Task-212-Grok-Parity-Hardening-And-Live-DOD.md), [Task-213: Auto-Detect And Sync Provider Models](../../08-Task/inprogress/Task-213-Auto-Detect-And-Sync-Provider-Models.md) (spin-off — provider-agnostic model-catalog auto-sync; Grok surfaced the gap), [Task-214: Grok Skill-Catalog And Selection Parity](../../08-Task/done/Task-214-Grok-Skill-Catalog-And-Selection-Parity.md) (spin-off — Grok's native `.grok/skills` support surfaced the gap)
...
- `Q-3` Does YOLO=false gating via live `session/request_permission` behave correctly for **all** tool classes (shell/run_terminal_command, write/edit, MCP tools), or do some tools fast-path without asking (Grok's docs describe safe read/grep fast-paths and note only `bypassPermissions` takes effect via flag)? Determine whether `--allow/--deny` glob rules or a `PreToolUse` hook are needed to force deny-by-default.
...
- `Q-5` Does Grok expose account **usage/quota** anywhere machine-readable (the only quota signal observed was a turn-time `402 personal-team-blocked:spending-limit`), or is the account card limited to email/plan + terminal usage-limit classification?
...
- `Q-7` Can Grok's persisted session SQLite (`~/.grok/sessions/`) be read for cross-provider handoff *source* extraction, or must Grok-as-source stay disabled like Gemini until an extractor is proven?
...
| YOLO off (approval gate) | Dangerous actions emit `permission_required`; deny stops the action. | `session/request_permission` (server→client request w/ `options[]`) → `TurnBridge.RequestApproval`; reply `{outcome:{selected, optionId}}`; deny blocks. | Verified (write approved via round-trip; wrong optionId → `PermissionRejected`). | `GR-04` |
| YOLO on | Runner auto-approves eligible actions; `ask_user` still surfaces. | `YoloPosture.GrokPermissionMode=bypassPermissions` + `RunnerAutoApprove`; `ask_user` never auto-approved. | — | `GR-05`, `GR-06` |
...
- `P-6` YOLO + posture (`Task-208`).
  ...
  - Verify deny-by-default actually holds across shell/write/MCP tools; if Grok fast-paths some tools, add `--allow/--deny` rules or a `PreToolUse` hook to enforce (`Q-3`).
  - Ensure no persisted Grok allowlist (`~/.grok/config.toml`/`.claude/settings.json` compat) can bypass the runner toggle (Claude `CA-079`/`BUG-069` analog).
...
- `R-4` YOLO=false may not gate every tool class if Grok fast-paths safe ops; deny-by-default might need `--allow/--deny` or a `PreToolUse` hook (`Q-3`).
...
- YOLO=false approval (live `session/request_permission` round-trip), YOLO=true auto-approval policy, and `ask_user` question UI work through the runner-owned gate; deny blocks the action.
...
- MCP: FlowPilot tools (+ Google Drive) are visible in the same turn, or the capability is honestly disabled with diagnostics.
...
| YOLO=false approval gate. | Golden-fixture test replays the captured `session/request_permission`, denies (`reject-once`) → action not executed; allows (`allow-once`) → executed. |
| YOLO=true runner policy. | Test proves eligible actions auto-approve only via runner policy and stale allowlists cannot bypass the toggle. |
...
_Updated by Task-206..212 implementation pass (2026-07-09). This table reflects what was **actually built and tested** in that pass, not aspirational coverage — see the honesty note at the end._
...
| Approval round-trip (server→client `session/request_permission` with `options[]`; reply `{outcome:{selected, optionId}}`) | **Verified live (2026-07-10) + unit-tested — real bypass gap found and mitigated** | The decision-vocabulary encoder (`grok_permission.go`) and full YOLO on/off routing (`grok_adapter.go handleInbound`) are covered by fake-dispatcher tests. Live-verified via a controlled experiment against the real test account: with the account's actual `~/.grok/config.toml` (`permission_mode="always-approve"`), a real write tool call executed with **zero** `session/request_permission` round-trip — confirming the account carries a stale Grok-side bypass exactly as suspected. Temporarily flipping `permission_mode` to `"default"` (then restoring the original file exactly) reproduced the correct round-trip: `session/request_permission` fired and a runner deny blocked the write. Since there is no `grok agent stdio` flag to force this server-side, and FlowPilot never rewrites the user's config file (mirrors `ensureClaudeConfigSettings`'s "never clobber" precedent), `grokConfigPermissionModeBypassesGating` (`grok_process.go`) now detects and warns on this condition at process-ensure time — a diagnostic, not a code-level fix for the underlying ACP-level gap. See Task-208 `DOD-5`. |
...
| MCP over ACP `mcpServers` + ambient-scan hazard | Verified hazard (twice), wiring built, **not live-exercised** | Ambient auto-discovery reproduced independently in the second live probe: even with `mcpServers:[]` and both `GROK_*_MCPS_ENABLED=false` flags set, Grok still auto-probed an "atlassian" MCP server sourced from the auto-installed xAI Official marketplace plugin (not from the two env flags at all, and not from `grok mcp list`, which reported empty) — a **new, more specific finding** than CP-46's original note: the two env flags mitigate Claude/Cursor compat-scanning, not Grok's own marketplace-plugin MCP auto-install. `grok_adapter.go` wires `mcpServers` (FlowPilot + Google Drive) and the MCP-ready-before-prompt gate via the reused `claudeMCPServer`; this is unit-tested (`grok_mcp_test.go`) but never run against a live Grok process with the FlowPilot MCP server actually reachable end to end. |
...
| Full CP-46 DOD parity | **Not complete — see honesty note** | A second pass (2026-07-10) re-verified chat/stream/resume and the permission gate against a **real, logged-in Grok account** (not just fake-transport), finding and fixing two real bugs: the `session/load` `_meta.sessionId` nesting gap, and the `permission_mode="always-approve"` ACP-channel bypass (now diagnosed/warned, not silently unenforced). Everything else below is still unit/fake-transport tested only, not a full live credentialed acceptance run. Deferred/not attempted in this pass: MCP-over-ACP tested against a live process; spawn_agent/ask_user live round-trip; multi-account switch live; `~/.grok/sessions` SQLite handoff-source extractor (stays disabled, `supportsHandoffSource(grok)==false`); replay-typed-user-prompt extractor; hub/review-loop-as-Grok; child stop/delete-cascade; the full `E2E-01`..`E2E-36` list. `ProviderCapabilities.Mcp` is only true once an instance is actually registered with a live `mcpServer` (not a static claim); `ApprovalEvents`/`Streaming`/`Resume`/`FileEvents`/`Interrupt` are backed by passing tests; `Mcp`/`SkillSelection`/`Vision` reporting is conservative by design. |
