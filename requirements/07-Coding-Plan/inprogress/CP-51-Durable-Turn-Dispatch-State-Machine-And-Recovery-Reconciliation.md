...
- `P-0` **Approval gate — STATUS: PARTIALLY CLOSED**: (1) SS-17 approval, (2) SD-24 acceptance, and (3) §3.6 governance answers remain required. (4) Task-257 is complete for **Codex/Grok/Claude**: `CE-CG` and `CE-CL` are ✅ with explicit `unprovable ⇒ uncertain` outcomes; Gemini is V2-disabled until `CE-GEM` completes. The scoped evidence result permits no unsafe receipt/attach wiring; production coding still requires the recorded document approvals.
...
**Rows with plausible but not fully-confirmed coverage** — a related test exists and passes, but it wasn't verified to cover the exact sub-case the row states; needs a closer read before flipping:

- `SDa` (Stop mid-settle bookkeeping) — `TestSettleSubBarriers_B8aToB8e_PartialThenResume` covers the B8a-e barriers generally; the specific "Stop mid-settle, dependents suppressed, finalizer still runs" case was not individually isolated in this pass.
- `LR` (scanner re-`Get`s fresh state after claim, before reconcile) — recovery-scanner tests pass, but no test name distinctly isolates the "fresh read after claim" ordering itself.
- `TP` (terminal-proof outcome fidelity across completed/failed/cancelled/completed-before-cancel) — `TestTerminalCommit_DerivesStopOutcomeFromDurableAuthority` overlaps this but a dedicated outcome-fidelity matrix test was not confirmed.
...
- `CE-GEM` — explicitly deferred by the doc itself (Task-257 Gemini evidence not yet recorded); consistent, not stale.
- `GR` — cannot run here at all (no C toolchain for `-race`/CGO); unknown, not failing. Needs a CI run or a machine with `gcc`.
- `TO` — no test distinctly targeting the TOCTOU-on-session-write scenario (a receipt commit racing a stale snapshot write) was found under this name or an obvious equivalent.
