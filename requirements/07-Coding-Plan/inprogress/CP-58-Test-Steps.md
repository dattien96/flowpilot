...
| C cp-harness | ✅ C1/C2/C4; ❌ C5 (BUG-356); ⏳ C3/C6 | run-556588 | C1 PASS (1 spawn entry). C2 PASS (`CP-38-abs-benchmark.md` đủ §1–§8, P-1/P-2/P-3). C3 pending (reviewer approve lần 1, chưa ép reject). C4 PASS (3 Tasks 910/911/912 ↔ P-*, Parent→CP-38, additive). C5 FAIL: audit park `blocked_validation_failed` — slice-only không validate node nên audit không pass được (xem BUG-356); Retry futile → Stop. C6 pending theo C5. |
| D Smoke variant | ⏳ | | |
| E Artifacts | ⏳ | | |
