...
| A Plan loop | ✅ A1/A2/A5; ⏳ A3/A4 | run-548341 | A1 PASS: reviewer reject lần 1 → `plan_synthesis` continue → `plan_writer` re-enter round 1/3; `runner.log`: `doc-writer` spawn đúng 1 lần (`run-548436`), re-entry là `session/prompt` trên CÙNG session `ses_f8fc57…` (không spawn mới) — reuse đúng chuẩn. Timeline lúc re-enter: `plan_reviewer` + `plan_synthesis` PENDING, `plan_writer` RUNNING. A5 PASS: cùng screenshot `context` + `preflight_contract_plan` vẫn DONE. A2 PASS: re-entry prompt chứa "[flow-engine] Feedback received on your last submission… Plan review verdict: changes_requested. Blocking: feature_key `calc-core-lcm`…", không lỗi routing. Lưu ý: reviewer reject vì feature_key `calc-core-lcm` chưa đăng ký (không phải mồi benchmark — mồi không cần thiết nữa, loop đã exercise; contract enforcement này là tín hiệu tốt). Bonus B3: reviewer spawn theo round (`run-548589` r1, `run-548848` r2). Chờ round 2 approve → freeze cho A3/A4 |
