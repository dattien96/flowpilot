...
| C6 | Drive sync round-trip | Sync project chats → Drive; machine B restore same `project_id` | Session + **dispatch** log restore; no empty overwrite of newer local | Both machines files | ☒ **attempted, real issues found — not clean, flagged separately (2026-07-22)**. Sync-up mechanically works (real uploads landed: 9× `[chat-sync] dispatch log uploaded project=db51ec26-… bytes=3522204` in the runner log, correct byte counts, no errors) — but batching gate-sandbox's 17 chats took 6-8 min and the desktop's unsynced-count badge went from "~12/17 synced" backward to showing "15" (regression, not investigated to a root cause this session). Root-caused (code, not guesswork) the dominant known cost: `syncChatRunToDrive` (`chat_session_sync.go:537`) runs once per run in a batch, and unconditionally re-exports + re-uploads the **entire** per-project `dispatch.ndjson` every single call (`syncDispatchLogToDrive`, `dispatch_drive_sync.go:21-45`, no cache/hash/dedup) — same 3.4MB file re-uploaded 9× for one batch, scaling worse as project history grows. Flagged as background task `task_d80cf120` (fix + investigate whether the progress-regression is the same cause or separate) rather than fixed inline, since this touches live production sync code and deserves its own plan/tests. **Method A (direct Drive read via `flowpilot_drive` MCP) did not pan out**: `search`/`listFolder` from Drive root surfaced neither `chat-sessions/` nor `dispatch.ndjson`, most likely because the MCP tool's granted scope is `drive.file` (visible only to files the *calling app itself* created) while the desktop's chat-sync feature very likely authenticates as a separate OAuth client — same human Google account (`cambt1001@gmail.com`), different app-identity, so the MCP tool structurally can't see what the desktop uploaded. Did not pursue Method B (simulate machine B via a second local profile) given the above findings already consumed the session's remaining budget for this case. **Restore-side property ("no empty overwrite of newer local") was not exercised at all.** |
...
| | | Live B1–B5 | ☐ | |
| | | Live C1–C12 | C1/C2/C3/C4/C5/C7/C8/C9/C10/C11/C12 ☑ (C1/C2/C7/C8 via existing evidence/unit re-run, not fresh live click-through; C10 session-reuse confirmed, cold-vs-warm timing not isolated); **C6 ☒ attempted — real perf bug + progress-UI regression found and flagged (`task_d80cf120`), Method A Drive-read blocked by OAuth scope, restore-side untested** | |
...
- **C6 Drive sync perf bug (open, 2026-07-22):** batch chat-sync re-uploads the entire per-project `dispatch.ndjson` once per run instead of once per batch (`syncChatRunToDrive` → `syncDispatchLogToDrive`, `chat_session_sync.go:600` / `dispatch_drive_sync.go:21`) — 9 redundant 3.4MB uploads confirmed live for one 17-chat batch. Flagged as background task `task_d80cf120`. A related, not-yet-root-caused desktop progress-badge regression (synced count going backward mid-batch) was folded into the same task.
- **C6 restore-side unverified:** only the sync-UP half was exercised live; "no empty overwrite of newer local on restore" (the doc's core anti-regression claim) has no live evidence yet — needs either a genuine second machine or a locally-simulated clean-state restore.
