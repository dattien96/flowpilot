...
| | | Live B1–B5 | ☐ | |
| | | Live C1–C12 | C1/C2/C3/C4/C5/C7/C8/C9/C10/C11/C12 ☑ (C1/C2/C7/C8 via existing evidence/unit re-run, not fresh live click-through; C10 session-reuse confirmed, cold-vs-warm timing not isolated); **C6 ☑ sync-up fully closed — perf bug (CA-397), unsynced-count reporting bug (BUG-309), Grok sync-never-worked bug (BUG-310), and perpetual-retry bug (BUG-311) all found+fixed same-day, 0/17 Gate-sandbox runs unsynced after; Method A Drive-read blocked by OAuth scope, restore-side (incl. Grok restore, a separate pre-existing gap) still untested** | |
