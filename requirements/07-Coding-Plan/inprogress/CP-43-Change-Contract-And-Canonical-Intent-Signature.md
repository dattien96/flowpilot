...
- Reviewers: `TBD`
- Created: `2026-07-03`
- Last Updated: `2026-07-27` (status re-audit — **stale metadata corrected**: P-1→P-4 done (Task-184/185/186/187), P-5 Task-188 `in_progress` (còn T-2 budget-drop + T-5 panel-beyond-MVP + live Head-first verify), P-6 Task-259 `draft` (readiness-reviewed, v1 GitNexus-only). Ghi chú `2026-07-13` cũ ("ready to implement P-1") đã lỗi thời từ lâu.)
...
- Child Documents: [Task-184: Change Contract Capture](../../08-Task/done/Task-184-Change-Contract-Capture.md) (P-1, **done**), [Task-185: Scope-Drift Detection](../../08-Task/done/Task-185-Scope-Drift-Detection.md) (P-2, **done**), [Task-186: Canonical Head And Intent Signature](../../08-Task/done/Task-186-Canonical-Head-And-Intent-Signature.md) (P-3, **done**), [Task-187: Superseding Decision Records And Retire](../../08-Task/done/Task-187-Superseding-Decision-Records-And-Retire.md) (P-4, **done** — residual: retire-intent detection, r-attach-spec/r-retire approval wiring), [Task-188: Canonical-Head Packing And Admin Visibility](../../08-Task/inprogress/Task-188-Canonical-Head-Packing-And-Admin.md) (P-5, **in_progress** — T-1/T-3/T-4 done; còn T-2 budget-drop + T-5 panel-beyond-MVP + live verify), [Task-259: source.dependence Context Source](../../08-Task/todo/Task-259-Source-Dependence-Context-Source.md) (P-6, **draft** — readiness-reviewed, v1 GitNexus-only)
- Related Documents: [CP-43-CATALOG: Context Source Catalog And Test Log](./CP-43-Context-Source-Catalog-And-Test-Log.md) (companion — full source catalog, per-source test battery, live E2E), [CP-54: Locus-Anchored Relevance Retrieval](../todo/CP-54-Locus-Anchored-Context-Relevance.md) (**downstream** — tầng retrieval/ranking nằm dưới `feature.history`/`chat.summary`; giải context-dilution khi một `feature_key` gom nhiều chủ đề con, đảo trục truy hồi sang locus vùng-code), [CP-44: Pluggable Context Source Registry](./CP-44-Pluggable-Context-Source-Registry.md) (**dependency** — Canonical Head is a context source cắm vào CP-44's registry; CP-44 lands first), [SD-17: Context And Regression Engine](../../06-System-Tech-Design/SD-17-Context-And-Regression-Engine.md) (D-11), [CP-35: Context And Regression Engine Rollout](../done/CP-35-Context-And-Regression-Engine-Rollout.md), [CP-23: Context Control & Wrong-Way Detection](./CP-23-Auto-Learn-To-Skill.md), [CP-32: UnitTest Rule](../done/CP-32-UnitTest-Rule.md), [CP-42: Flow Pack And Generic Node Behavior Refactor](../done/CP-42-Flow-Pack-And-Generic-Node-Behavior-Refactor.md) (P-8 residual only, §11; P-4 typed-context-bindings re-homed to CP-44), [BUG-243: Flow Mode Validate And Audit Behaviors Disconnected From Task-170/171](../../09-BugFix/done/BUG-243-Flow-Mode-Validate-And-Audit-Behaviors-Disconnected-From-Task-170-171.md) (**un-folded 2026-07-08** → standalone, lands first; no longer CP-43 scope)
- Replaces: `None`
...
- **Update (2026-07-16):** the declared-scope `Change Contract` surfaced through the context-source registry (as anticipated above) landed via CP-50 P-4 / [Task-247](../../08-Task/done/Task-247-Change-Contract-Context-Source-And-Downstream-Prompt.md) (`change.contract` context source + downstream-prompt injection), not as a bespoke `P-5` slot here.
...
### 4.6 `P-6` `source.dependence` context source — blast-radius from the declared contract (added 2026-07-17) → [Task-259](../../08-Task/todo/Task-259-Source-Dependence-Context-Source.md)

`P-2` already computes drift with `structure.Dependents` **inside the flow gate** (a warn/block rule). `P-6` surfaces the *same* GitNexus signal as a **context source** for the prompt instead of a gate: once a run has a Change Contract (`P-1`), the `source.dependence` source reads its `declared_paths`/`declared_symbols`, **normalizes them to concrete impact targets** (symbols first, then concrete code files; dir-buckets/globs/doc files dropped), and for each runs `structure.Provider.Dependents` (`npx gitnexus impact <target> --json`) to render a bounded blast-radius block — "changing this declared target affects these symbols/files/flows". This answers the owner's question *"đã handle context-depend-code qua gitnexus chưa — đổi cái này ảnh hưởng cái nào"* at the context layer.

- Registers **after** `change.contract` (its input) and **before** `source.excerpt`; joins `defaultContextSourceIDs`. Uses priority `3` (same int as `change.contract`) and the stable-sort SourceType tiebreak for exact position — no renumbering of existing sources ([Task-259](../../08-Task/todo/Task-259-Source-Dependence-Context-Source.md) T-2).
- **Target normalization is load-bearing (Task-259 T-3):** an *inferred* contract stores only **top-level dir buckets** (`"apps"`) and `declared_symbols` is essentially never populated, but `gitnexus impact` is symbol/file-oriented — so an inferred contract yields an **empty** section by design; `P-6` is most useful with a *declared* contract listing real files/symbols.
- **v1 is GitNexus-only (Task-259 T-5):** when GitNexus is absent the source renders an honest "not indexed" note rather than the file-level fallback (the fallback provider is uncancellable and walks the whole repo per target — deferred to a follow-up).
- Deterministic (tool/graph, **no AI** at collect; output sorted); degrade-soft (AC-9): no/inferred contract / GitNexus absent / error → empty (warning-free) section, never blocks the turn. Bounded: ≤10 targets, ≤15 dependents/target, one shared wall-clock budget.
- **Not a gate** — `r-scope` (P-2) stays the enforcement path; `P-6` is prompt context only. Never routed through `composeFlowNodeAgentPrompt` (HIGH-risk append path per CP-43-CATALOG §5) — renders via the generic context-package pass only.
- Full source/collect classification, the per-source test battery, the code guide, and the live E2E case (B12) live in [Task-259](../../08-Task/todo/Task-259-Source-Dependence-Context-Source.md) (§4/§6) and the companion catalog: [CP-43-Context-Source-Catalog-And-Test-Log.md](./CP-43-Context-Source-Catalog-And-Test-Log.md) §3/§4/§6.
...
- [x] `P-1` Every code-mutating step has a stored `Contract` (declared or inferred) with `feature_key`, `intent`, `declared_paths`; no-declaration turns do not block. Done via [Task-184](../../08-Task/done/Task-184-Change-Contract-Capture.md), 2026-07-13.
- [ ] `P-2` **Partially done** via [Task-185](../../08-Task/todo/Task-185-Scope-Drift-Detection.md) (`in_progress`), 2026-07-13. An edit outside the declared scope raises `r-scope` (warn by default) with the offending paths ✓; `r-scope` blocks only when configured and `structure.Available()` ✓ — implemented at **file level** (`structure.Dependents` on out-of-scope paths), not true symbol level; genuine symbol-level severity would need diff-hunk→symbol extraction that doesn't exist anywhere in this codebase and was deliberately not built here (avoids the AST cost `SD-21 D-2` rejects). Left unchecked until that gap is closed or explicitly accepted as file-level-only for v1.
- [x] `P-3` **Done** via [Task-186](../../08-Task/todo/Task-186-Canonical-Head-And-Intent-Signature.md) (`done`), 2026-07-13. Each feature has a `CanonicalHead` with a reproducible `intent_signature`; editing a governing spec flips it to `spec_drifted`; an unreconciled out-of-contract code change flips it to `code_drifted`; an in-contract change refreshes the Head to `current`; birth (spec_less) minting; and the `spec_less → current` attach-spec re-baseline is now round-trippable via an explicit desktop confirmation action (`POST .../canonical-head/rebaseline` → `RebaselineWithSpec`). Governing-doc discovery (features → their SS/SD/CP docs) was a latent gap fixed by [BUG-280](../../09-BugFix/done/BUG-280-Features-Never-Link-To-Governing-Docs-So-Canonical-Head-Stays-Spec-Less.md) — docs now declare `Feature Keys:`, without which every Head stayed `spec_less` forever.
- [x] `P-4` **Done** via [Task-187](../../08-Task/todo/Task-187-Superseding-Decision-Records-And-Retire.md) (`done`), 2026-07-13. `A → B → A` churn is represented as current-A + a rejected-B `Decision` with a reason (`FoldDecisions`, wired into every gate-passing `UpdateHead` call); `RetireHead` copies decisions into rename/merge targets and preserves retired Heads for provenance; retire is now callable via an explicit desktop action (`POST .../canonical-head/retire`), and `r-retire` fires as a conservative FEATURE-KEYS.md-based nudge (`detectRetirePending`). Design decision: retire is explicit-human-action, not diff-auto-detected (see Task-187). Note: FoldDecisions inclusion is keyword-based (a rejection phrased without a marker keyword is missed) — a known heuristic limitation, not a gap.
- [ ] `P-5` **Partially done** via [Task-188](../../08-Task/todo/Task-188-Canonical-Head-Packing-And-Admin.md) (`in_progress`), 2026-07-13. Resolved code turns lead with the Canonical Head block before any raw history ✓ (`featureHistorySource.Fetch` prepends `RenderHeadBlock`); the desktop app (`apps/desktop-flowpilot` — `apps/admin-web` is deprecated and was NOT used) has a manual-lookup "Canonical Head" section in `ProjectsSettings.tsx` showing behavior/status/rejected-decisions/step-Contract ✓, but it is not a per-project feature browser and shows declared paths rather than a computed in/out-of-scope diff. Left unchecked because the packer never demotes/drops raw history under budget pressure (no logged drop), and no live manual E2E confirmed prompt ordering for either Claude or Codex.
- [x] Satisfies `SS-14` US-3 (declare + flag), AC-7 (records symbols/files changed, auditable), AC-8 (spec-changed context flagged stale) — at the file-level scope documented in `P-2`/`P-3` above.
- [x] All contract/head/drift computation is non-fatal and retryable (`AC-9`); raw artifact save is never blocked. Every `changecontract` call site in `gate_hook.go` degrades to "nothing to report" on error rather than blocking the turn.
- [x] `go build ./...` and `go test ./internal/changecontract/... ./internal/flowgate/...` pass; no regression in `CP-35` module tests. (233 tests passed across `changecontract`+`flowgate`+`contextsync` combined as of 2026-07-13; full `./internal/runner/...` regression run pending — see below.)
- [x] `contracts.ndjson` stays local-only; `canonical/*.json` syncs to `context-engine/` via the `CP-35 P-8` mechanism. (`contextsync.EngineStore.SharedFiles()`, Task-188 T-4.)
