...
`P-2` already computes drift with `structure.Dependents` **inside the flow gate** (a warn/block rule). `P-6` surfaces the *same* GitNexus signal as a **context source** for the prompt instead of a gate: once a run has a Change Contract (`P-1`), the `source.dependence` source reads its `declared_paths`/`declared_symbols` and, for each, runs `structure.Provider.Dependents` (`npx gitnexus impact <target> --json`, file-level fallback when GitNexus is absent) to render a bounded blast-radius block — "changing this declared target affects these symbols/files/flows". This answers the owner's question *"đã handle context-depend-code qua gitnexus chưa — đổi cái này ảnh hưởng cái nào"* at the context layer.

- Registers **after** `change.contract` (its input) and **before** `source.excerpt`; joins `defaultContextSourceIDs`.
- Deterministic (tool/graph, **no AI** at collect); degrade-soft (AC-9): no contract / GitNexus stale / error → empty section, never blocks the turn.
- **Not a gate** — `r-scope` (P-2) stays the enforcement path; `P-6` is prompt context only.
- Full source/collect classification, the per-source test battery, and the live E2E case (B12) live in the companion catalog: [CP-43-Context-Source-Catalog-And-Test-Log.md](./CP-43-Context-Source-Catalog-And-Test-Log.md) §3/§4/§6.
