...
- Reviewers: `TBD`
- Created: `2026-06-23`
- Last Updated: `2026-09-19` (added AC-21 Contract-First Scaffold TDD CP-67)
- Parent Documents: `Product Vision`
- Child Documents: [SD-17: Context And Regression Engine](../06-System-Tech-Design/SD-17-Context-And-Regression-Engine.md), [SD-22: Pluggable Context Source Registry](../06-System-Tech-Design/SD-22-Pluggable-Context-Source-Registry.md) (US-9), [SD-23: Generic Artifact Framework](../06-System-Tech-Design/SD-23-Generic-Artifact-Framework.md) (US-10, AC-17), [CP-35: Context And Regression Engine Rollout](../07-Coding-Plan/done/CP-35-Context-And-Regression-Engine-Rollout.md), [CP-63: IDE-Grade LSP Runtime](../07-Coding-Plan/todo/CP-63-IDE-Grade-LSP-Runtime.md), [CP-64: Reproduce-First TDD Gate](../07-Coding-Plan/done/CP-64-Reproduce-First-TDD-Gate.md), [CP-66: Living Knowledge Base](../07-Coding-Plan/todo/CP-66-Living-Knowledge-Base-Context-Source.md), [CP-67: Contract-First Scaffold TDD & Signature Lock Gate](../07-Coding-Plan/todo/CP-67-Contract-First-Scaffold-TDD-And-Signature-Lock.md)
...
- Replaces: `None`
- Tags: `context, regression, code-graph, oracle-guard, two-tier-intelligence, lsp, reproduce-first, living-knowledge, contract-first-tdd`
...
- `AC-21` (Contract-First Scaffold TDD, CP-67): For feature/Task flows (`task-harness`, `vibe-sprint`), the TDD stage generates full Production Stubs + Executable RED Tests (compile OK, runtime RED) enforced by `r-scaffold-red` gate. Coder writes only body code; signature lock (`r-signature-lock`, signature hash (chỉ signature, không bao gồm body)) prevents any signature change, addition, or deletion without batch renegotiation via Main Agent ($`negotiationCap: 5`). 100% outcomes use typed tool schemas (`submit_scaffold_outcome`, `submit_coder_outcome`).
...
- `AC-21` (Contract-First Scaffold TDD, CP-67): For feature/Task flows (`task-harness`, `vibe-sprint`), the TDD stage generates full Production Stubs + Executable RED Tests (compile OK, runtime RED) enforced by `r-scaffold-red` gate. Coder writes only body code; signature lock (`r-signature-lock`) prevents any signature change, addition, or deletion without batch renegotiation via Main Agent ($`negotiationCap: 5`). 100% outcomes use typed tool schemas (`submit_scaffold_outcome`, `submit_coder_outcome`).
