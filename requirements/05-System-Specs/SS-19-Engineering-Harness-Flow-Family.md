...
- Document ID: `SS-19`
- Title: `Engineering Harness Flow Family (task-harness, bug-plan-harness, cp-harness)`
- Phase: `system_spec`
- Status: `approved`
- Owner: `FlowPilot`
...
- Child Documents: [SD-19 Agent Flow Engine](../06-System-Tech-Design/SD-19-Agent-Flow-Engine.md), [SD-20 Flow Gate Rule Semantics](../06-System-Tech-Design/SD-20-Flow-Gate-Rule-Semantics.md), [CP-58 Bug/Task/CP Harness](../07-Coding-Plan/done/CP-58-Bug-Task-Cp-Harness-Plan-Review-Loop.md), [CP-62 Zcode Harness Parity](../07-Coding-Plan/done/CP-62-Zcode-Harness-Parity.md)
- Related Documents: [SS-14 Code Context And Regression Safety](./SS-14-Code-Context-And-Regression-Safety.md), [SS-15 Agent Review Loop](./SS-15-Agent-Review-Loop-Until-Clean.md), [SS-18 Vibe Working Mode](./SS-18-Vibe-Working-Mode.md)
- Replaces: `None`
- Tags: `harness, agent-flow, flow-definition, pipeline, tdd, review-loop, task-harness`
- Feature Keys: `agent-flow-engine, flowgate, zcode-parity`

## AI Quick View

### Summary

- Define the standard **Harness Flow Family**: the production-grade, multi-stage engineering pipelines (`task-harness`, `bug-plan-harness`, `cp-harness`) that translate human intent into tested, verified, and audited code.
- Unlike ad-hoc prompt chains, each harness flow follows a deterministic, closed topology:
  $$\text{preflight} \to \text{freeze} \to \text{context} \to \text{tdd} \to \text{coder} \to \text{validate} \to \text{synthesis} \to \text{audit}$$
- Harness flows enforce **TDD-first**: test signatures and expectations must be generated before implementation code.
- Reviews are performed by isolated reviewer agents operating under read-only postures, returning structured per-AC verdict rows rather than free-form prose.
...
- `AC-2` **TDD Mandatory**: Implementation code is never generated without prior test signatures derived from the task specification.
...
- In scope:
  - Standard harness flow topologies (`task-harness`, `bug-plan-harness`, `cp-harness`).
  - Stage contracts and transitions.
  - Acceptance node requirements (`validate`, `synthesis`, `audit`).
- Out of scope:
...
- `AC-1` Stage sequencing must strictly honor: Preflight $\to$ Freeze Scope $\to$ Context Retrieval $\to$ TDD $\to$ Coder $\to$ Validate $\to$ Review Synthesis $\to$ Audit.
...
- `AC-5` The final `audit` node drafts the change-audit note and updates task frontmatter to `Status: done`.

## 6. Definition of Done

- All 3 harness topologies exist as tested FlowDefinitions in `flow-pack`.
- Automated test suites confirm node isolation, retry loops, and fail-closed termination.
