...
- `KD-1` Vibe is a **policy** (`working_mode=vibe`), not a separate orchestrator; the runner still owns Workflow/Session/Gate and the harness `Dev — FlowPilot — AI` / `User — FlowPilot — AI` stays intact.
- `KD-2` Two isolated Owner agents (configurable same or different provider/model) replace every Dev `1/2/3` decision. One Owner is not trusted alone; consensus is required.
- `KD-3` Only `r-requirement` (including green+`TamperedTestPaths` weaken) and a 5-round no-consensus debate escalate to the user; every other `r-*`/`permission_required`/`user_question` is handled by starting `vibe-owner-debate` via the resolver.
...
- `AC-5` Every `r-*`/`permission_required`/`user_question` other than `r-requirement` (including green+`TamperedTestPaths` weaken, which is always `r-requirement` per `BR-4`) is **not** surfaced as a Dev `1/2/3` card in Vibe. Instead, the gate resolver starts the FlowDefinition `vibe-owner-debate` (exactly 2 isolated Owners, `cohort: owner_debate`, `join: all`, `cap: 5`) debating only through Main (hub-only, `SS-15 BR-1` / `SS-16 BR-5`); the resolver knows only the flow id, never `owner.md` (domain-free, `SS-16 BR-1`).
- `AC-6` An Owner debate (`vibe-owner-debate`) is bounded to **at most 5 rounds** (`cap: 5`, `onCap: escalate`). If the two Owners reach consensus within the cap, that remediation is applied automatically without a user card (retry/reprompt/re-scope the sprint). If they do not agree within 5 rounds, the flow escalates to `ask_user` and no further automatic retry occurs until the user resolves.
- `AC-7` Only two cases ever produce a user ask in Vibe: `r-requirement` (requirement drift, including weaken, `BR-4`) or 5-round no-consensus (`AC-6`). Every other gate path terminates `done` or auto-retries under the debate cap, never as a user card.
- `AC-8` In `dev` mode, gate handling is byte-for-byte the current Dev behavior (`T-3`..`T-9` in `SD-20`, Dev `1/2/3` cards) and `r-requirement` is **inert** (vibe-only). A regression test proves `dev` mode does not receive Owner debate and does not fire `r-requirement`.
...
- `AC-10` An out-of-the-box detailed game requirement file drives a full **auto sprint-by-sprint** demonstration (e.g. 3 sprints): each sprint passes `AC-3`/`AC-4`, non-requirement gates auto-resolve via `vibe-owner-debate` (`AC-5`/`AC-6`), and the only pauses, if any, are at `r-requirement` or at Owner cap.
...
- `BR-7` **Vibe is data.** Every Vibe behavior is declared in FlowDefinitions (`vibe-ingest`, `vibe-sprint`, `vibe-owner-debate`) and agents `owner` + `vibe-intake`. The generic engine carries no role strings (`SS-16 AC-3`/`BR-1`). A new Vibe variation requires only YAML + skill, not engine code. No Go code may name `owner.md` or any role.
