...
- **Restore reattach.** A Drive-restored chat has **no locally-active leg** (all legs `closed(restored)` — `SD-26 D-8`). The next user turn / Tab / provider pick reattaches via direct `createRun(chatId, switchFromRunID=latestLeg)` (not the switch endpoint) — envelope seeds the new leg identically; missing sidecars → `session_unavailable`, missing provider binary → `provider_unavailable`, chat always opens with full text.
