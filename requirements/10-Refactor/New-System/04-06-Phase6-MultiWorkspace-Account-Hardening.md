...
- [x] **Multi-workspace:** per-run `cwd` binding is authoritative on the live path (StartRunInput.Cwd → run → TurnRequest → thread/start → admin session view); runs on different workspaces are independent (T-02). _The legacy `runner.go` `ExecutePrompt` path still keys off `r.workspace` and is retired with that fallback (criteria in `04-07-Migration-Notes.md`), not re-audited._
- [x] **Account switch:** `SetActiveAccount` interrupts in-flight turns + marks them recoverable (re-sendable, not silent auto-replay) and scopes the previous account's runs out (`409`) until reactivated (T-43); the app-server **process** recreate-on-scope-change is implemented in `ensureCodexAppServer` (mock-process verified). _Recreate against a real `codex` binary is the live-acceptance step._
...
- [x] Threads account-scoped: resume/turn only while the owning account is active (the `409 provider_account_changed` gate, P2 + T-43). _Codex `thread/resume`/`thread/list` JSONL scoping under `CODEX_HOME` is verified at the live-acceptance step (the gate + scope binding are in)._
...
- [x] Desktop real-wiring (`04-01` Part B): `HttpWsRunnerClient` swap, live stream + reconnect replay, approval + question round-trip, the **real IdeBridge** (code/cursor/studio/xed), and short-name + full-path tooltip are all done (`tsc`/`vite build` green). _A live end-to-end run vs the web path is the live-acceptance step._
