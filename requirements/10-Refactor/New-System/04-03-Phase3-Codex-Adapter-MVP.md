...
- [x] `thread/start` + `turn/start` wired and exercised; `resume/list/read` + `interrupt` param builders present; **`mcpServers` carried on `thread/start`** incl. the `ask_user` tool; the run→thread mapping is in-memory in the InteractiveService. _DB persistence to `workflow_provider_sessions` attaches with live Supabase._
...
- [x] Concurrency: dispatcher multiplexes concurrent calls + per-thread routing (tested, T-38). _A full two-thread adapter run + `thread/list` by cwd against the installed build is the live-acceptance step (T-13/T-23, 06 Part D)._
