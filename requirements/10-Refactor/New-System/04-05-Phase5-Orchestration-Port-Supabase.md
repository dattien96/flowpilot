...
- [ ] Desktop and web runs go through the same Go-runner path (T-20) — **recorded deferral:** the desktop already goes through the runner (`HttpWsRunnerClient`); the web side becomes true once Admin Web is made thin (the item above). Blocked only on that refactor.
