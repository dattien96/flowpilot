...
- [x] Expiry/interrupt while pending: approval/question expires → **recoverable** `turn_failed`; the bridge returns an error on expiry/ctx-cancel so the codex adapter replies **deny** to the inbound request (provider never hangs). _The error `ask_user` **tool-result** variant (vs the deny reply) is exercised once the live MCP proxy intercepts the tool call (T-40)._
- [x] **`ask_user` MCP tool (model-driven, best-effort)**: registered on `thread/start` `mcpServers` (`codexAskUserMcpServer`, discovered via `tools/list`) + prompt reinforcement (`askUserReinforcement`); the pause→`user_question_required`→options card→`AnswerQuestion`→resume mechanism is built and tested. _Whether the model actually calls it is best-effort by nature (needs a live model); the registration + reinforcement are in (T-29)._
- [x] **Workflow-driven question (deterministic)**: `AskWorkflowQuestion` emits `user_question_required` directly at a defined step (no model tool call), same card + resume; tested in P5 (`TestWorkflowDrivenQuestion`, T-30).
